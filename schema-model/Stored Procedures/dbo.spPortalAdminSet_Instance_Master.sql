...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalAdminSet_Instance_Master]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
/*
--UPDATE
EXEC [spPortalAdminSet_Instance_Master]
	@UserID = -10,
	@InstanceID = 304,
	@VersionID = 1001,
	@WorkflowID = 1001,
...
EXEC [spPortalAdminSet_Instance_Master] @GetVersion = 1
*/
...
DECLARE
	...
	@ToBeChanged nvarchar(255) = 'Template state',
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Set Master Instance',
		...
		IF @WorkflowID IS NOT NULL AND @DeleteYN = 0
			BEGIN
				IF @WorkflowName IS NULL OR @ProcessID IS NULL OR @InitialWorkflowStateID IS NULL OR @RefreshActualsInitialWorkflowStateID IS NULL OR @ModelingStatusID IS NULL OR @SelectYN IS NULL
					...
					BEGIN
						SET @Message = 'To update an existing member parameter @WorkflowName, @ProcessID, @InitialWorkflowStateID, @RefreshActualsInitialWorkflowStateID, @ModelingStatusID AND @SelectYN must be set'
						SET @Severity = 16
						GOTO EXITPOINT
					END
