...
SET ANSI_NULLS ON
GO
create PROCEDURE [dbo].[spBR_BR11_sega_old]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		SELECT
			@CallistoDatabase = 'pcDATA_UNI_20240720',--A.[DestinationDatabase],
...
select '#EntityPair1'
					...
					SELECT
					    ...
					    ,[Path]
    					,@EntityParent_MemberId
					    ,CONVERT(nvarchar(20), @EntityParent_MemberId)
					    ,iif([Path] LIKE '%' + CONVERT(nvarchar(20), @EntityParent_MemberId) + '%', 'yes', 'no')
					FROM
						#EntityHierarchy
					WHERE
						[Elim] = 0 --AND
...
-- 							[GL_Product_Line_MemberID],
-- 							[LineItem_MemberID],
-- 							[Financials_Value]
-- 							)
						SELECT [sega08] = 1,
							[BusinessProcess_MemberId] = 190,
							[Entity_MemberId] = 30000033,
							[Account_MemberID] = DCS.[Account_MemberID],
							[BusinessRule_MemberID] = DCS.[BusinessRule_MemberID],
							[Currency_MemberID] = DCS.[Currency_MemberID],
							[Scenario_MemberID] = DCS.[Scenario_MemberID],
							[Time_MemberID] = DCS.[Time_MemberID],
							[TimeDataView_MemberID] = DCS.[TimeDataView_MemberID],
							[Version_MemberID] = DCS.[Version_MemberID],
							[WorkflowState_MemberID] = DCS.[WorkflowState_MemberID],
							[Flow_MemberID] = CASE DCS.[Flow_MemberId] WHEN 121 THEN 122 WHEN 122 THEN 121 ELSE DCS.[Flow_MemberId] END,
							[GL_Company_MemberID] = DCS.[GL_Company_MemberID],
							[GL_Department_MemberID] = DCS.[GL_Department_MemberID],
							[GL_Product_Line_MemberID] = DCS.[GL_Product_Line_MemberID],
							[LineItem_MemberID] = DCS.[LineItem_MemberID],
							[Financials_Value] = DCS.[Financials_Value] * -1
						FROM
							#DC_Source DCS
-- 							INNER JOIN #EntityPair EP1 ON EP1.Entity_MemberId = DCS.[Entity_MemberId]
-- 							INNER JOIN #EntityPair EP2 ON EP2.Entity_MemberId = DCS.[InterCompany_Entity_MemberID]
						WHERE
							DCS.[CheckedEntity] in (30000033, 30000034, 30000035)
