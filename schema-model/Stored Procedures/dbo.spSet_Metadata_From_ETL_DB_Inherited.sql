...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spSet_Metadata_From_ETL_DB_Inherited]
	@UserID int = -10,
	@InstanceID int = 390,
	@VersionID int = 1011,
...
EXEC spRun_Procedure_KeyValuePair
	@ProcedureName = 'spSet_Metadata_From_ETL_DB_Inherited',
	@JSON = '
		[
...
EXEC [dbo].[spSet_Metadata_From_ETL_DB_Inherited] 
	@UserID = -10,
	@InstanceID = 380,
	@VersionID = 1022,
	@Debug = 1
...
--GetVersion
EXEC [spSet_Metadata_From_ETL_DB_Inherited] @GetVersion = 1
*/
DECLARE
	...
	@SourceID_Inherited_Financials int, 
	@SourceTypeID int,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.0.3.2151' SET @Description = 'Set correct ProcedureID. Set correct [SourceFormula] column name.'
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		SELECT
			...
			@SourceID_Inherited_Financials = S.[InheritedFrom],
...
*/
	...
	SET @Step = 'Copy Entity structure from pcETL to pcINTEGRATOR_Data'
		SET @SQLStatement = '
			...
			FROM
				...
				INNER JOIN pcINTEGRATOR..[Source] S ON S.InheritedFrom = E.SourceID
	...
	SET @Step = 'Entity_Book'
		...
		SET @SQLStatement = '
			...
			FROM
				[' + @ETLDatabase + '].[dbo].[Entity] E
				INNER JOIN pcINTEGRATOR..[Source] S ON S.InstanceID = ' + CONVERT(nvarchar(10), @InstanceID) + ' AND S.VersionID = ' + CONVERT(nvarchar(10), @VersionID) + ' AND S.InheritedFrom = E.SourceID
				INNER JOIN pcINTEGRATOR..[SourceType] ST ON ST.SourceTypeID = S.SourceTypeID
	...
	IF @Debug <> 0
		SELECT
			...
			[@SourceID_Inherited_Financials] = @SourceID_Inherited_Financials
		...
		SET @SQLStatement = '
			...
			FROM
				...
				INNER JOIN ' + @ETLDatabase + '..[FinancialSegment] FS ON FS.SourceID = ' + CONVERT(nvarchar(10), @SourceID_Inherited_Financials) + ' AND FS.[EntityCode] = E.MemberKey + ''_'' + EB.Book
			WHERE
	...
	SET @Step = 'Copy data into EntityPropertyValue'	
		SET @SQLStatement = '
			...
			FROM
				...
				INNER JOIN [pcINTEGRATOR_Data].[dbo].[Source] PCIS ON PCIS.InstanceID = PCIE.InstanceID AND PCIS.VersionID = PCIE.VersionID AND PCIS.InheritedFrom = ETLE.SourceID
			WHERE
				ETLE.SourceID = ' + CONVERT(nvarchar(10), @SourceID_Inherited_Financials)
