...
CREATE PROCEDURE [dbo].[spGet_DataClass_DimensionMember_20230921_nehatest]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
AS
/*
EXEC [spGet_DataClass_DimensionMember_20230921_nehatest]
	@UserID=7881,
	@InstanceID=596,
	@VersionID=1091,
	@DataClassID=5795,
	@PropertyList='Account.Sign|Entity.Currency|Time.Level|Time.TimeFiscalYear|Time.TimeYear|Time.TimeMonth|FullAccount.ReverseSign|',
	@AssignmentID=22755,
	@DimensionList='-77|-1|-34|-2|-55|-3|-6|-7|-26|-27|5755|5756',
	@OnlySecuredDimYN=0,
	@ShowAllMembersYN=0,
	@OnlyDataClassDimMembersYN=0,
	@Parent_MemberKey=NULL,
	@DebugBM=7
...
BEGIN TRY
	...
	SET @Step = 'Check if called, else create #ReadAccess'
		IF OBJECT_ID (N'tempdb..#ReadAccess', N'U') IS NULL
			BEGIN
				...
				EXEC [spGet_ReadAccess] @UserID = @UserID, @InstanceID = @InstanceID, @VersionID = @VersionID, @UseCacheYN=@UseCacheYN
			END

			EXEC [spGet_ReadAccess] @UserID = @UserID, @InstanceID = @InstanceID, @VersionID = @VersionID, @UseCacheYN=@UseCacheYN, @StorageTypeBM_DataClass = 2
	...
	SET @Step = 'Get WriteAccess on Assignment'
		...
		FROM
			...
			INNER JOIN [pcINTEGRATOR_Data].[dbo].[Workflow] WF ON WF.WorkflowID = A.WorkflowID AND WF.SelectYN <> 0 AND WF.DeletedID IS NULL
			INNER JOIN [pcINTEGRATOR_Data].[dbo].[Scenario] S ON S.ScenarioID = WF.ScenarioID AND S.InputAllowedYN <> 0 AND (S.ClosedMonth < WF.TimeTo OR S.ClosedMonth IS NULL) AND S.SelectYN <> 0 AND S.DeletedID IS NULL
			INNER JOIN [pcINTEGRATOR_Data].[dbo].[OrganizationPosition] OP ON OP.OrganizationPositionID = A.OrganizationPositionID AND OP.DeletedID IS NULL
		WHERE
			A.InstanceID = @InstanceID AND
		...
		IF @DebugBM & 2 > 0 SELECT [@InputAllowedYN] = @InputAllowedYN, [@WorkflowID] = @WorkflowID, [@OrganizationHierarchyID] = @OrganizationHierarchyID
		...
		IF @InputAllowedYN <> 0
			...
			FROM
				...
				) OL ON OL.OrganizationLevelNo = WFAR.OrganizationLevelNo AND OL.OrganizationHierarchyID = WFAR.OrganizationHierarchyID
