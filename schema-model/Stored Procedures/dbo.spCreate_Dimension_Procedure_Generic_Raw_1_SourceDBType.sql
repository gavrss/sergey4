...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spCreate_Dimension_Procedure_Generic_Raw_1_SourceDBType]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
EXEC spRun_Procedure_KeyValuePair
	@ProcedureName = 'spCreate_Dimension_Procedure_Generic_Raw_1_SourceDBType',
	@JSON = '
		[
...
EXEC [spCreate_Dimension_Procedure_Generic_Raw_1_SourceDBType] @UserID=-10, @InstanceID=390, @VersionID=1011, @Debug=1
EXEC spCreate_Dimension_View_Generic @SourceID = 983,  @DimensionID = -1,  @Debug = true  --Account	E9
EXEC spCreate_Dimension_View_Generic @SourceID = 983,  @DimensionID = -2,  @Debug = true  --BusinessProcess	E9
EXEC spCreate_Dimension_View_Generic @SourceID = 983,  @DimensionID = -5,  @Debug = true  --Rate	E9
EXEC spCreate_Dimension_View_Generic @SourceID = 983,  @DimensionID = -9,  @Debug = true  --Customer	E9

EXEC [spCreate_Dimension_Procedure_Generic_Raw_1_SourceDBType] @GetVersion = 1
*/
...
DECLARE
	...
	@SQLStatement_EntityPriority nvarchar(max),
	@SQLStatement_GroupBy nvarchar(max),
	@SQLStatement_StepTable_Create nvarchar(max) = '',
	@SQLStatement_StepTable_Insert nvarchar(max) = '',
	@SQLStatement_InsertInto nvarchar(max),
	@Action nvarchar(10),
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Create raw procedures picking dimension data from source database',
		...
		IF @Version = '2.0.1.2143' SET @Description = 'Based on [spCreate_Dimension_View_Generic], changed to create SP instead of VIEW.'
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		SELECT @JobID = CASE WHEN @JobID = 0 THEN @ProcedureID ELSE @JobID END
	...
	SET @Step = 'Create temp table #MappingType'
...
--			@SequenceBMLeafLevel = MAX(SequenceBM)
	...
	SET @Step = 'Set dimension properties in temp table (#DimensionProperty)'
		...
		SET @SQLStatement = '
			...
			SELECT DISTINCT
				...
				P.PropertyName,
				P.DataTypeID,
				Size = CASE WHEN P.DataTypeID = 3 THEN 255 ELSE P.Size END,
				P.DependentDimensionID,
				DependentDimensionName = DD.DimensionName,
				DefaultPropertyYN = [dbo].[GetDefaultPropertyYN](P.DimensionID , P.PropertyID, ' + CONVERT(nvarchar(10), @SourceTypeBM) + ', ' + CONVERT(nvarchar(10), @ModelBM) + '),
				P.DynamicYN,
    ...
    SET @Step = 'Determine ObjectName'
		...
		IF @Debug <> 0 SELECT [ObjectName] = @ObjectName, [Version] = @Version
    ...
    SET @Step = 'Determine CREATE or ALTER'
		...
		SET @ActionStatement = 'EXEC ' + @ETLDatabase + '.dbo.sp_CheckObject ''' + @ObjectName + '''' + ', ' + '''P''' 
	...
	SET @Step = 'Start set @SQLStatement'
		SET @SQLStatement = '
--EXEC ' + @ObjectName + ' @SequenceBMStep = 3, @StaticMemberYN = 0, @Debug = 1
...
'
		...
		SELECT
			@SQLStatement_StepTable_Create = CHAR(9) + CHAR(9) + 'CREATE TABLE [#' + @DimensionName + '_Members_Raw]' + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + '(',
			@SQLStatement_StepTable_Insert = 'INSERT INTO [#' + @DimensionName + '_Members_Raw]' + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '('

		SELECT
			@SQLStatement_StepTable_Create = @SQLStatement_StepTable_Create + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + '[' + [PropertyName] + '] ' + REPLACE([DataTypeCode], 'bit', 'int') + CASE WHEN SizeYN = 0 THEN '' ELSE ' (' + CONVERT(nvarchar(10), Size) + ') COLLATE DATABASE_DEFAULT' END + ',',
			@SQLStatement_StepTable_Insert = @SQLStatement_StepTable_Insert + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '[' + [PropertyName] + '],'
		FROM
			(
			SELECT DISTINCT
				DP.[PropertyName], DT.[DataTypeCode], DP.Size, DT.SizeYN, DP.SortOrder
			FROM
				#DimensionProperty DP
				INNER JOIN DataType DT ON DT.DataTypeID = DP.DataTypeID
			WHERE
				DP.DefaultPropertyYN = 0
--				DP.DynamicYN <> 0
--				DP.SequenceBM & @SequenceBMSum > 0
			) sub
		ORDER BY
			sub.[SortOrder], sub.[PropertyName]

		SELECT
			@SQLStatement_StepTable_Create = LEFT(@SQLStatement_StepTable_Create, LEN(@SQLStatement_StepTable_Create) -1),
			@SQLStatement_StepTable_Insert = LEFT(@SQLStatement_StepTable_Insert, LEN(@SQLStatement_StepTable_Insert) -1)

		SELECT
			@SQLStatement_StepTable_Create = @SQLStatement_StepTable_Create + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + ')',
			@SQLStatement_StepTable_Insert = @SQLStatement_StepTable_Insert + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + ')'

		IF @Debug <> 0 PRINT @SQLStatement_StepTable_Create
		IF @Debug <> 0 PRINT @SQLStatement_StepTable_Insert

		SET @SQLStatement = @SQLStatement + @SQLStatement_StepTable_Create

		IF @Debug <> 0 SELECT SQLStatement = @SQLStatement, SQLStatement_StepTable_Create = @SQLStatement_StepTable_Create
...
*/
						...
						SET @SQLStatement_EntityPriority = '
				...
				SELECT
					...
					[SQLStatement] = ''' + @SQLStatement_Prio + ''''
						...
						IF @Debug <> 0 PRINT @SQLStatement_EntityPriority

						EXEC (@SQLStatement_EntityPriority)
	...
	SET @Step = 'Set @SQLStatement_Sub'
			IF @Debug <> 0
				SELECT
					...
					SQLStatement_Prio = @SQLStatement_Prio,
					SequenceCounter = @SequenceCounter,
					SequenceBMStep = @SequenceBMStep,
					SequenceDescription = @SequenceDescription,
					SQLStatement_StepTable_Insert = @SQLStatement_StepTable_Insert,
					SQLStatement_RawSelect = @SQLStatement_RawSelect,
					SQLStatement_RawFrom = @SQLStatement_RawFrom,
					SQLStatement_MapJoin = @SQLStatement_MapJoin,
					SQLStatement_Prio = @SQLStatement_Prio,
					SQLStatement_RawWhere = @SQLStatement_RawWhere,
					SQLStatement_GroupBy = @SQLStatement_GroupBy

				SET @SQLStatement = @SQLStatement + CHAR(13) + CHAR(10) + '
	SET @Step = ''@SequenceCounter = ' + CONVERT(nvarchar(10), @SequenceCounter) + ', @SequenceBMStep = ' + CONVERT(nvarchar(10), @SequenceBMStep) + ', ' + @SequenceDescription + '''
		IF @SequenceBMStep & ' + CONVERT(nvarchar(10), @SequenceBMStep) + ' > 0
			BEGIN
			' + @SQLStatement_StepTable_Insert + '
			SELECT DISTINCT'
...
'				+ @SQLStatement_RawWhere + CASE WHEN LEN(@SQLStatement_GroupBy) > 0 THEN CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + 'GROUP BY' + @SQLStatement_GroupBy ELSE '' END + '
	...
	SET @Step = 'Get default members'
		...
		SET @SQLStatement = @SQLStatement + CHAR(13) + CHAR(10) + CHAR(13) + CHAR(10) + '
	SET @Step = ''Static Rows''
		IF @StaticMemberYN <> 0
			BEGIN
			' + @SQLStatement_StepTable_Insert + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + @SQLStatement_Member + '

			SET @Selected = @@ROWCOUNT
			IF @Debug <> 0 SELECT Step = @Step, Selected = @Selected
			END'

	SET @Step = 'Create the Return rows part'
	...
	SET @Step = ''Return rows''
		IF OBJECT_ID (N''tempdb..#' + @DimensionName + '_Members'', N''U'') IS NULL
			BEGIN
				SELECT TempTable = ''#' + @DimensionName + '_Members_Raw'', * FROM [#' + @DimensionName + '_Members_Raw] ORDER BY [Label]
				SET @Selected = @@ROWCOUNT
			END
		ELSE
			BEGIN
				INSERT INTO [#' + @DimensionName + '_Members]
					(' + @SQLStatement_InsertInto + '
					)
	SELECT TOP 1000000' + @SQLStatement_Select + '
	FROM
		(
		SELECT' + @SQLStatement_MaxRawSelect + '
		FROM
			[#' + @DimensionName + '_Members_Raw] [Raw]
		GROUP BY
			[Raw].[Label]
		) [MaxRaw]
	...
	ORDER BY
		CASE WHEN [MaxRaw].[Label] IN (''All_'', ''NONE'') OR [MaxRaw].[Label] LIKE ''%NONE%'' THEN ''  '' + [MaxRaw].[Label] ELSE [MaxRaw].[Label] END
		
				SET @Selected = @@ROWCOUNT
			END'

                IF @Debug <> 0 INSERT INTO wrk_Debug ([ProcedureName], [Comment], [SQLStatement]) SELECT [ProcedureName] = OBJECT_NAME(@@PROCID), [Comment] = 'Complete SQL View', [SQLStatement] = @SQLStatement
				IF @Debug <> 0 SELECT ETLDatabase = @ETLDatabase, DimensionName = @DimensionName
                ...
                SET @Step = 'Replace variables'
                    EXEC pcINTEGRATOR..spReplace_CodeWord @ApplicationID = @ApplicationID, @ObjectTypeBM = 7, @TranslatedLanguageID = @LanguageID, @StringIn = @SQLStatement, @StringOut = @SQLStatement OUTPUT
						IF @Debug <> 0 SELECT PlaceHolder = 1, SQLStatement = @SQLStatement
                    SET @SQLStatement = REPLACE (@SQLStatement, '@SourceDatabase', ISNULL(@SourceDatabase, ''))
						IF @Debug <> 0 SELECT PlaceHolder = 2, SQLStatement = @SQLStatement, SourceDatabase = @SourceDatabase
                    SET @SQLStatement = REPLACE (@SQLStatement, '@Owner', ISNULL(@Owner, ''))
						IF @Debug <> 0 SELECT PlaceHolder = 3, SQLStatement = @SQLStatement, [Owner] = @Owner
                    SET @SQLStatement = REPLACE (@SQLStatement, '@DestinationDatabase', @DestinationDatabase)
						IF @Debug <> 0 SELECT PlaceHolder = 4, SQLStatement = @SQLStatement, DestinationDatabase = @DestinationDatabase
                    SET @SQLStatement = REPLACE (@SQLStatement, '@ETLDatabase_Linked', ISNULL(@ETLDatabase_Linked, @ETLDatabase))
						IF @Debug <> 0 SELECT PlaceHolder = 5, SQLStatement = @SQLStatement, ETLDatabase_Linked = ISNULL(@ETLDatabase_Linked, @ETLDatabase)
                    SET @SQLStatement = REPLACE (@SQLStatement, '@SourceType', ISNULL(@SourceType, ''))
						IF @Debug <> 0 SELECT PlaceHolder = 6, SQLStatement = @SQLStatement, SourceType = @SourceType
                    SET @SQLStatement = REPLACE (@SQLStatement, '@SourceID_varchar', @SourceID_varchar)
						IF @Debug <> 0 SELECT PlaceHolder = 7, SQLStatement = @SQLStatement, SourceID_varchar = @SourceID_varchar
                    SET @SQLStatement = REPLACE (@SQLStatement, '@SourceID', @SourceID)
						IF @Debug <> 0 SELECT PlaceHolder = 8, SQLStatement = @SQLStatement, SourceID = @SourceID
					SET @SQLStatement = REPLACE (@SQLStatement, '@DimensionID', @DimensionID)
						IF @Debug <> 0 SELECT PlaceHolder = 9, SQLStatement = @SQLStatement, DimensionID = @DimensionID

					SET @SQLStatement = REPLACE(@SQLStatement, '@InstanceID', @InstanceID)
					SET @SQLStatement = REPLACE(@SQLStatement, '@VersionID', @VersionID)

					SET @SQLStatement = REPLACE(@SQLStatement, '@ApplicationID', @ApplicationID)
						IF @Debug <> 0 SELECT PlaceHolder = 10, SQLStatement = @SQLStatement, ApplicationID = @ApplicationID
					SET @SQLStatement = REPLACE(@SQLStatement, '@DefaultCurrency', ISNULL(@DefaultCurrency, ''))
						IF @Debug <> 0 SELECT PlaceHolder = 11, SQLStatement = @SQLStatement, DefaultCurrency = @DefaultCurrency
                    SET @SQLStatement = REPLACE (@SQLStatement, '@StartYear', ISNULL(CONVERT(nvarchar(10), @StartYear), ''))
						IF @Debug <> 0 SELECT PlaceHolder = 12, SQLStatement = @SQLStatement, StartYear = @StartYear
                    SET @SQLStatement = REPLACE(@SQLStatement, '''', '''''')
						IF @Debug <> 0 SELECT PlaceHolder = 13, SQLStatement = @SQLStatement, SingleQuotes = ''''''
	...
	SET @Step = ''''Drop temp tables''''
		DROP TABLE [#' + @DimensionName + '_Members_Raw]
...
SET @Step = ''''Define exit point''''
	...
	RAISERROR (@Message, @Severity, 100)'
					...
					IF @Debug <> 0 INSERT INTO wrk_Debug ([ProcedureName], [Comment], [SQLStatement]) SELECT [ProcedureName] = OBJECT_NAME(@@PROCID), [Comment] = 'Creation of SP', [SQLStatement] = @SQLStatement
