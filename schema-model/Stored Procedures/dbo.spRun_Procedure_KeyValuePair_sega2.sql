...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spRun_Procedure_KeyValuePair_sega2]
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'For @SQLStatement assembling: added "N" symbol before each nvarchar value because we lost unicode chars'
...
BEGIN TRY
	...
	SET @Step = 'Fill temp table #KeyValuePair'
		...
		ELSE
			BEGIN
    			SET @Message = '1Not valid combination of parameters. Either @JSON or @XML must be set.'
				SET @Severity = 16
				GOTO EXITPOINT
			END
	...
	SET @Step = 'Exec @SQLStatement'
		...
		IF @JSON_table IS NOT NULL
			BEGIN
				...
				select 'sega15', @SQLStatement, @JSON_table_Value
