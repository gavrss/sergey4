...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_Entity_Callisto]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_Dim_Entity_Callisto] @UserID = -10, @InstanceID = -1318, @VersionID = -1256, @DebugBM = 3
EXEC [spIU_Dim_Entity_Callisto] @UserID = -10, @InstanceID = -1335, @VersionID = -1273, @DebugBM = 3
EXEC [spIU_Dim_Entity_Callisto] @UserID = -10, @InstanceID = 533, @VersionID = 1058, @DebugBM = 7
EXEC [spIU_Dim_Entity_Callisto] @UserID = -10, @InstanceID = 574, @VersionID = 1045, @DebugBM=7 --GMN
EXEC [spIU_Dim_Entity_Callisto] @UserID = -10, @InstanceID = 531, @VersionID = 1041, @DebugBM=7 --PCX

EXEC [spIU_Dim_Entity_Callisto] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Insert rows into Callisto Entity dimension table.',
...
BEGIN TRY
	...
	SET @Step = 'Fetch members'
		EXEC [spIU_Dim_Entity_Raw] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @JobID=@JobID, @Debug = @DebugSub

		UPDATE #Entity_Members SET [Label] = [MemberKey], [RNodeType] = CASE WHEN NodeTypeBM & 1 > 0 THEN 'L' ELSE '' END + CASE WHEN NodeTypeBM & 2 > 0 THEN 'P' ELSE '' END  + CASE WHEN NodeTypeBM & 8 > 0 THEN 'C' ELSE '' END
	...
	SET @Step = 'Update Description and dimension specific Properties from source system where Synchronized is set to true.'
		SET @SQLStatement = '
			...
			FROM
				[' + @CallistoDatabase + '].[dbo].[S_DS_Entity] [Entity] 
				INNER JOIN [#Entity_Members] Members ON Members.Label COLLATE DATABASE_DEFAULT = [Entity].LABEL 
			WHERE 
				[Entity].[Synchronized] <> 0'
	...
	SET @Step = 'Insert new members from source system'
		SET @SQLStatement = '
			INSERT INTO [' + @CallistoDatabase + '].[dbo].[S_DS_Entity]
				(
				[MemberId],
				[Label],
				[Description],
			...
			WHERE
				NOT EXISTS (SELECT 1 FROM [' + @CallistoDatabase + '].[dbo].[S_DS_Entity] [Entity] WHERE Members.Label = [Entity].Label)'
	...
	SET @Step = 'Update MemberId'
		EXEC [pcINTEGRATOR].[dbo].[spSet_MemberId] @Database = @CallistoDatabase, @Dimension = N'Entity', @JobID = @JobID, @Debug = @DebugSub
	...
	SET @Step = 'Return rows'
		IF @DebugBM & 1 > 0 EXEC('SELECT [Table] = ''[' + @CallistoDatabase + '].[dbo].[S_DS_Entity]'', * FROM [' + @CallistoDatabase + '].[dbo].[S_DS_Entity]')
