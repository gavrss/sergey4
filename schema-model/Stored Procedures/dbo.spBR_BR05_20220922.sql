...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR05_20220922]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					SET @Step = 'Update HistRate'
						IF @SequenceBM & 2 > 0
							BEGIN
								EXEC [pcINTEGRATOR].[dbo].[spBR_BR05_Fx_HistRate] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @ConsolidationGroup = @ConsolidationGroup, @JobID = @JobID, @Debug = @DebugSub	
							END
									...
									SET @Step = 'Fill FACT_Financials'
										IF @SequenceBM & 384 = 384
											BEGIN
												...
												EXEC [spIU_DC_Financials_Callisto] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @SequenceBM=0, @ConsolidationGroupYN=1, @ConsolidationStep=1, @FieldTypeBM=1, @FiscalYear=@FiscalYear, @ConsolidationGroup=@ConsolidationGroup, @Entity=@Entity_MemberKey, @JobID=@JobID

												EXEC [spIU_DC_Financials_Callisto] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @SequenceBM=0, @ConsolidationGroupYN=1, @ConsolidationStep=2, @FieldTypeBM=4, @FiscalYear=@FiscalYear, @ConsolidationGroup=@ConsolidationGroup, @Entity=@Entity_MemberKey, @JobID=@JobID
