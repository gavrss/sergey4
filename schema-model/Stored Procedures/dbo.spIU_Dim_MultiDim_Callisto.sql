...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_MultiDim_Callisto]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [pcINTEGRATOR].[dbo].[spIU_Dim_MultiDim_Callisto] @Debug=N'0',
@DimensionID=N'7070',@InstanceID=N'767',@UserID=N'31384',@VersionID=N'1190',@DebugBM=63

EXEC [pcINTEGRATOR].[dbo].[spIU_Dim_MultiDim_Callisto] @DebugBM='31',@DimensionID='5828',@InstanceID='621',@UserID='-10',@VersionID='1105'
EXEC pcINTEGRATOR.dbo.[spIU_Dim_MultiDim_Callisto] @UserID = -10, @InstanceID=572, @VersionID=1080, @DimensionID = 5666, @HierarchyNo=0, @DebugBM=3

EXEC pcINTEGRATOR.dbo.[spIU_Dim_MultiDim_Callisto] @UserID = -10, @InstanceID=573, @VersionID=1080, @DimensionID = 5610, @DebugBM=3
EXEC [spIU_Dim_MultiDim_Callisto] @UserID = -10, @InstanceID = 561, @VersionID = 1044, @DimensionID = 9181, @DebugBM=3 --DNKLY
EXEC [spIU_Dim_MultiDim_Callisto] @UserID = -10, @InstanceID = 574, @VersionID = 1045, @DimensionID = 9188, @DebugBM=3 --GMN
EXEC [spIU_Dim_MultiDim_Callisto] @UserID = -10, @InstanceID = 574, @VersionID = 1081, @DimensionID = 5604, @DebugBM=3 --GMN, FullAccount
EXEC [spIU_Dim_MultiDim_Callisto] @UserID = -10, @InstanceID = -1443, @VersionID = -1381, @DimensionID = null, @DimensionName = 'FullAccount', @DebugBM=7 --GMN, FullAccount
EXEC [spIU_Dim_MultiDim_Callisto] @UserID = -10, @InstanceID = 574, @VersionID = 1081, @DimensionID = 5615, @DebugBM=3 --GMN, GL_COA
EXEC [spIU_Dim_MultiDim_Callisto] @UserID = -10, @InstanceID = 573, @VersionID = 1080, @DimensionID = 5603, @DebugBM=3 --ACAME
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2198' SET @Description = 'DB-1437: Added UPDATE query for [Description] and [HelpText] columns.'
		IF @Version = '2.1.2.2199' SET @Description = 'FDB-2977: Increased data size from 255 to 512 for [SqlSelect] column in #ColumnList.'
...
--			SegmentName = PD.DimensionName,
	...
	SET @Step = 'Create and fill temp table #ColumnList'
		CREATE TABLE #ColumnList
			...
			[SqlSelect] nvarchar(512) DEFAULT '' COLLATE DATABASE_DEFAULT,
			[SortOrder] int
			)
	...
	SET @Step = 'Update column [SqlSelect] in temp table #ColumnList for MemberKey and Description.'
		...
		IF @DebugBM & 32 > 0 SELECT [LEN_@MemberKey] = LEN(@MemberKey), [@MemberKey] = @MemberKey,[LEN_@MemberDescription] = LEN(@MemberDescription), [@MemberDescription] = @MemberDescription
		...
		DECLARE DataClass_Cursor CURSOR FOR
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					--Update [Description] and [HelpText] columns (S_DS_XXX)
					SET @SQLStatement = '
						UPDATE DC
						SET
					...
					--Update [Description] and [HelpText] columns (O_DS_XXX)
					SET @SQLStatement = '
						UPDATE DC
						SET
							[Description] = ' + REPLACE(@MemberDescription, '', '''') + ',
							...
							[Updated] = CONVERT(nvarchar(50), FORMAT(GetDate(),''yyyy-MM-dd HH:mm:ss.fff'')) 
						FROM
							[' + @CallistoDatabase + '].[dbo].[O_DS_' + @DimensionName + '] DC ' + @SqlJoin + ' 
						WHERE
							DC.Synchronized <> 0 AND
							DC.RNodeType = ''L'' AND 
							DC.MemberId NOT IN (-1) AND
