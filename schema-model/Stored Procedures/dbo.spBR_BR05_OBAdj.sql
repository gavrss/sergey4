...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR05_OBAdj]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
BEGIN TRY
	...
	SET @Step = 'Create temp table(s)'
		CREATE TABLE #Journal_Cons
			...
			[InterCompanyEntity] nvarchar(50),
	...
	SET @Step = 'Fill temp table #Journal_Cons'
		SET @SQLStatement = '
			...
			SELECT 
				M.[Entity], M.[Book], M.[ConsolidationMethodBM], M.[Account],
				...
				[InterCompanyEntity] = CASE WHEN M.[InterCompanyEntity] = '''' THEN NULL ELSE M.[InterCompanyEntity] END,
		...
		SET @SQLStatement = @SQLStatement + '
				...
				SELECT
					...
					[InterCompanyEntity] = ISNULL(J.[InterCompanyEntity], ''''),
	...
	SET @Step = 'Insert new rows into [Journal]'
		SET @SQLStatement = '
			...
			SELECT
				...
				[InterCompanyEntity] = JC.[InterCompanyEntity],
				...
				[Value_Book] = [Adjustment_Book],
				[Currency_Group] = ''' + @GroupCurrency + ''',
				[Value_Group] = [Adjustment_Group],
				[Currency_Transaction] = '''',
				[Value_Transaction] = 0,
	...
	SET @Step = 'Fill temp table #OBAdj_Rule_Cursor_Table'
		CREATE TABLE #OBAdj_Rule_Cursor_Table
			(
		...
		INSERT INTO #OBAdj_Rule_Cursor_Table
			(
			[RuleType],
			[RuleID],
			[DimensionFilter],
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#OBAdj_Rule_Cursor_Table', * FROM #OBAdj_Rule_Cursor_Table ORDER BY [RuleType], [SortOrder]

	SET @Step = 'Set Rule_ConsolidationID'
		IF CURSOR_STATUS('global','OBAdj_Rule_Cursor') >= -1 DEALLOCATE OBAdj_Rule_Cursor
		DECLARE OBAdj_Rule_Cursor CURSOR FOR
			...
			SELECT
				...
				[DimensionFilter]
			FROM
				#OBAdj_Rule_Cursor_Table
			ORDER BY
				[RuleType],
				[SortOrder] DESC

			OPEN OBAdj_Rule_Cursor
			FETCH NEXT FROM OBAdj_Rule_Cursor INTO @RuleType, @RuleID, @DimensionFilter
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					SELECT
						...
						@StepReference = 'BR05_OA_' + RIGHT(@RuleType, 4) + '_' + CONVERT(nvarchar(15), @RuleID)
					...
					IF @DebugBM & 2 > 0 
						BEGIN
							IF LEN(@SQLStatement) > 4000 
								BEGIN
									PRINT 'Length of @SQLStatement more than 4000, see pcINTEGRATOR_Log..wrk_Debug; OBAdj, Set RuleID.'
									EXEC [dbo].[spSet_wrk_Debug]
										@UserID = @UserID,
										@InstanceID = @InstanceID,
										@VersionID = @VersionID,
		...
		CLOSE OBAdj_Rule_Cursor
		DEALLOCATE OBAdj_Rule_Cursor

SELECT * FROM #JournalBase ORDER BY Account, YearMonth, FiscalPeriod
/*
