...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spSet_Hierarchy_JaWo]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
EXEC [spSet_Hierarchy_JaWo] @UserID = -10, @InstanceID = 615, @VersionID = 1102, @DimensionID = 6813, @SourceTable= '[pcDATA_AIMA].[dbo].[S_DS_ProjectManual]', @DebugBM=3 --AIMA
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'FEA-11149: Modified INSERT query, making JOIN clause dynamic for @HierarchyTypeID = 3. Added HierarchyTypeID 7 and 8 (Like standard, with DimensionFilter)'
...
BEGIN TRY
	...
	SET @Step = 'Create and fill Cursor table, #Hierarchy_Cursor.'
		...
		SELECT DISTINCT
			...
			[DimensionFilter] = DH.[DimensionFilter]
		FROM
			pcINTEGRATOR_Data..Dimension_StorageType DS
			INNER JOIN pcINTEGRATOR..DimensionHierarchy DH ON DH.[InstanceID] IN (0, DS.[InstanceID]) AND DH.[VersionID] IN (0, DS.[VersionID]) AND DH.[DimensionID] = DS.[DimensionID] AND DH.[HierarchyTypeID] IN (1,2,3,4,6,7,8) AND	(DH.[HierarchyNo] = @HierarchyNo OR @HierarchyNo IS NULL)
		WHERE
	...
	SET @Step = 'Update hierarchies for all type of hierarchies.'
		...
		DECLARE Hierarchy_Cursor CURSOR FOR
			...
			FETCH NEXT FROM Hierarchy_Cursor INTO @HierarchyNo, @HierarchyName, @DimensionHierarchy, @HierarchyTypeID, 	@BaseDimension, @BaseHierarchy, @BaseDimensionFilter, @PropertyHierarchy, @BusinessRuleID, @DimensionFilter
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					IF @HierarchyTypeID IN (6, 8) --Automated by ETL
						BEGIN
							SET @SQLStatement = '
...
*/
					...
					IF @HierarchyTypeID IN (7, 8) --Standard with filter / Automated by ETL
						BEGIN
							SET @Step = 'Insert new members into the default hierarchy. Not Synchronized members will not be added.'
								IF @StorageTypeBM & 4 > 0 AND @DimensionTypeID <> 27
									BEGIN
										--SET @DimensionFilter = 'D1.' + REPLACE(@DimensionFilter, '''', '''''') + ' AND'
										SET @DimensionFilter = '(' + @DimensionFilter + ') AND'
