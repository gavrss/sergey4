...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_20220826]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
--	@MasterDataClassID int = NULL, --If set @DataClassID is of type SpreadingKey
	...
	@RowList nvarchar(max) = NULL, --Only valid for @ResultTypeBM=256 and 512
...
EXEC [spPortalGet_DataClass_Data_New] @GetVersion = 1
*/
...
DECLARE
	...
	@TimeFilterTuple nvarchar(1000),
	@TimeFilterTupleString nvarchar(1000),
	@TimeFilterString nvarchar(4000),
...
BEGIN TRY
	...
	SET @Step = 'Create temp table #GroupBy'
		...
		SELECT
			...
			[PropertyName] = 'Label'
		WHERE
			NOT EXISTS (SELECT 1 FROM #GroupBy GB WHERE GB.[DimensionName] = 'Time')
	...
	SET @Step = 'Get temp table #MultiDim when needed'
		...
		IF @ResultTypeBM & 804 > 0 AND (SELECT COUNT(1) FROM #FilterTable WHERE [MultiDimIncludedYN] <> 0) > 0
			BEGIN
				SET @MultiDimYN = 1
	...
	SET @Step = 'Get parts for SQL-statement'
		IF @ResultTypeBM & 804 > 0 --4, 32, 256, 512
			BEGIN
				SELECT
				...
				FROM
					...
					INNER JOIN #FilterTable FT ON FT.[StepReference] = @StepReference AND FT.[TupleNo] = GB.[TupleNo] AND FT.[DimensionName] = GB.[DimensionName]
				ORDER BY
					GB.[SortOrder]
	...
	SET @Step = '@ResultTypeBM & 12'
		IF @ResultTypeBM & 12 > 0
			BEGIN
				...
				IF @DebugBM & 2 > 0  SELECT [@WorkflowStateYN] = @WorkflowStateYN
				...
				--		INNER JOIN #Time T ON T.[MemberId] = ' + CASE WHEN @TimeDimensionTypeID = -49 THEN 'DC.[TimeDay_MemberId] / 100' ELSE 'DC.[Time_MemberId]' END + CASE WHEN @MultiDimYN <> 0 THEN @SQL_MultiDimJoin ELSE '' END + '
				--	WHERE
				--		' + @SQL_Where_Total + '
				--		1 = 1
				...
				SET @SQLStatement = '
					...
					FROM
						[' + @CallistoDatabase + '].[dbo].[FACT_' + @DataClassName + '_default_partition] DC WITH (INDEX([NCCSI_' + @DataClassName + ']))
						' + CASE WHEN LEN(@TimeFilterString) > 0 THEN 'INNER JOIN #Time T ON T.[MemberId] = ' + CASE WHEN @TimeDimensionTypeID = -49 THEN 'DC.[TimeDay_MemberId] / 100' ELSE 'DC.[Time_MemberId]' END ELSE '' END + CASE WHEN @MultiDimYN <> 0 THEN @SQL_MultiDimJoin ELSE '' END + '
					WHERE
						' + @SQL_Where_Total + '
						1 = 1
					' + CASE WHEN LEN(@SQL_GroupBy1) > 0 THEN 'GROUP BY' + @SQL_GroupBy1 ELSE '' END
				...
				--SET @SQLStatement = '			
				--	SELECT' + @SQL_Select2 + ' 
				--		[' + @Measure + '_Value] = ROUND(SUM(DC.[' + @Measure + '_Value]), 4),' + @SQL_Tuple + '
...
/*
				SET @SQLStatement = '			
					...
					FROM (
					SELECT' + @SQL_Select2 + '
						[' + @Measure + '_Value] = ROUND(SUM(CASE WHEN [TimeView].[MemberId] = [Time].[MemberId] THEN DC.[' + @Measure + '_Value] ELSE 0 END), 4),' + @SQL_Tuple + '
						[WorkflowStateID] = MAX(DC.[WorkflowStateID]),
						[Workflow_UpdatedBy_UserID] = MAX(DC.[Workflow_UpdatedBy_UserID])
					FROM
						' + @TmpGlobalTable + ' DC
					...
					' + CASE WHEN LEN(@SQL_GroupBy2) > 0 THEN 'GROUP BY' + @SQL_GroupBy2 ELSE '' END + ') sub
						'
*/
/*
				SET @SQLStatement = '			
					...
					INTO ' + @TmpGlobalTable_2 + '
					FROM (
					SELECT' + @SQL_Select2 + CASE WHEN @WorkflowStateYN <> 0 THEN '
						[WorkflowStateID] = MAX(CASE WHEN [TimeView].[MemberId] = [Time].[MemberId] THEN DC.WorkflowStateID ELSE -1 END),
					...
					FROM
						' + @TmpGlobalTable + ' DC' + CASE WHEN LEN(@TimeFilterString) > 0 THEN '
						INNER JOIN #Time [TimeView] ON [TimeView].[MemberID] = DC.[Time_MemberId]
		...
		IF @ResultTypeBM & 768 > 0
			BEGIN
				SET @SQLStatement = '
					...
					FROM
						[' + @CallistoDatabase + '].[dbo].[FACT_' + @DataClassName + '_default_partition] DC WITH (INDEX([NCCSI_' + @DataClassName + ']))
						INNER JOIN #Time T ON T.[MemberId] = DC.[Time_MemberId]' + CASE WHEN @MultiDimYN <> 0 THEN @SQL_MultiDimJoin ELSE '' END + '
	...
	SET @Step = '@ResultTypeBM & 512'
		IF @ResultTypeBM & 512 > 0
			BEGIN
				...
				IF @DebugBM & 2 > 0 SELECT * FROM #PipeStringSplit
...
--' + @SQL_Join2 + '
				...
				SELECT
					@AggregationSet = LEFT(@AggregationSet, LEN(@AggregationSet) - 1),
					@AggregationSum = LEFT(@AggregationSum, LEN(@AggregationSum) - 1)
