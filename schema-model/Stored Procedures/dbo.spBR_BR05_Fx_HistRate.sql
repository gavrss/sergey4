...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR05_Fx_HistRate]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
EXEC [spBR_BR05_Fx_HistRate] @UserID=-10, @InstanceID=527, @VersionID=1055, @SequenceBM = 2, @ConsolidationGroup = 'Group', @Entity = 'GGI03', @Account = '1699', @FiscalYear=2022, @DebugBM=3
...
EXEC [spBR_BR05_Fx_HistRate] @UserID=-10, @InstanceID=572, @VersionID=1080, @SequenceBM = 2, @ConsolidationGroup = 'G_BRADKEN', @Entity = 'BR01', @Account = '6991', @DebugBM=3
EXEC [spBR_BR05_Fx_HistRate] @UserID=-10, @InstanceID=572, @VersionID=1080, @SequenceBM = 1, @ConsolidationGroup = 'G_BRADKEN', @DebugBM=3
EXEC [spBR_BR05_Fx_HistRate] @UserID=-10, @InstanceID=572, @VersionID=1080, @SequenceBM = 3, @ConsolidationGroup = 'G_BRADKEN', @FiscalYear=2021, @DebugBM=3
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2187' SET @Description = 'Dynamic pickup of Account_RE. Added parameter @FiscalYear.'
	...
	SET @Step = 'CREATE TABLE #EntityBook'
		IF OBJECT_ID(N'TempDB.dbo.#EntityBook', N'U') IS NULL
			BEGIN
				...
				FROM 
					Entity E
					INNER JOIN Entity_Book EB ON EB.[InstanceID] = E.[InstanceID] AND EB.[VersionID] = E.[VersionID] AND EB.[EntityID] = E.[EntityID] AND EB.[BookTypeBM] & 18 > 0 AND EB.[SelectYN] <> 0 AND (EB.[Book] = @Book OR @Book IS NULL)
				...
				SET @Step = 'Create and fill table #FiscalYear'
					...
					SET @SQLStatement = '
						...
						WHERE
							...
							J.[BalanceYN] <> 0 AND
							J.[Scenario] = ''ACTUAL''' + CASE WHEN @FiscalYear IS NOT NULL THEN ' AND
							J.[FiscalYear] >= ' + CONVERT(nvarchar(15), @FiscalYear) ELSE '' END + '
						GROUP BY
							J.[InstanceID],
							J.[Entity],
							J.[Book],
...
--						[GroupAmount_Flow] = ROUND(CASE WHEN @Operator = '*' THEN HR.FunctionalAmount_Flow * (FxD.[FxRate] / FxS.[FxRate]) ELSE HR.FunctionalAmount_Flow / (FxD.[FxRate] / FxS.[FxRate]) END, 4),
						[GroupAmount_Flow] = ROUND(HR.FunctionalAmount_Flow * ISNULL(HR.[SetHistRate], CASE WHEN @Operator = '*' THEN FxD.[FxRate] / FxS.[FxRate] ELSE 1 / (FxD.[FxRate] / FxS.[FxRate]) END), 4),
			...
			BEGIN
				...
				SET @Step = 'Fill of table #Time.'
					...
					SET @SQLStatement = '
						...
						WHERE
							...
							T.[Level] = ''Month''' + CASE WHEN @FiscalYear IS NOT NULL THEN ' AND
							T.TimeFiscalYear_MemberId >= ' + CONVERT(nvarchar(15), @FiscalYear - 1) ELSE '' END + '
						ORDER BY
							T.MemberId'
				...
				SET @Step = 'Fill of table #HistRate_CursorTable.'
					SET @SQLStatement = '
						...
						FROM
							' + @JournalTable + ' J
						WHERE
							J.[YearMonth] >= ' + CONVERT(nvarchar(15), @MinYearMonth) + CASE WHEN @FiscalYear IS NOT NULL THEN ' AND
							J.[FiscalYear] >= ' + CONVERT(nvarchar(15), @FiscalYear - 1) ELSE '' END + '
						GROUP BY
							[YearMonth]'
					...
					SELECT DISTINCT
						...
						[PrevFiscalYear] = JFYP.[FiscalYear]
					INTO
						#HistRate_CursorTable
					FROM
						#Time T
						INNER JOIN #Jrn_FiscalYear JFY ON JFY.[YearMonth] = T.[YearMonth]
						INNER JOIN #Time T1 ON T1.[RowOrder] = T.[RowOrder] - 1
						INNER JOIN #Jrn_FiscalYear JFYP ON JFYP.[YearMonth] = T1.[YearMonth]
					WHERE
					...
					IF @FiscalYear IS NOT NULL
						DELETE #HistRate_CursorTable WHERE [FiscalYear] < @FiscalYear
				...
				SET @Step = 'HistRate_Cursor'
					...
					DECLARE HistRate_Cursor CURSOR FOR
						...
						FETCH NEXT FROM HistRate_Cursor INTO @YearMonth, @PrevYearMonth, @FiscalYear_Cursor, @PrevFiscalYear

						WHILE @@FETCH_STATUS = 0
							BEGIN
								IF @DebugBM & 2 > 0 SELECT [@YearMonth] = @YearMonth, [@PrevYearMonth] = @PrevYearMonth, [@FiscalYear_Cursor] = @FiscalYear_Cursor, [@PrevFiscalYear] = @PrevFiscalYear
								...
								SET @SQLStatement = '
									...
									WHERE
										J.[InstanceID] = ' + CONVERT(nvarchar(15), @InstanceID) + ' AND
										J.[FiscalYear] = ' + CONVERT(nvarchar(15), @FiscalYear_Cursor) + ' AND
...
--									[HistRate] = ROUND(CASE WHEN HR1.[FunctionalAmount_Open] = 0 THEN ISNULL(HR1.[SetHistRate], 0) ELSE HR1.[GroupAmount_Open] / HR1.[FunctionalAmount_Open] END, 11),
									[HistRate] = ROUND(CASE WHEN ROUND(HR1.[FunctionalAmount_Open], 0) = 0 THEN ISNULL(HR1.[SetHistRate], 0) ELSE HR1.[GroupAmount_Open] / HR1.[FunctionalAmount_Open] END, 11),
									[FunctionalAmount_Open] = HR1.[FunctionalAmount_Open],
									[GroupAmount_Open] = HR1.[GroupAmount_Open],
--									[GroupAmount_Open] = CASE WHEN ROUND(HR1.[FunctionalAmount_Open], 0) = 0 THEN 0 ELSE HR1.[GroupAmount_Open] END,
									[FunctionalAmount_Open_Adj] = HR1.[FunctionalAmount_Open_Adj],
									[GroupAmount_Open_Adj] = CASE WHEN @PrevFiscalYear <> @FiscalYear_Cursor AND HR.Account = EB.[Account_RE] THEN ISNULL(J.[Value_Group_YearOpen], 0) - HR1.[GroupAmount_Open] ELSE 0 END
								FROM
									#HistRate HR
									...
									INNER JOIN #EntityBook EB ON EB.[MemberKey] = HR.[Entity] AND EB.[Book] = HR.[Book] AND EB.[SelectYN] <> 0
									LEFT JOIN #Journal J ON
										J.[Entity] = HR.[Entity] AND
										J.[Book] = HR.[Book] AND
										J.[Account] = HR.[Account] AND
