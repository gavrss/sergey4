...
CREATE PROCEDURE [dbo].[spIU_DC_Journal]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
-- if the source databases can be cached...
if exists(SELECT 1
         FROM [pcINTEGRATOR_Data]..[Application]
         WHERE
            [InstanceID] = @InstanceID AND
            [VersionID] = @VersionID AND
            [SelectYN] <> 0 AND
            [UseCachedSourceDatabaseYN] <> 0)
    begin
        EXEC [sp_Tool_Copy_SourceDatabase_ToProdServer] @UserID=@UserID, @InstanceID = @InstanceID, @VersionID = @VersionID
    end

/*
-- TPH only instance:
if @InstanceID = 460 and  @VersionID = 1052
    begin
    SET @Step = 'Copy the database from the source to local SQLServer'
            update pcINTEGRATOR_data..Source
             set SourceDatabase = 'pcSource_TPH'
                     WHERE InstanceID = 460 and  @VersionID = 1052
             and SourceDatabase like '%pcSource_TPH%'

            DECLARE  @DefaultDataPath NVARCHAR(255)
                    ,@DefaultLogPath NVARCHAR(255)
                    ,@LinkedServer nvarchar(255) = 'dspSource01'
                    ,@SourceDatabaseShort nvarchar(255) = 'pcSource_TPH'
                    --,@SQLStatement nvarchar(1000)
                    ,@DynamicSQL nvarchar(1000);

            set @SQLStatement = 'EXEC ' + @LinkedServer + '.' + @SourceDatabaseShort + '.sys.sp_executesql N''USE [pcSource_TPH]; DBCC SHRINKFILE (N''''pcSource_TPH_log'''', 1);'''
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
							...
							SET @JSON = '
								...
								+ CASE WHEN @SourceTypeID IN (1, 11) AND @BalanceType_B_D_SumYN IS NOT NULL THEN ',' + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '{"TKey" : "BalanceType_B_D_SumYN",  "TValue": "' + CONVERT(nvarchar(10), @BalanceType_B_D_SumYN) + '"}' ELSE '' END +
								+ CASE WHEN @MaxSourceCounter IS NULL OR @SourceTypeID NOT IN (1, 3, 11) THEN '' ELSE ',' + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '{"TKey" : "MaxSourceCounter",  "TValue": "' + CONVERT(nvarchar(20), @MaxSourceCounter) + '"}' END +
								']'
...
*/
	...
	SET @Step = 'Delete already existing rows from [Journal]'
		SET @SQLStatement = '
			...
			WHERE 
				J.[TransactionTypeBM] NOT IN (8,16,32) AND 
...
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_01', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
	...
	SET @Step = 'Fill Journal table'
		SET @SQLStatement = '
		INSERT INTO ' + @JournalTable + ' with(tablock)
			(
			[JobID],
			[InstanceID],
			[Entity],
...
if exists(SELECT 1
         ...
         WHERE
            ...
            [UseCachedSourceDatabaseYN] <> 0)
    begin
        EXEC [sp_Tool_DropCachedDatabase_OnProdServer] @UserID=@UserID, @InstanceID = @InstanceID, @VersionID = @VersionID
    end
    /*
-- TPH only instance:
if @InstanceID = 460 and  @VersionID = 1052
    begin
            update pcINTEGRATOR_data..Source
            set SourceDatabase = 'dspSource01.pcSource_TPH'
                     WHERE InstanceID = 460 and  @VersionID = 1052
            and SourceDatabase like '%pcSource_TPH%'
