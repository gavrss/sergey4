...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR05_Fx_Opening_Old]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
BEGIN TRY
	...
	SET @Step = 'Insert new rows into #JournalBase, Historic'
		IF @HistoricYN <> 0
			BEGIN	
--Arne
--DROP TABLE pcINTEGRATOR_Log..tmp_JournalBase_Bradken
--DROP TABLE pcINTEGRATOR_Log..tmp_CountedFiscalPeriods_Bradken
--DROP TABLE pcINTEGRATOR_Log..tmp_EntityBook_Bradken

--SELECT * INTO pcINTEGRATOR_Log..tmp_JournalBase_Bradken FROM #JournalBase
--SELECT * INTO pcINTEGRATOR_Log..tmp_CountedFiscalPeriods_Bradken FROM #CountedFiscalPeriods
--SELECT * INTO pcINTEGRATOR_Log..tmp_EntityBook_Bradken FROM #EntityBook
...
SELECT 
	...
	[Value_Book] = J.[ValueDebit_Book] - J.[ValueCredit_Book],
	[Value_Group] = (J.[ValueDebit_Book] - J.[ValueCredit_Book]) * (FxD.[FxRate] / FxS.[FxRate])
FROM
	' + @JournalTable + ' J
	...
	LEFT JOIN #FxRate FxD ON 
		FxD.[Currency] = ''' + @Currency_Group + ''' AND 
		FxD.[Rate_MemberId] = 101 AND --Average
		FxD.[Scenario_MemberId] = 110 AND --ACTUAL
		FxD.[Time_MemberId] = FP1.[YearMonth]
	LEFT JOIN #FxRate FxS ON
		FxS.[Currency] = J.[Currency_Book] AND
...
WHERE
	...
	J.[BalanceYN] <> 0 AND
	J.[JournalSequence] IN (''OB_ADJ'') AND
	' + @JournalFilter
...
EXEC (@SQLStatement)
				...
				SET @SQLStatement = @SQLStatement + '
					SELECT
						...
						[JournalSequence] = CASE WHEN sub.[JournalSequence] IN (''G_ELIMADJ'') THEN sub.[JournalSequence] ELSE ''JRNL'' END,
				...
				SET @SQLStatement = @SQLStatement + '
						...
						WHERE
							...
							J.Scenario = ''' + @Scenario + ''' AND
							J.[BalanceYN] <> 0 AND
							J.[JournalSequence] NOT IN (''ELIM'') AND
							' + @JournalFilter
/*
				SET @SQLStatement = @SQLStatement + '
					--Opening adjustment
...
*/									
				SET @SQLStatement = @SQLStatement + '
						...
						WHERE
							...
							J.Scenario = ''' + @Scenario + ''' AND
							J.[BalanceYN] <> 0 AND
							ISNULL(J.[Flow], ''NONE'') NOT IN (''OP_Opening'') AND
							' + @JournalFilter + '
						) sub
					...
					GROUP BY
						...
						sub.[Book],
						sub.[Account],
						CASE WHEN sub.[JournalSequence] IN (''G_ELIMADJ'') THEN sub.[JournalSequence] ELSE ''JRNL'' END,
						sub.[Segment01],
						sub.[Segment02],
						sub.[Segment03],
						sub.[Segment04],
...
SET @SQLStatement = '
	...
	WHERE
		...
		J.Scenario = ''' + @Scenario + ''' AND
		J.[BalanceYN] <> 0 AND
		ISNULL(J.[Flow], ''NONE'') NOT IN (''OP_Opening'')'
