...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_New2]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
--		SELECT @TimeType = CASE WHEN (SELECT COUNT(1) FROM #FilterTable WHERE DimensionID IN (-49) AND LEN(LeafLevelFilter) > 0) > 0 THEN 'TimeDay' ELSE 'Time' END
	...
	SET @Step = 'Get temp table #MultiDim when needed'
		...
		IF @ResultTypeBM & 812 > 0 AND (SELECT COUNT(1) FROM #FilterTable WHERE [MultiDimIncludedYN] <> 0) > 0
			BEGIN
				SET @MultiDimYN = 1
IF @DebugBM & 2 > 0 SELECT 'Arne1 MultiDim'
...
IF @DebugBM & 2 > 0 SELECT 'Arne4 MultiDim', [@DebugSub] = @DebugSub
...
IF @DebugBM & 2 > 0 SELECT 'Arne5 MultiDim'
...
IF @DebugBM & 2 > 0 SELECT 'Arne6 MultiDim'

			IF @DebugBM & 2 > 0 SELECT TempTable = '#MultiDim', * FROM #MultiDim

IF @DebugBM & 2 > 0 SELECT 'Arne7 MultiDim'
		END
