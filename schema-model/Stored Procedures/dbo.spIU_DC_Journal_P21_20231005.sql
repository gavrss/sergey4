...
CREATE PROCEDURE [dbo].[spIU_DC_Journal_P21_20231005]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_DC_Journal_P21] @UserID = -10, @InstanceID = 515, @VersionID = 1064, @SequenceBM = 2, @Entity_MemberKey = 'REM', @Book = GL, @FiscalYear = 2023, @FullReloadYN = 1, @DebugBM = 31
...
BEGIN TRY
	...
	SET @Step = 'Create and fill temp table #FiscalPeriod'
		CREATE TABLE #FiscalPeriod
			...
			FiscalPeriod int,
			YearMonth int,
			ClosingFiscalYear int,
			ClosingFiscalPeriod int
			)
	...
	SET @Step = 'Insert opening balances into temp table #Journal'
		IF @SequenceBM & 2 > 0 AND (SELECT COUNT(1) FROM #FiscalPeriod WHERE FiscalPeriod = 0) > 0 
			...
			BEGIN
				UPDATE FP
				SET
					ClosingFiscalYear = FiscalYear - 1,
					ClosingFiscalPeriod = 12
				FROM
					#FiscalPeriod FP
				WHERE
					FP.FiscalPeriod = 0

				IF @DebugBM & 2 > 0 SELECT TempTable = '#FiscalPeriod', * FROM #FiscalPeriod
