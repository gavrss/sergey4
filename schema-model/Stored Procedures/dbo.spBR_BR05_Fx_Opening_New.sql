...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR05_Fx_Opening_New]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'Added correction of Opening Retained earnings to match Opening from ERP.'
...
--						  J.[FiscalPeriod],
				...
				SELECT    [ReferenceNo] = 13000000 + ROW_NUMBER() OVER(ORDER BY J.[Entity], J.[Book], J.[FiscalYear], J.[Account]),
				...
				from #OBERPADJ J
				INNER JOIN #EntityBook EB ON EB.[MemberKey] = J.[Entity] AND EB.[Book] = J.[Book] AND EB.[SelectYN] <> 0
				INNER JOIN #CountedFiscalPeriods FP2 ON FP2.FiscalYear = J.FiscalYear AND FP2.FiscalPeriod = 1
				GROUP BY
						  J.[Entity],
						  J.[Book],
						  J.[FiscalYear],
