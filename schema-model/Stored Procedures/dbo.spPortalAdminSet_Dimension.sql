...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalAdminSet_Dimension]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'FDB-1848: Set [ETLProcedure] column when inserting into [Dimension_StorageType] table.'
...
BEGIN TRY
	...
	SET @Step = 'Create and fill #DimList'
		CREATE TABLE #DimList
			...
			[ETLProcedure] nvarchar(255) COLLATE DATABASE_DEFAULT,
		...
		SET @Step = 'Check if the Dimension Name already exist'
		IF (SELECT COUNT(1) FROM #DimList where [DimensionID] is NULL )<>0
			BEGIN

				IF (SELECT COUNT(1) FROM [Dimension] D 
					INNER JOIN #DimList DL ON DL.[DimensionName] = D.[DimensionName] AND DL.[DimensionID] IS NULL WHERE D.SelectYN <> 0 AND D.InstanceID = @InstanceID AND D.DeletedID IS NULL ) <> 0
					BEGIN
						SET @Message = 'Selected Dimension Name already exist for this instance.'
						SET @Severity = 16
						GOTO EXITPOINT
					...
					ELSE
						BEGIN
							...
							SELECT
								...
								[NumberHierarchy] = ISNULL(DL.[NumberHierarchy], [dbo].[f_GetDefaultValue] ('Dimension_StorageType', 'NumberHierarchy')),
								...
								[ETLProcedure] = CASE WHEN @DimensionTypeID = 27 THEN 'spIU_Dim_MultiDim_Callisto' ELSE DL.[ETLProcedure] END
							FROM
								#DimList DL 
							WHERE
								DL.ResultTypeBM = 1 AND
