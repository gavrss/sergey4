...
CREATE PROCEDURE [dbo].[spIU_Dim_InvoiceNo_AP_Callisto]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
AS
/*
EXEC [spIU_Dim_InvoiceNo_AP_Callisto] @UserID = -10, @InstanceID = -1089, @VersionID = -1086, @DebugBM = 1

EXEC [spIU_Dim_InvoiceNo_AP_Callisto] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Insert rows into Callisto InvoiceNo_AP tables from different sources.',
...
BEGIN TRY
	...
	SET @Step = 'Create temp tables'
		CREATE TABLE #InvoiceNo_AP_Members
			(
			[MemberId] BIGINT,
	...
	SET @Step = 'Fetch members'
		EXEC [spIU_Dim_InvoiceNo_AP_Raw] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @SourceTypeID=@SourceTypeID, @SourceDatabase=@SourceDatabase, @SequenceBMStep=@SequenceBMStep, @StaticMemberYN=@StaticMemberYN, @MappingTypeID=@MappingTypeID, @JobID=@JobID, @Debug=@DebugSub

		UPDATE #InvoiceNo_AP_Members 
		SET
			[Label] = [MemberKey], 
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#InvoiceNo_AP_Members', * FROM #InvoiceNo_AP_Members
	...
	SET @Step = 'Update Description and dimension specific Properties from source system where Synchronized is set to true.'
		SET @SQLStatement = '
			...
			FROM
				[' + @CallistoDatabase + '].[dbo].[S_DS_' + @DimensionName + '] [InvoiceNo_AP] 
				INNER JOIN [#InvoiceNo_AP_Members] [Members] ON [Members].Label COLLATE DATABASE_DEFAULT = [InvoiceNo_AP].Label				
				LEFT JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_Supplier] [Supplier] ON [Supplier].Label COLLATE DATABASE_DEFAULT = [Members].[Supplier]
				LEFT JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_TimeDay] [Paid_Time] ON [Paid_Time].Label COLLATE DATABASE_DEFAULT = [Members].[Paid_Time]
			WHERE 
				[InvoiceNo_AP].[Synchronized] <> 0 AND
				(
					[InvoiceNo_AP].[Description] <> Members.[Description] OR
					[InvoiceNo_AP].[HelpText] <> Members.[HelpText] OR
					[InvoiceNo_AP].[Supplier] <> Members.[Supplier] OR 
					[InvoiceNo_AP].[Supplier_MemberId] <> ISNULL([Supplier].[MemberId], -1) OR  
					[InvoiceNo_AP].[Paid] <> Members.[Paid] OR  
					[InvoiceNo_AP].[Paid_Time] <> Members.[Paid_Time] OR  
					[InvoiceNo_AP].[Paid_Time_MemberId] <> ISNULL([Paid_Time].[MemberId], -1) OR
					' + CASE WHEN @MappingTypeID <> 0 THEN '[InvoiceNo_AP].[MemberKeyBase] <> [Members].[MemberKeyBase] OR' ELSE '' END + '
					' + CASE WHEN @MappingTypeID <> 0 THEN '[InvoiceNo_AP].[Entity] <> Members.[Entity] OR' ELSE '' END + '
					' + CASE WHEN @MappingTypeID <> 0 THEN '[InvoiceNo_AP].[Entity_MemberID] <> [Members].[Entity_MemberID] OR' ELSE '' END + '
					' + CASE WHEN @NodeTypeBMYN <> 0 THEN  '[InvoiceNo_AP].[NodeTypeBM] <> [Members].[NodeTypeBM] OR [InvoiceNo_AP].[NodeTypeBM] IS NULL OR' ELSE '' END + '
					[InvoiceNo_AP].[Source] <> Members.[Source]
				)'
	...
	SET @Step = 'Insert new members from source system'
		SET @SQLStatement = '
			...
			SELECT
				...
				[Members].[Source]
			FROM   
				[#InvoiceNo_AP_Members] [Members]
				LEFT JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_Supplier] [Supplier] ON [Supplier].Label COLLATE DATABASE_DEFAULT = [Members].[Supplier]
				LEFT JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_TimeDay] [Paid_Time] ON [Paid_Time].Label COLLATE DATABASE_DEFAULT = [Members].[Paid_Time]
			WHERE
				NOT EXISTS (SELECT 1 FROM [' + @CallistoDatabase + '].[dbo].[S_DS_' + @DimensionName + '] [InvoiceNo_AP] WHERE Members.Label = [InvoiceNo_AP].Label)'
	...
	SET @Step = 'Drop temp tables'
		DROP TABLE [#InvoiceNo_AP_Members]
		DROP TABLE [#EB]
