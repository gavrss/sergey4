...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spGet_DataClass_DimensionMember_20240305_nehatest]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
EXEC [spGet_DataClass_DimensionMember_20240305_nehatest]
	@UserID=7892,
	@InstanceID=678,
	@VersionID=1136,
	@DataClassID=6070,
	@PropertyList='Account.Sign|Entity.Currency|Group.Currency|Time.Level|Time.TimeFiscalYear|Time.TimeYear|Time.TimeMonth|null.null',
	@AssignmentID=NULL,
	@DimensionList='-1',
	@OnlySecuredDimYN=0,
	@ShowAllMembersYN=0,
	...
	@DimensionList='-77|-1|-34|-2|-55|-3|-4|-7|-26|-27|5962|5963|6025',
	@PropertyList='Account.Sign|Entity.Currency|Group.Currency|Time.Level|Time.TimeFiscalYear|Time.TimeYear|Time.TimeMonth|null.null',
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'FDB-1830: After step: "Leaf level members for ...": added precalculated variables: @CountMembers, @SumMembers. Made statement more simple and 40x faster in "while" condition'
				...
				IF @DebugBM & 16 > 0 SELECT [Step] = 'Temp table #DimRows is filled for ' + @DimensionName, [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
					...
					ELSE IF @StorageTypeBM_Dimension & 4 > 0
						BEGIN
                            ...
                            SELECT @CountMembers = 0
                                  ,@SumMembers = 0
                            SELECT @CountMembers = COUNT(1)
                                  ,@SumMembers = SUM(MemberId)
                            FROM #Members P WHERE ISNULL(P.ParentMemberId, 0) <> 0 AND NOT EXISTS (SELECT 1 FROM #Members M WHERE M.MemberID = P.ParentMemberID)

							WHILE (@CountMembers > 0 AND @SumMembers <> @SumMemberID) 
/*						
							WHILE 
