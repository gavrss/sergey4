...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR01_20250629_sega]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2191' SET @Description = 'Fixed join back to initial cursor in the destination loop.'
		IF @Version = '2.1.2.2192' SET @Description = 'Listen to EqualityString in #FilterTable.'
		IF @Version = '2.1.2.2193' SET @Description = 'Delete from FACT independent of how WorkflowState is set. Handle @EnhancedStorageYN.'
		IF @Version = '2.1.2.2199' SET @Description = 'Changed @SQL_Join_Addition2 calculation.'
...
--			@DimensionFilter = ISNULL(DimensionFilter, DimensionSetting)
		...
		SELECT
			...
			@DC_StorageTypeBM = [StorageTypeBM]
		FROM
		...
		SELECT
			@DataClassDatabase = A.[DestinationDatabase],
	...
	IF @DebugBM & 16 > 0 SELECT [Step] = 'After the Step: ' + @Step, [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
...
--			NOT EXISTS (SELECT 1 FROM BR01_Step D WHERE D.BusinessRuleID = S.BusinessRuleID AND D.BR01_StepPartID = -1 AND D.MemberKey = S.MemberKey AND ISNULL(D.DataClassID, M.DataClassID) = S.DataClassID)
			NOT EXISTS (SELECT 1 FROM BR01_Step D WHERE D.BusinessRuleID = S.BusinessRuleID AND D.BR01_StepPartID = -1 AND D.MemberKey = S.MemberKey AND D.DataClassID = ISNULL(S.DataClassID, M.DataClassID))
		GROUP BY
			S.[BR01_StepID],
			S.MemberKey,
			S.ModifierID,
					...
					ELSE
						BEGIN
							EXEC pcINTEGRATOR..spGet_FilterTable
								...
								@StorageTypeBM = 4, --@StorageTypeBM,
								@Debug = 0
--                         	IF @DebugBM & 16 > 0 SELECT [Step] = 'After spGet_FilterTable', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
...
--                     IF @DebugBM & 16 > 0 SELECT [Step] = 'After SELECT @SQL_Insert, @SQL_Select', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)

					SELECT
						@SQL_Where = @SQL_Where +
							CASE WHEN ISNULL(FT.[LeafLevelFilter], '') <> '' AND ISNULL(FT.[PropertyName], '') = ''
							THEN
								CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'DC.[' + FT.[DimensionName] + '_MemberID] ' + FT.[EqualityString] + ' (' + FT.[LeafLevelFilter] + ') AND'
							ELSE
								''
							END,
						@SQL_Join = @SQL_Join +
					...
					IF @DebugBM & 2 > 0 PRINT '@SQL_Where:' + CHAR(13) + CHAR(10) + @SQL_Where
					...
					IF @ModifierID = 70
						BEGIN
                            ...
                            -- =================================================================================
                            -- OPTIMIZED DYNAMIC SQL BLOCK
                            -- Replace your existing @SQLStatement construction logic with this block.
                            -- =================================================================================

                            DECLARE @GroupByFinal NVARCHAR(MAX);

                            -- Step 1: Create a clean list of columns for the final GROUP BY clause.
                            -- This removes the 'DC.' and 'MV.' table aliases from your @SQL_GroupBy variable.
                            -- Note: If you use other aliases in @SQL_GroupBy, add more REPLACE functions.
                            SET @GroupByFinal = REPLACE(REPLACE(@SQL_GroupBy, 'DC.', ''), 'MV.', '');


                            -- Step 2: Build the new, efficient query string using a Common Table Expression (CTE).
                            declare @SQLStatementOptimal nvarchar(max);
                            SET @SQLStatementOptimal = '
                            -- Use a Common Table Expression (CTE) to select the filtered data in a single pass.
                            -- This avoids scanning the large FACT table multiple times.
                            WITH DataCTE AS (
                                SELECT '
                                    + @SQL_Select_MV_70 + ', -- This is your existing variable for dimension selection.
                                    MV.ModifierID,
                                    DC.[' + @MasterDataClassName + '_Value]
                                FROM
                                    [' + @DataClassDatabase + '].[dbo].[FACT_' + @MasterDataClassName + '_default_partition] AS DC
                                    ' + @SQL_Join + '
                                    -- The JOIN to the modifier table now efficiently filters for both sets of data at once.
                                    INNER JOIN [#Modifier_Value] AS MV ON DC.Time_MemberId = MV.[MemberKey_Modified]
                                WHERE
                                    MV.DimensionName = ''Time''
                                    AND (
                                        (MV.ModifierID = 0 AND MV.Parameter = 0)
                                        OR
                                        (MV.ModifierID = ' + CONVERT(NVARCHAR(10), @ModifierID) + ' AND MV.Parameter = ' + CONVERT(NVARCHAR(50), @Parameter) + ')
                                    )
                                    AND ' + @SQL_Where + '
                            )
                            -- Insert the final result, calculated using conditional aggregation on the CTE data.
                            -- This avoids the expensive LEFT JOINs from the original query.
                            INSERT INTO #BR01_FACT (' + @SQL_Insert + ')
                            SELECT
                                ' + REPLACE(
                                        REPLACE(
                                            @SQL_Select_70, -- Start with your original final SELECT list
                                            'M.[', '['      -- Remove the "M." alias prefix, as columns now come directly from the CTE
                                        ),
                                        -- Replace the original, multi-join calculation with the new, efficient conditional aggregation
                                        '(ISNULL(CM.[' + @MasterDataClassName + '_Value], 0.0) - ISNULL(PM.[' + @MasterDataClassName + '_Value], 0.0)) * 1',
                                        '(
                                            ISNULL(SUM(CASE WHEN ModifierID = 0 THEN [' + @MasterDataClassName + '_Value] ELSE 0 END), 0.0)
                                            -
                                            ISNULL(SUM(CASE WHEN ModifierID = ' + CONVERT(NVARCHAR(10), @ModifierID) + ' THEN [' + @MasterDataClassName + '_Value] ELSE 0 END), 0.0)
                                        )'
                                    ) + '
                            FROM
                                DataCTE
                            GROUP BY
                                ' + @GroupByFinal + ';'; -- Use the cleaned list of columns for the GROUP BY

                            -- For debugging, you can print the generated statement before executing it.
                            -- PRINT @SQLStatement;
						END
					ELSE
						BEGIN
					...
					IF @DebugBM & 2 > 0 AND LEN(@SQLStatement) > 4000
						BEGIN
							...
							SET @Comment = '#BR01_FACT Initial; @InitialStepID = ' + CONVERT(NVARCHAR(10), @InitialStepID) + ' Optimal query';
							EXEC [dbo].[spSet_wrk_Debug]
								@UserID = @UserID,
								@InstanceID = @InstanceID,
								@VersionID = @VersionID,
								...
								@CalledProcedureName = @ProcedureName,
								@Comment = @Comment,
								@SQLStatement = @SQLStatementOptimal
						END
					ELSE IF @DebugBM & 2 > 0
						PRINT @SQLStatement

--                     IF @DebugBM & 16 > 0 SELECT [Step] = 'Before main exec', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)

					EXEC (@SQLStatement)

                    IF @DebugBM & 16 > 0 SELECT [Step] = 'After main exec', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
...
--						@SQL_Join_Multiply = @SQL_Join_Multiply + CASE WHEN DCL.BaseObjectYN = 0 AND DCL.DataType <> 'float' THEN CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'OP.[' + ColumnName + '] = F.[' + ColumnName + '] AND' ELSE '' END,
						...
						@SQL_Join_Addition2 = @SQL_Join_Addition2 + CASE WHEN DCL.BaseObjectYN = 0 AND DCL.DataType <> 'float' THEN CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '(OP.[' + ColumnName + '] = M.[' + ColumnName + '] OR OP.[' + ColumnName + '] IS NULL) AND' ELSE '' END
					FROM
						#DataClassList DCL
					WHERE
...
--                     IF @DebugBM & 16 > 0 SELECT [Step] = 'Before exec1', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
					EXEC (@SQLStatement)
                    IF @DebugBM & 16 > 0 SELECT [Step] = 'After exec1', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
					...
					WHILE @BR01_StepPartID <= (SELECT MAX(BR01_StepPartID) FROM BR01_Step S WHERE S.BusinessRuleID = @BusinessRuleID AND S.[BR01_StepID] = @BR01_StepID)
						BEGIN
							...
							FROM
								...
								LEFT JOIN #BR01_Initial_Cursor_Table [IS] ON [IS].[BR01_StepID] = S.[BR01_StepID] AND [IS].[MemberKey] = S.[MemberKey]
								AND ISNULL([IS].[ModifierID], 0) = ISNULL(S.[ModifierID], 0) AND ISNULL([IS].[Parameter], 0) = ISNULL(S.[Parameter], 0)
								AND ISNULL([IS].[DimensionFilter], '') = ISNULL(S.[DimensionFilter],'')
							...
							IF @InitialStepID IS NULL
								BEGIN
									...
									IF @DebugBM & 2 > 0 PRINT @SQLStatement
--                                     IF @DebugBM & 16 > 0 SELECT [Step] = 'Before exec2', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
                                    EXEC (@SQLStatement)
                                    IF @DebugBM & 16 > 0 SELECT [Step] = 'After exec2', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
						...
						WHERE
							...
							+ 'AND (F.[InitialStepID] IS NULL )'


									IF @DebugBM & 2 > 0 PRINT @SQLStatement
--                                     IF @DebugBM & 16 > 0 SELECT [Step] = 'Before exec3', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
                                    EXEC (@SQLStatement)
                                    IF @DebugBM & 16 > 0 SELECT [Step] = 'After exec3', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
...
--                                 IF @DebugBM & 16 > 0 SELECT [Step] = 'Before exec4', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
                                EXEC (@SQLStatement)
                                IF @DebugBM & 16 > 0 SELECT [Step] = 'After exec4', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
							END
						...
						WHERE
							[' + @BaseObject + '_MemberID] = ' + CONVERT(NVARCHAR(20), @DestinationMemberID)
									...
									IF @DebugBM & 2 > 0 PRINT @SQLStatement
--                                     IF @DebugBM & 16 > 0 SELECT [Step] = 'Before exec5', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
                                    EXEC (@SQLStatement)
                                    IF @DebugBM & 16 > 0 SELECT [Step] = 'After exec5', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
...
--                                                     IF @DebugBM & 16 > 0 SELECT [Step] = 'Before exec6', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
                                                    EXEC (@SQLStatement)
                                                    IF @DebugBM & 16 > 0 SELECT [Step] = 'After exec6', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
						...
						WHERE
							F.[' + @BaseObject + '_MemberID] = ' + CONVERT(NVARCHAR(20), @DestinationMemberID)
											...
											IF @DebugBM & 2 > 0 PRINT @SQLStatement
--                                             IF @DebugBM & 16 > 0 SELECT [Step] = 'Before exec7', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
                                            EXEC (@SQLStatement)
                                            IF @DebugBM & 16 > 0 SELECT [Step] = 'After exec7', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
										END
									ELSE IF @Operator IN ('+', '-')
										BEGIN
...
--                                             IF @DebugBM & 16 > 0 SELECT [Step] = 'Before exec8', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
                                            EXEC (@SQLStatement)
                                            IF @DebugBM & 16 > 0 SELECT [Step] = 'After exec8', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
						...
						IF @ModifierID IN (10, 20)
							BEGIN
								...
								IF @DebugBM & 2 > 0 PRINT @SQLStatement
--                                 IF @DebugBM & 16 > 0 SELECT [Step] = 'Before exec9', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
                                EXEC (@SQLStatement)
                                IF @DebugBM & 16 > 0 SELECT [Step] = 'After exec9', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
							END
						...
						IF @DebugBM & 1 > 0 PRINT @SQLStatement
--                         IF @DebugBM & 16 > 0 SELECT [Step] = 'Before exec10', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
                        EXEC (@SQLStatement)
                        IF @DebugBM & 16 > 0 SELECT [Step] = 'After exec10', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
						...
						IF @DebugBM & 2 > 0
							BEGIN
								...
								IF @DebugBM & 1 > 0 PRINT @SQLStatement
--                                 IF @DebugBM & 16 > 0 SELECT [Step] = 'Before exec11', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
                                EXEC (@SQLStatement)
                                IF @DebugBM & 16 > 0 SELECT [Step] = 'After exec11', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
							END
...
--------
	...
	SET @Step = 'Delete existing records from FACT table'
		...
		WHERE
			...
			DCL.DimensionID NOT IN (-63) --WorkflowState
		ORDER BY
			SortOrder
