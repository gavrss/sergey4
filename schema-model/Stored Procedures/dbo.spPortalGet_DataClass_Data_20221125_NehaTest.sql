...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_20221125_NehaTest]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [pcINTEGRATOR].[dbo].[spPortalGet_DataClass_Data_20221125_NehaTest] @DebugBM=63,@ActingAs='5901',@DataClassID='5799',
@Filter='TimeView=Periodic|Account=Income_Statement_|Flow=All_|BusinessProcess=CONSOLIDATED|BusinessRule=All_|Currency=CAD|Entity=CIOT|Scenario=ACTUAL|Time=FY2022|Version=NONE|GL_Branch=All_|GL_Cost_Center=All_|GL_Department=All_|GL_Posted=TRUE'
,@GroupBy='Time.TimeFiscalYear|Account',@InstanceID='598',@Measure='Financials',@ResultTypeBM='4',
@RowList='SupressZeroYN=0',@UseCacheYN='true',@UserID='10841',@VersionID='1092'
			...
			BEGIN
				...
				IF @DebugBM & 2 > 0
					BEGIN
						...
						SET @SQLStatement = '
							SELECT [Account_MemberKey] = A.[Label], TempTable = ''' + @TmpGlobalTable + ''', tmp.* 
							FROM ' + @TmpGlobalTable + ' tmp
							INNER JOIN pcDATA_CIOT.dbo.[S_DS_Account] A ON A.[MemberId] = tmp.[Account_MemberId]
							ORDER BY A.[Label]'
						EXEC (@SQLStatement)
					END
			END
			...
			BEGIN
				...
				IF @DebugBM & 32 > 0
					BEGIN
						--SET @SQLStatement = '			
						--	SELECT
						--		[ResultTypeBM] = 4444,' + @SQL_Select2 + CASE WHEN @ResultTypeBM & 128 > 0 THEN CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CASE WHEN @LineItemExistsYN <> 0 THEN '[LineItemYN] = MAX(CASE WHEN LI.[Dummy] IS NOT NULL AND [TimeView].[MemberId] = [Time].[MemberId] THEN 1 ELSE 0 END),' ELSE '[LineItemYN] = 0,' END ELSE '' END + CASE WHEN @WorkflowStateYN <> 0 THEN '
						--		[WorkflowStateID] = ' + CASE WHEN LEN(@TimeFilterString) > 0 THEN 'MAX(CASE WHEN [TimeView].[MemberId] = [Time].[MemberId] THEN DC.WorkflowStateID ELSE -1 END)' ELSE 'MAX(DC.[WorkflowStateID])' END + ',
						--		[Workflow_UpdatedBy_UserID] = MAX(DC.[Workflow_UpdatedBy_UserID]),' ELSE '' END + @SQL_Tuple + '
						--		[' + @Measure + '_Value] = ' + CASE WHEN LEN(@TimeFilterString) > 0 THEN 'ROUND(SUM(CASE WHEN [TimeView].[MemberId] = [Time].[MemberId] THEN DC.[' + @Measure + '_Value] ELSE 0 END), 4)' ELSE 'ROUND(SUM(DC.[' + @Measure + '_Value]), 4)' END + '
						--	FROM
						...
						SET @SQLStatement = '			
							SELECT
								[ResultTypeBM] = 444444444,
								[Account_MemberKey] = [Account].[Label],
								[Time_TimeFiscalYear] = [Time].[TimeFiscalYear],
								[DC.Time_MemberId] = max(DC.[Time_MemberId]),
								[TimeView.MemberID] = max([TimeView].[MemberID]),
								[Time.MemberId] = max([Time].[MemberId]),
								...
								[Financials_Value] = ROUND(SUM(CASE WHEN [TimeView].[MemberId] = [Time].[MemberId] THEN DC.[Financials_Value] ELSE 0 END), 4)
							FROM
								' + @TmpGlobalTable + '  DC
								INNER JOIN #Time [TimeView] ON [TimeView].[MemberID] = DC.[Time_MemberId]
								INNER JOIN #Time [Time] ON 1 = 1
								LEFT JOIN [pcDATA_CIOT].[dbo].[S_DS_Account] [Account] ON [Account].MemberId = DC.[Account_MemberId]
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
							...
							IF @Filter IS NOT NULL AND @PropertyNameFilter IS NULL
								BEGIN
									EXEC pcINTEGRATOR..spGet_LeafLevelFilter_New3
										@UserID = @UserID,
										@InstanceID = @InstanceID,
										@VersionID = @VersionID,
