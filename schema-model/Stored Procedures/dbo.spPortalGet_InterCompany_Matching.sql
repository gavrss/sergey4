...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_InterCompany_Matching]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'Commented the line: --SET @YearMonth = @Time, Set variable @YearMonth = max of YearMonth in temp table #YearMonth. If @ResultTypeBM = 16, Set Entity = NULL. Only include Entities included in advanced consolidations (Exist in EntityHierarchy)'
...
--SET @YearMonth = @Time
	...
	SET @Step = 'Create temp table #EntityExist'
		CREATE TABLE #EntityExist
			(
			Entity nvarchar(50)
			)
...
IF @Debug <> 0 SELECT 'Arne1'
				...
				SELECT
					@InterCompany_BusinessRuleID = MAX(BusinessRuleID)
				FROM
					pcINTEGRATOR_Data..BusinessRule
				WHERE
					InstanceID = @InstanceID AND
					VersionID = @VersionID AND
					BR_TypeID = 14 AND
				...
				IF @DebugBM & 2 > 0 SELECT [@InterCompany_BusinessRuleID] = @InterCompany_BusinessRuleID
				...
				DECLARE ICmatch_Cursor CURSOR FOR
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
							...
							Truncate Table #EntityExist

							INSERT INTO #EntityExist 
								(Entity)
							SELECT Distinct 
								Entity = E.MemberKey 
                            FROM 
								[pcINTEGRATOR_Data].[dbo].[Entity] E
								Inner Join [pcINTEGRATOR_Data].[dbo].[EntityHierarchy] EH ON EH.EntityID = E.EntityID
							WHERE
								E.InstanceID = @InstanceID AND E.VersionID = @VersionID AND E.SelectYN <> 0

							If @DebugBM & 2 > 0 SELECT Temptable = '#EntityExist', * from #EntityExist order BY Entity   
							...
							SET @SQLStatement = '
								...
								FROM 
									' + @JournalTable + ' J
									Inner Join #EntityExist E1 ON E1.Entity = J.Entity 
									Inner Join #EntityExist E2 ON E2.Entity = J.[InterCompanyEntity]
								WHERE
...
--									J.[FiscalPeriod] <= ' + CONVERT(nvarchar(15), @FiscalPeriod) + ' AND
				...
				IF @DebugBM & 2 > 0 SELECT [#MatchingBase8] = '#MatchingBase8', TempTable = '#MatchingBase8', * FROM #MatchingBase8
	...
	SET @Step = 'Fill #MatchingBase8 from [pcINTEGRATOR_Log].[dbo].[wrk_InterCompany_Matching]'
		IF @ResultTypeBM & 24 > 0 AND @ResultTypeBM & 32 = 0
			BEGIN
				IF @DebugBM & 2 > 0 
					BEGIN
						SELECT Info = 'Insert data from [pcINTEGRATOR_Log].[dbo].[wrk_InterCompany_Matching] into #MatchingBase8'
						SELECT
							[@InstanceID] = @InstanceID,
							[@VersionID] = @VersionID,
							[@Entity] = @Entity,
	...
	SET @Step = '@ResultTypeBM & 8'
		IF @ResultTypeBM & 8 > 0
			BEGIN
				...
				IF @DebugBM & 2 > 0 SELECT [@Rule_ICmatchID] = @Rule_ICmatchID, [@RULE_Consolidation] = @RULE_Consolidation
