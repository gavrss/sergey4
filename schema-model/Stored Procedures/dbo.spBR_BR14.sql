...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR14]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'Join on Entity when using MemberKeyBase, Added test on [Method] = Property when adding [PropertyFilter] into #FilterTable'
					...
					IF @Method = 'Property'
						BEGIN
							...
							SELECT
								...
								[Method] = 'Property'
							WHERE
								NOT EXISTS (SELECT 1 FROM #FilterTable FT WHERE FT.[StepReference] = @StepReference AND FT.[DimensionName] = LEFT(@ICCounterpart, CHARINDEX('.', @ICCounterpart) -1) AND FT.[Method] = 'Property')
						...
						WHILE @@FETCH_STATUS = 0
							BEGIN
								...
								IF @DimMethod = 'Property' AND @PropertyLen = 0
									BEGIN
										SET @SQLJoin = @SQLJoin + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'INNER JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_' + @PropertyDimension + '] [' + @PropertyDimension + '] ON [' + @PropertyDimension + '].[' + CASE WHEN @MappingTypeID > 0 THEN 'MemberKeyBase' ELSE 'Label' END + '] = J.[' + ISNULL(@Segment, @PropertyDimension) + ']' + CASE WHEN @MappingTypeID > 0 THEN ' AND [' + @PropertyDimension + '].[Entity] = J.[Entity]' ELSE '' END
									END
								ELSE
									BEGIN
										SET @SQLJoin = @SQLJoin + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'INNER JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_' + @PropertyDimension + '] [' + @PropertyDimension + '] ON [' + @PropertyDimension + '].[' + CASE WHEN @MappingTypeID > 0 THEN 'MemberKeyBase' ELSE 'Label' END + '] = J.[' + @PropertyDimension + ']' + CASE WHEN @MappingTypeID > 0 THEN ' AND [' + @PropertyDimension + '].[Entity] = J.[Entity]' ELSE '' END
