...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spCheckSum_TrialBalance_Journal] 
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,

	--SP-specific parameters
	@SequenceBM int = 2,	--2 - GLJrnDtl VS EFP Journal;
...
EXEC [spCheckSum_TrialBalance_Journal] @UserID = -10, @InstanceID = 531, @VersionID = 1057, @CheckSumValue = @CheckSumValue OUT, @CheckSumStatus10 = @CheckSumStatus10 OUT, @CheckSumStatus20 = @CheckSumStatus20 OUT, @CheckSumStatus30 = @CheckSumStatus30 OUT, @CheckSumStatus40 = @CheckSumStatus40 OUT, @JobID = 34814
...
--Get details
EXEC [spCheckSum_TrialBalance_Journal] @UserID = -10, @InstanceID = 531, @VersionID = 1057, 
@ResultTypeBM = 4, @CheckSumStatusBM = 7, @DebugBM=3

EXEC spRun_Procedure_KeyValuePair
	@ProcedureName = 'spCheckSum_TrialBalance_Journal',
	@JSON = '
		[
...
EXEC [spCheckSum_TrialBalance_Journal] @UserID=-10, @InstanceID=531, @VersionID=1057, @DebugBM=3,@SequenceBM=4,@ShowAllYN=0
,@ResultTypeBM=7,@Entity_MemberKey='PCXNW',@Entity_Book='MAIN',@FiscalYear=2021,@FiscalPeriod=1
,@Account='40000'

EXEC [spCheckSum_TrialBalance_Journal] @UserID=-10, @InstanceID=509, @VersionID=1045, @DebugBM=3
,@ResultTypeBM=7,@ShowAllYN=0
,@FiscalYear=2021

EXEC [spCheckSum_TrialBalance_Journal] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Compare Journal data between between [Erp].[GLJrnDtl] and EFP Journal.',
...
*/
	...
	SET @Step = 'Create temp table #TrialBalance_CheckSum_ByFiscalPeriod'
		...
		FROM
			#TrialBalance
		WHERE
			[SourceTable] IN ('GLJrnDtl', 'Journal_excl_FP0')
		GROUP BY
			[Entity],
			[Book],
			[FiscalYear],
		...
		IF @ResultTypeBM & 1 > 0
			BEGIN 
				...
				SELECT
					...
					[VersionID] = @VersionID,
					[ProcedureID] = @ProcedureID,
					[CheckSumRowKey] = sub.[Entity] + '_' + sub.[Book] + '_' + CONVERT(NVARCHAR(15), sub.[FiscalYear]) + '_' + CONVERT(NVARCHAR(15), sub.[FiscalPeriod]) + '_' + sub.[Account] + '_Journal_Diff_' +  CONVERT(NVARCHAR(20), ROUND(sub.[Journal] - sub.[GLJrnDtl], 4))
				FROM
					#TrialBalance_CheckSum_ByFiscalPeriod sub
				WHERE
					ROUND(sub.[Journal] - sub.[GLJrnDtl], 4) <> 0 AND
					NOT EXISTS (SELECT 1 FROM [pcINTEGRATOR_Log].[dbo].[CheckSumRowLog] CSRL WHERE CSRL.[CheckSumRowKey] = sub.[Entity] + '_' + sub.[Book] + '_' + CONVERT(NVARCHAR(15), sub.[FiscalYear]) + '_' + CONVERT(NVARCHAR(15), sub.[FiscalPeriod]) + '_' + sub.[Account] + '_Journal_Diff_' +  CONVERT(NVARCHAR(20), ROUND(sub.[Journal] - sub.[GLJrnDtl], 4)) AND CSRL.[ProcedureID] = @ProcedureID AND CSRL.[InstanceID] = @InstanceID AND CSRL.[VersionID] = @VersionID AND CSRL.[CheckSumStatusBM] & 12 = 0)
				ORDER BY 
	...
	SET @Step = 'Calculate @CheckSumValue'
		IF @ResultTypeBM & 3 > 0
			BEGIN
				...
				FROM
					[pcINTEGRATOR_Log].[dbo].[CheckSumRowLog] CSRL
					--INNER JOIN #TrialBalance_CheckSum_ByFiscalPeriod TB ON TB.InstanceID = CSRL.[InstanceID] AND TB.VersionID = CSRL.[VersionID] AND CSRL.[CheckSumRowKey] LIKE '%_Journal_Diff_%'
				WHERE
					CSRL.[InstanceID] = @InstanceID AND
					CSRL.[VersionID] = @VersionID AND
					CSRL.[ProcedureID] = @ProcedureID AND
                    CSRL.[CheckSumRowKey] LIKE '%_Journal_Diff_%'
				OPTION (MAXDOP 1)

				IF @Debug <> 0 
					SELECT 
						[CheckSumRowKey] = 'Journal_Diff',
		...
		IF @ResultTypeBM & 4 > 0
			BEGIN
				SELECT
					...
					[Journal] = TB.Journal,
					[Journal_Diff] = ROUND(TB.[Journal] - TB.[GLJrnDtl], 4),
