...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_OrderNo_Raw_20220128]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
				...
				IF @SequenceBMStep & 2 > 0 --Parent
					BEGIN
						SET @SQLStatement = '
							...
							--WHERE
							--	YEAR(OH.OrderDate) >= ' + CONVERT(NVARCHAR(15), @StartYear) + ' AND
							--	NOT EXISTS (SELECT 1 FROM ' + @ETLDatabase + '.[dbo].[ClosedPeriod] CPC WHERE
							--				CPC.[SourceID] = ' + CONVERT(NVARCHAR(15), @SourceID_Sales) + ' AND 
							--				CPC.[EntityCode] = EB.Entity AND
							--				CPC.[TimeYear] * 100 + CPC.[TimeMonth] = CONVERT(int, CONVERT(nvarchar(10), OH.OrderDate, 112)) / 100 AND
							--				CPC.[UpdateYN] = 0)'
		...
		IF @CalledYN = 0
			BEGIN
				SELECT
					...
					*
					--[MemberId],
					--[MemberKey] = CASE WHEN @MappingTypeID IN (0, 1) AND [Entity] <> '' THEN [Entity] + '_' ELSE '' END + [MemberKey] + CASE WHEN (SELECT CASE WHEN @MappingTypeID IN (0, 1) THEN 1 ELSE @MappingTypeID END) = 2 AND [Entity] <> '' THEN '_' + [Entity] ELSE '' END,
					--[Description],
					--[HelpText],
					--[NodeTypeBM],
					--[Source],
