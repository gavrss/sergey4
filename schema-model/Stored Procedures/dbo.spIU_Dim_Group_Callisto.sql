...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_Group_Callisto]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_Dim_Group_Callisto] @UserID = -10, @InstanceID = -1335, @VersionID = -1273, @DebugBM = 1
EXEC [spIU_Dim_Group_Callisto] @UserID = -10, @InstanceID = 476, @VersionID = 1029, @DebugBM = 3
EXEC [spIU_Dim_Group_Callisto] @UserID = -10, @InstanceID = 527, @VersionID = 1043, @DebugBM = 3

EXEC [spIU_Dim_Group_Callisto] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Insert rows into Callisto Group dimension table.',
...
BEGIN TRY
	...
	SET @Step = 'Fetch members'
		EXEC [spIU_Dim_Group_Raw] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @JobID=@JobID, @Debug = @DebugSub

		UPDATE #Group_Members SET [Label] = [MemberKey], [RNodeType] = CASE WHEN NodeTypeBM & 1 > 0 THEN 'L' ELSE '' END + CASE WHEN NodeTypeBM & 2 > 0 THEN 'P' ELSE '' END  + CASE WHEN NodeTypeBM & 8 > 0 THEN 'C' ELSE '' END

		IF @DebugBM & 2 > 0 SELECT TempTable = '#Group_Members', * FROM #Group_Members
	...
	SET @Step = 'Update Description and dimension specific Properties from source system where Synchronized is set to true.'
		SET @SQLStatement = '
			...
			FROM
				[' + @CallistoDatabase + '].[dbo].[S_DS_Group] [Group] 
				INNER JOIN [#Group_Members] Members ON Members.[Label] = [Group].[Label] 
			WHERE 
				[Group].[Synchronized] <> 0'
	...
	SET @Step = 'Insert new members from source system'
		SET @SQLStatement = '
			INSERT INTO [' + @CallistoDatabase + '].[dbo].[S_DS_Group]
				(
				[MemberId],
				[Label],
				[Description],
			...
			WHERE
				NOT EXISTS (SELECT 1 FROM [' + @CallistoDatabase + '].[dbo].[S_DS_Group] [Group] WHERE Members.Label = [Group].Label)'
	...
	SET @Step = 'Update MemberId'
		EXEC [pcINTEGRATOR].[dbo].[spSet_MemberId] @Database = @CallistoDatabase, @Dimension = N'Group', @JobID = @JobID, @Debug = @DebugSub
	...
	SET @Step = 'Return rows'
		IF @DebugBM & 1 > 0 EXEC('SELECT [Table] = ''[' + @CallistoDatabase + '].[dbo].[S_DS_Group]'', * FROM [' + @CallistoDatabase + '].[dbo].[S_DS_Group]')
