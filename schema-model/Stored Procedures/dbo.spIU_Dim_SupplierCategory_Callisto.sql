...
CREATE PROCEDURE [dbo].[spIU_Dim_SupplierCategory_Callisto]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
BEGIN TRY
	...
	SET @Step = 'Update Description and dimension specific Properties from source system where Synchronized is set to true.'
		SET @SQLStatement = '
			...
			WHERE 
				[SupplierCategory].[Synchronized] <> 0 AND
				(
					[SupplierCategory].[Description] <> Members.[Description] OR
					[SupplierCategory].[HelpText] <> Members.[HelpText] OR
					' + CASE WHEN @MappingTypeID <> 0 THEN 'isnull([SupplierCategory].[MemberKeyBase], '''') <> [Members].[MemberKeyBase] OR' ELSE '' END + '
					' + CASE WHEN @MappingTypeID <> 0 THEN 'isnull([SupplierCategory].[Entity] <> Members.[Entity], '''') OR' ELSE '' END + '
					' + CASE WHEN @MappingTypeID <> 0 THEN 'isnull([SupplierCategory].[Entity_MemberID], 0) <> [Members].[Entity_MemberID] OR' ELSE '' END + '
					' + CASE WHEN @NodeTypeBMYN <> 0 THEN  'isnull([SupplierCategory].[NodeTypeBM], 0) <> [Members].[NodeTypeBM] OR [SupplierCategory].[NodeTypeBM] IS NULL OR' ELSE '' END + '
					[SupplierCategory].[Source] <> Members.[Source]
				)'
