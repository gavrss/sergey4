...
CREATE PROCEDURE [dbo].[spJIRA_IU_API_Calls]
...
exec spJIRA_IU_API_Calls @Operation = 'GetStatus', @BodyData = '', @Exist_JiraIssueID = 58726, @Exist_JiraIssueStatusID = 10014, @issueID = @issueID OUT, @issueStatusID = @issueStatusID out
...
DECLARE @Url VARCHAR(8000) = CASE 
								WHEN @Operation = 'Insert'			THEN 'https://dspanel.atlassian.net/rest/api/3/issue'
								WHEN @Operation = 'Update'			THEN 'https://dspanel.atlassian.net/rest/api/3/issue/' + CAST(@Exist_JiraIssueID AS VARCHAR(255))
								WHEN @Operation = 'UpdateFromJira'	THEN 'https://dspanel.atlassian.net/rest/api/3/search'
								WHEN @Operation = 'GetVersionsList'	THEN 'https://dspanel.atlassian.net/rest/api/3/project/' + @JiraProject + '/versions'
								WHEN @Operation = 'GetDBEnvironments' THEN 'https://dspanel.atlassian.net/rest/api/3/field/customfield_12962/context/14272/option'
								WHEN @Operation = 'GetUserList'		THEN 'https://dspanel.atlassian.net/rest/api/3/users?maxResults=1000'
								WHEN @Operation = 'AddVersion'		THEN 'https://dspanel.atlassian.net/rest/api/3/version'
								ELSE									 'https://dspanel.atlassian.net/rest/api/3/issue/' + CAST(@Exist_JiraIssueID AS VARCHAR(255)) + '?fields=status'
								END, 
		@Method VARCHAR(5) = CASE 
								...
								ELSE							'GET'
								END,
		@Authorization VARCHAR(8000) = 'Basic c2VyZ2lpLmdhdnJ5bGVua29AZHNwYW5lbC5jb206TVlZM3hqYkZFOUpEQ2VGNGEwV2oyRUVD',--Basic auth token, Api key,...
...
'
{
    ...
    "id": "58726",
    "self": "https://dspanel.atlassian.net/rest/api/3/issue/58726",
    "key": "SEG-182",
    "fields": {
        "status": {
            "self": "https://dspanel.atlassian.net/rest/api/3/status/10434",
            "description": "This issue is being actively worked on at the moment by the assignee.",
            "iconUrl": "https://dspanel.atlassian.net/",
            "name": "In Progress",
            "id": "10434",
            "statusCategory": {
                "self": "https://dspanel.atlassian.net/rest/api/3/statuscategory/4",
                "id": 4,
                "key": "indeterminate",
                "colorName": "yellow",
                "name": "In Progress"
...
------- additional: on Update and if current stauts in (Deployed, Closed) then change status to "Reopened" -------------------------------------------------------------

IF (@Operation = 'Update' AND @Exist_JiraIssueStatusID IN (10001, 6)) -- (Deployed, Closed)
BEGIN
	DECLARE @UrlTransition VARCHAR(8000) =  'https://dspanel.atlassian.net/rest/api/3/issue/' + CAST(@Exist_JiraIssueID AS VARCHAR(255)) + '/transitions'
...
IF @Operation ='GetVersionsList'
	BEGIN
		...
		'[
				{
					"self": "https://dspanel.atlassian.net/rest/api/3/version/17062",
					"id": "17062",
					"name": "2.1.1.2176",
					"archived": false,
					"released": true,
				...
				{
					"self": "https://dspanel.atlassian.net/rest/api/3/version/17026",
					"id": "17026",
					"name": "2.0.2.2144",
					"archived": false,
					"released": true,
				...
				{
					"self": "https://dspanel.atlassian.net/rest/api/3/version/17027",
					"id": "17027",
	...
	BEGIN
		...
		'[
			{
				"self": "https://dspanel.atlassian.net/rest/api/3/user?accountId=557058:5ea453f3-5741-4d89-a716-60c8aa2d2991",
				"accountId": "557058:5ea453f3-5741-4d89-a716-60c8aa2d2991",
				"accountType": "atlassian",
				"emailAddress": "andrey.polyakov@dspanel.com",
				"avatarUrls": {
			...
			{
				"self": "https://dspanel.atlassian.net/rest/api/3/user?accountId=5be37f8aa430e70fb00dfdaa",
			...
			{
				"self": "https://dspanel.atlassian.net/rest/api/3/user?accountId=557058:2ceed135-912e-4ead-a65a-9cb8a177c304",
				"accountId": "557058:2ceed135-912e-4ead-a65a-9cb8a177c304",
				"accountType": "atlassian",
				"emailAddress": "eve.liu@dspanel.com",
				"avatarUrls": {
...
IF @Operation in ('Insert', 'Update')
	BEGIN
		...
		exec spJIRA_IU_API_Calls @Operation = 'GetStatus', @BodyData = '', @Exist_JiraIssueID = @issueIDforGetStatus, @issueID = @issueID OUT, @issueStatusID = @issueStatusID out
    END 
	...
	BEGIN
		...
		UPDATE EA
		   SET   [JiraIssueStatusID]	= JIRA.statusID
				,ClearedYN				= CASE WHEN jira.statusName in ('Deployed', 'Closed') THEN 1
												ELSE 0
												END
				...
				"issues": [
					{
						...
						"id": "58728",
						"self": "https://dspanel.atlassian.net/rest/api/3/issue/58728",
						"key": "SEG-183",
						"fields": {
							"summary": "Error Attention",
							"status": {
								"self": "https://dspanel.atlassian.net/rest/api/3/status/10433",
								"description": "",
								"iconUrl": "https://dspanel.atlassian.net/",
								"name": "To Do",
								"id": "10433",
								"statusCategory": {
									"self": "https://dspanel.atlassian.net/rest/api/3/statuscategory/2",
									"id": 2,
									"key": "new",
									"colorName": "blue-gray",
									"name": "To Do"
					...
					},
					{
						"expand": "operations,versionedRepresentations,editmeta,changelog,renderedFields",
						"id": "58725",
						"self": "https://dspanel.atlassian.net/rest/api/3/issue/58725",
						"key": "SEG-181",
						"fields": {
					...
					},
					{
						"expand": "operations,versionedRepresentations,editmeta,changelog,renderedFields",
						"id": "58718",
						"self": "https://dspanel.atlassian.net/rest/api/3/issue/58718",
						"key": "SEG-174",
						"fields": {
