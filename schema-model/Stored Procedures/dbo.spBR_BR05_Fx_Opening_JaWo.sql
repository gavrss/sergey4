...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR05_Fx_Opening_JaWo]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
SELECT TempTable = '#HistoricCursor_Table', * FROM #HistoricCursor_Table ORDER BY SortOrder
			END
...
--
				SELECT [@JournalFilter]=@JournalFilter
--				
				IF LEN(@JournalFilter) > 3
					SET @JournalFilter = '(' + LEFT(@JournalFilter, LEN(@JournalFilter) - 3) + ')'
				
				IF LEN(@JournalBaseFilter) > 0
					SET @JournalBaseFilter = '[Rule_FXID] IN (' + LEFT(@JournalBaseFilter, LEN(@JournalBaseFilter) - 1) + ')'
...
--							J.[Account] NOT IN (''CYNI_B'') AND
				...
				SET @SQLStatement = @SQLStatement + '
					...
					GROUP BY
						...
						sub.[Book],
						sub.[Account],
						CASE WHEN sub.[JournalSequence] IN (''G_ELIMADJ'', ''G_URPA'', ''G_GROUPADJ'') THEN sub.[JournalSequence] ELSE ''JRNL'' END,
						sub.[Segment01],
						sub.[Segment02],
						sub.[Segment03],
						sub.[Segment04],
