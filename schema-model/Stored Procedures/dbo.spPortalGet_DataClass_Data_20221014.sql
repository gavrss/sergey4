...
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_20221014]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
						...
						BEGIN
							...
							IF @Filter IS NOT NULL
								BEGIN
									...
									@UserID = @UserID, @InstanceID = @InstanceID, @VersionID = @VersionID,
									@DatabaseName = @CallistoDatabase, @DimensionID = @DimensionID, @HierarchyName = @HierarchyName, @Filter = @Filter, 
									@StorageTypeBM_DataClass = @StorageTypeBM_DataClass, @StorageTypeBM = @StorageTypeBM,
									@FilterLevel = @FilterLevel, @FilterType = @FilterType, @LeafLevelFilter = @LeafLevelFilter OUT, @Debug = @DebugSub
							...
							IF @GroupByYN <> 0
								BEGIN
									SELECT
										...
										@SQL_Join64 = @SQL_Join64 + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'LEFT JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_' + @DimensionName + '] [' + @DimensionName + '] ON [' + @DimensionName + '].MemberId = DC.[' + @DimensionName + '_MemberId]',
