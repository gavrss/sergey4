...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_BusinessProcess_Callisto]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_Dim_BusinessProcess_Callisto] @UserID = -10, @InstanceID = 603, @VersionID = 1095, @DebugBM = 1
EXEC [spIU_Dim_BusinessProcess_Callisto] @UserID = -10, @InstanceID = 533, @VersionID = 1058, @DebugBM = 7
EXEC [spIU_Dim_BusinessProcess_Callisto] @UserID = -10, @InstanceID = 574, @VersionID = 1045, @DebugBM=7 --GMN

EXEC [spIU_Dim_BusinessProcess_Callisto] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Insert rows into Callisto BusinessProcess dimension tables.',
...
BEGIN TRY
	...
	SET @Step = 'Fetch members'
		EXEC [spIU_Dim_BusinessProcess_Raw] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @JobID=@JobID, @Debug = @DebugSub

		UPDATE #BusinessProcess_Members 
		SET 
			[Label] = [MemberKey],
	...
	SET @Step = 'Update Description and dimension specific Properties from source system where Synchronized is set to true.'
		SET @SQLStatement = '
			...
			FROM
				[' + @CallistoDatabase + '].[dbo].[S_DS_BusinessProcess] [BusinessProcess] 
				INNER JOIN [#BusinessProcess_Members] Members ON Members.Label COLLATE DATABASE_DEFAULT = [BusinessProcess].LABEL 
			WHERE 
				[BusinessProcess].[Synchronized] <> 0'
	...
	SET @Step = 'Insert new members from source system'
		SET @SQLStatement = '
			INSERT INTO [' + @CallistoDatabase + '].[dbo].[S_DS_BusinessProcess]
				(
				[MemberId],
				[Label],
				[Description],
			...
			WHERE
				NOT EXISTS (SELECT 1 FROM [' + @CallistoDatabase + '].[dbo].[S_DS_BusinessProcess] [BusinessProcess] WHERE Members.Label = [BusinessProcess].Label)'
	...
	SET @Step = 'Update MemberId'
		EXEC [pcINTEGRATOR].[dbo].[spSet_MemberId] @Database = @CallistoDatabase, @Dimension = N'BusinessProcess', @JobID = @JobID, @Debug = @DebugSub
	...
	SET @Step = 'Return rows'
		IF @DebugBM & 1 > 0 EXEC('SELECT [Table] = ''[' + @CallistoDatabase + '].[dbo].[S_DS_BusinessProcess]'', * FROM [' + @CallistoDatabase + '].[dbo].[S_DS_BusinessProcess]')
