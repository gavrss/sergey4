...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_AccountReceivable_Callisto_20241204_nehatest]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_DC_AccountReceivable_Callisto_20241204_nehatest] @UserID=-10, @InstanceID=603, @VersionID=1095, @DebugBM=15

EXEC [spIU_DC_AccountReceivable_Callisto] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Insert new rows into Callisto AccountReceivable FACT table',
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		FROM
			...
			INNER JOIN [pcINTEGRATOR_Data].[dbo].[Model] M ON M.ApplicationID = A.ApplicationID AND M.ModelBM = 512 AND M.SelectYN <> 0
	...
	SET @Step = 'Create temp table'
		...
		(
			...
			[TimeDay] int,
			[AccountReceivable_Value] float DEFAULT 0.0
		)

	SET @Step = 'Insert into temp table'
		EXEC [pcINTEGRATOR].[dbo].[spIU_DC_AccountReceivable_Raw_20241204_nehatest] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @DataClassID=@DataClassID, @JobID=@JobID, @Debug = @DebugSub

		IF @DebugBM & 2 > 0 SELECT TempTable = '#DC_AccountReceivable', * FROM #DC_AccountReceivable ORDER BY [Entity], [Account], [TimeDay]
	...
	SET @Step = 'Set all existing rows that should be inserted to 0'
		SET @SQLStatement = '
			...
			FROM
				' + @CallistoDatabase + '.[dbo].[FACT_AccountReceivable_default_partition] F
	...
	SET @Step = 'Insert new rows'
		SET @SQLStatement = '
			INSERT INTO ' + @CallistoDatabase + '.[dbo].[FACT_AccountReceivable_default_partition]
				(
				[Account_MemberId],
				[Aging_AR_MemberId],
				[BusinessProcess_MemberId],
			...
			SELECT
				...
				[Aging_AR_MemberId] = ISNULL([Aging_AR].[MemberId], -1),
			...
			FROM
				...
				LEFT JOIN ' + @CallistoDatabase + '.[dbo].[S_DS_Aging_AR] [Aging_AR] ON [Aging_AR].[Label] COLLATE DATABASE_DEFAULT = [Raw].[Aging_AR]
				LEFT JOIN ' + @CallistoDatabase + '.[dbo].[S_DS_BusinessProcess] [BusinessProcess] ON [BusinessProcess].[Label] COLLATE DATABASE_DEFAULT = [Raw].[BusinessProcess]
				...
				LEFT JOIN ' + @CallistoDatabase + '.[dbo].[S_DS_Customer] [Customer] ON [Customer].[Label] COLLATE DATABASE_DEFAULT = [Raw].[Customer]
				LEFT JOIN ' + @CallistoDatabase + '.[dbo].[S_DS_Entity] [Entity] ON [Entity].[Label] COLLATE DATABASE_DEFAULT = [Raw].[Entity]
				LEFT JOIN ' + @CallistoDatabase + '.[dbo].[S_DS_InvoiceNo_AR] [InvoiceNo_AR] ON [InvoiceNo_AR].[Label] COLLATE DATABASE_DEFAULT = [Raw].[InvoiceNo_AR]
				LEFT JOIN ' + @CallistoDatabase + '.[dbo].[S_DS_Scenario] [Scenario] ON [Scenario].[Label] COLLATE DATABASE_DEFAULT = [Raw].[Scenario]
	...
	SET @Step = 'Drop the temp table'
		DROP TABLE #DC_AccountReceivable
