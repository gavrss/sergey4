...
SET ANSI_NULLS ON
GO
create PROCEDURE [dbo].[spGet_GLJrnDtl_NotBalance_Sega]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
	...
	SET @Step = 'Create #GLJrnDtl_NotBalance'
		IF OBJECT_ID(N'TempDB.dbo.#GLJrnDtl_NotBalance', N'U') IS NULL
			BEGIN
				...
				CREATE NONCLUSTERED INDEX [IX_NotBalance]
					ON [dbo].#GLJrnDtl_NotBalance
					(
						[SourceID],  -- (= 1184)
						[Company],   -- = 'XLP'
						[BookID],    -- = 'MAIN'
	...
	SET @Step = 'Create #FP_CursorTable'
		...
		SET @SQLStatement = '
			...
			HAVING
				 ...
				 SUM(BookDebitAmount - BookCreditAmount) NOT BETWEEN -0.0049 AND 0.0049
			ORDER BY
				[Company],
				[BookID],
				[FiscalYear],
