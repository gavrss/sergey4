...
CREATE PROCEDURE [dbo].[spIU_Dim_InvoiceNo_AR_Callisto]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
AS
/*
EXEC [spIU_Dim_InvoiceNo_AR_Callisto] @UserID = -10, @InstanceID = 531, @VersionID = 1057, @DebugBM = 15

EXEC [spIU_Dim_InvoiceNo_AR_Callisto] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Insert rows into Callisto InvoiceNo_AR tables from different sources.',
		...
		IF @Version = '2.1.1.2173' SET @Description = 'Changed references to [Model] from BaseModelID = -7 (Financials) to BaseModelID = -4 (Sales) OR BaseModelID = -11 (AccountReceivable).'
		IF @Version = '2.1.1.2177' SET @Description = 'Loop through all SourceDatabases when fetching raw data.'
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		IF @DebugBM & 2 > 0 SELECT [@DimensionID]=@DimensionID, [@DimensionName]=@DimensionName, [@SequenceBMStep]=@SequenceBMStep, [@StaticMemberYN]=@StaticMemberYN, [@MappingTypeID]=@MappingTypeID
	...
	SET @Step = 'Create temp tables'
		CREATE TABLE #InvoiceNo_AR_Members
			(
			[MemberId] BIGINT,
			...
			[Paid] NVARCHAR(1) COLLATE DATABASE_DEFAULT,
			[Paid_Time] NVARCHAR(255) COLLATE DATABASE_DEFAULT,
			[Paid_Time_MemberId] BIGINT,
	...
	SET @Step = 'Create cursor table'
		CREATE TABLE #InvoiceNo_AR_Cursor_table
			(
			SourceTypeID INT,
			SourceDatabase NVARCHAR(100) COLLATE DATABASE_DEFAULT
			)

		INSERT INTO #InvoiceNo_AR_Cursor_table
			(
			SourceTypeID,
			SourceDatabase
			)
		...
		WHERE
			...
			(M.BaseModelID = -4 OR --Sales
			M.BaseModelID = -10) AND --AccountReceivable
			M.SelectYN <> 0

		IF @DebugBM & 2 > 0 SELECT [TempTable] = '#InvoiceNo_AR_Cursor_table', * FROM #InvoiceNo_AR_Cursor_table

	SET @Step = 'InvoiceNo_AR_Cursor'
		IF CURSOR_STATUS('global','InvoiceNo_AR_Cursor') >= -1 DEALLOCATE InvoiceNo_AR_Cursor
		DECLARE InvoiceNo_AR_Cursor CURSOR FOR
			
			SELECT 
				SourceTypeID,
				SourceDatabase
			FROM
				#InvoiceNo_AR_Cursor_table
			ORDER BY
				SourceTypeID,
				SourceDatabase

			OPEN InvoiceNo_AR_Cursor
			FETCH NEXT FROM InvoiceNo_AR_Cursor INTO @SourceTypeID, @SourceDatabase

			WHILE @@FETCH_STATUS = 0
				BEGIN
					SET @Step = 'Fetch members'
						EXEC [spIU_Dim_InvoiceNo_AR_Raw] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @SourceTypeID=@SourceTypeID, @SourceDatabase=@SourceDatabase, @SequenceBMStep=@SequenceBMStep, @StaticMemberYN=@StaticMemberYN, @MappingTypeID=@MappingTypeID, @JobID=@JobID, @Debug=@DebugSub

					FETCH NEXT FROM InvoiceNo_AR_Cursor INTO @SourceTypeID, @SourceDatabase
				END
		CLOSE InvoiceNo_AR_Cursor
		DEALLOCATE InvoiceNo_AR_Cursor

	SET @Step = 'Update #InvoiceNo_AR_Members'
		UPDATE #InvoiceNo_AR_Members 
		SET
			[Label] = [MemberKey], 
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#InvoiceNo_AR_Members', * FROM #InvoiceNo_AR_Members
	...
	SET @Step = 'Update Description and dimension specific Properties from source system where Synchronized is set to true.'
		SET @SQLStatement = '
			...
			SET
				...
				[Customer] = [Members].[Customer], 
				[Customer_MemberId] = ISNULL([Customer].[MemberId], -1),
				[Paid] = [Members].[Paid], 
				[Paid_Time] = [Members].[Paid_Time], 
				[Paid_Time_MemberId] = ISNULL([Paid_Time].[MemberId], -1),
			...
			FROM
				[' + @CallistoDatabase + '].[dbo].[S_DS_' + @DimensionName + '] [InvoiceNo_AR] 
				INNER JOIN [#InvoiceNo_AR_Members] [Members] ON [Members].Label COLLATE DATABASE_DEFAULT = [InvoiceNo_AR].Label				
				LEFT JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_Customer] [Customer] ON [Customer].Label COLLATE DATABASE_DEFAULT = [Members].[Customer]
				LEFT JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_TimeDay] [Paid_Time] ON [Paid_Time].Label COLLATE DATABASE_DEFAULT = [Members].[Paid_Time]
			WHERE 
				[InvoiceNo_AR].[Synchronized] <> 0 AND
				(
					[InvoiceNo_AR].[Description] <> Members.[Description] OR
					[InvoiceNo_AR].[HelpText] <> Members.[HelpText] OR
					[InvoiceNo_AR].[Customer] <> Members.[Customer] OR 
					[InvoiceNo_AR].[Customer_MemberId] <> ISNULL([Customer].[MemberId], -1) OR  
					[InvoiceNo_AR].[Paid] <> Members.[Paid] OR  
					[InvoiceNo_AR].[Paid_Time] <> Members.[Paid_Time] OR  
					[InvoiceNo_AR].[Paid_Time_MemberId] <> ISNULL([Paid_Time].[MemberId], -1) OR
					' + CASE WHEN @MappingTypeID <> 0 THEN 'isnull([InvoiceNo_AR].[MemberKeyBase], '''') <> [Members].[MemberKeyBase] OR' ELSE '' END + '
					' + CASE WHEN @MappingTypeID <> 0 THEN 'isnull([InvoiceNo_AR].[Entity], '''') <> Members.[Entity] OR' ELSE '' END + '
					' + CASE WHEN @MappingTypeID <> 0 THEN 'isnull([InvoiceNo_AR].[Entity_MemberID], '''') <> [Members].[Entity_MemberID] OR' ELSE '' END + '
					' + CASE WHEN @NodeTypeBMYN <> 0 THEN  '[InvoiceNo_AR].[NodeTypeBM] <> [Members].[NodeTypeBM] OR [InvoiceNo_AR].[NodeTypeBM] IS NULL OR' ELSE '' END + '
					[InvoiceNo_AR].[Source] <> Members.[Source]
				)'
	...
	SET @Step = 'Insert new members from source system'
		SET @SQLStatement = '
			...
			SELECT
				...
				[Members].[Source]
			FROM   
				[#InvoiceNo_AR_Members] [Members]
				LEFT JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_Customer] [Customer] ON [Customer].Label COLLATE DATABASE_DEFAULT = [Members].[Customer]
				LEFT JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_TimeDay] [Paid_Time] ON [Paid_Time].Label COLLATE DATABASE_DEFAULT = [Members].[Paid_Time]
			WHERE
				NOT EXISTS (SELECT 1 FROM [' + @CallistoDatabase + '].[dbo].[S_DS_' + @DimensionName + '] [InvoiceNo_AR] WHERE Members.Label = [InvoiceNo_AR].Label)'
	...
	SET @Step = 'Drop temp tables'
		DROP TABLE [#InvoiceNo_AR_Members]
		DROP TABLE [#EB]
