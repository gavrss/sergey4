...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spGet_MultiDimFilter_20240621]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
--Remove will possibly cause problem when using multiple MultiDims in the same dataclass
		...
		IF @DebugBM & 2 > 0 SELECT [@SQL_MultiDimJoin] = @SQL_MultiDimJoin, [@SQL_MultiDimJoin_SubQuery] = @SQL_MultiDimJoin_SubQuery

		SELECT @SQL_MultiDimJoin = REPLACE(@SQL_MultiDimJoin, '#MultiDim', @SQL_MultiDimJoin_SubQuery)
