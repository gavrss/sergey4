...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spRun_Procedure_KeyValuePair]
...
EXEC [spRun_Procedure_KeyValuePair] @JSON='[{"TKey":"UserID","TValue":"31334"},{"TKey":"InstanceID","TValue":"413"},{"TKey":"VersionID","TValue":"1008"}]'
, @ProcedureName='spPortalAdminSet_InstanceLogin'
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'For @SQLStatement assembling: added "N" symbol before each nvarchar value because we lost unicode chars. FDB-2054: allow access for Users with SecurityRoleID = -10 (Global Administrator) and existing in InstanceID = 1 (DSPanel). FDB-2428: Modified query for @ResultTypeBM = 2 to include Users with @SecurityRoleID = -10 (Global Administrator) from InstanceID = 8 (Epicor).'
				...
				IF
					...
					*/
					...
					FROM
						...
						--Add Users with SecurityRoleID = -10 (Global Administrator) and existing in InstanceID = 1 (DSPanel) and 8 (Epicor)
						UNION 
						SELECT 
							SRU.UserID
						FROM 
						...
						WHERE 
							...
							SRU.[UserID] = @UserID AND
							SRU.[SelectYN] <> 0 AND
							SRU.[SecurityRoleID] = -10 --Global Administrator
						) sub
					) < 1
