...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_Journal_P21]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
EXEC [spIU_DC_Journal_P21] @Book=N'GL',@DebugBM=N'15',@Entity_MemberKey=N'2',@FullReloadYN=N'1',
	...
	SET @Step = 'Insert opening balances into temp table #Journal'
		IF @SequenceBM & 2 > 0 AND (SELECT COUNT(1) FROM #FiscalPeriod WHERE FiscalPeriod = 0) > 0 
			...
			BEGIN
				...
				--temp fix for InstanceID=672 (AAHD), 709 (EOHI), -1330 (EOHI2), 795 (SMAI), 731 (BFRN)
				IF (@InstanceID IN (672, 709, -1330, 795, 731))
					BEGIN
