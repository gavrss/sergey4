...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalConvert_DataClass_Filter_New]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
		...
		ELSE IF @ResultTemplate = 'Generic'
			BEGIN
				...
				SET @SQLStatement = '
					...
					WHERE
						[name] = ''spPortalGet_' + @DataClassName + '_Drill_HC_New'''
				...
				IF @CheckExistence <> 0
					BEGIN
						...
						SET @SQLStatement = '[' + @ETLDatabase + '].[dbo].[spPortalGet_' + @DataClassName + '_Drill_HC_New] ' + '@ResultTypeBM = ' + CONVERT(nvarchar(15), @ResultTypeBM)
						EXEC (@SQLStatement)
					END
				ELSE
					BEGIN
