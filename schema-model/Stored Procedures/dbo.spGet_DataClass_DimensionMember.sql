...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spGet_DataClass_DimensionMember]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'FDB-1830: After step: "Leaf level members for ...": added precalculated variables: @CountMembers, @SumMembers. Made statement more simple and 40x faster in "while" condition. FDB-1877: Modified fix for FDB-1830; corrected WHILE statement to "WHILE (@CountMembers > 0 AND @SumMembers <> @SumMemberID)".'
