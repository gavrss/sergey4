...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spGet_DimensionMember_List_Column_sega]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
		...
		IF @NodeTypeBM & 1 > 0
			BEGIN
				...
				ORDER BY
					DP.SortOrder
			END
select [sega] = 'sega1', * from #MultiDimension
	...
	SET @Step = 'Create and fill temp table #Hierarchy'
		...
		IF @NodeTypeBM & 3 > 0
			...
			END
select [sega_temptable] = '#Hierarchy', * from #Hierarchy
...
select [sega_temptable] = '#Property', [@ObjectGuiBehaviorBM] = @ObjectGuiBehaviorBM, [@DimensionID] = @DimensionID, [@NodeTypeBM]=@NodeTypeBM  , * from #Property
