...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_Journal_EpicorERP_sega]
	@UserID int = -10,
	@InstanceID int = 460,
	@VersionID int = 1052,
...
--	@FiscalPeriod int = NULL,
	...
	@OB_ERP_InsertYN bit = 1, -- 0 = do not insert OB_ERP, OB_ADJ not calculated; compute opening balances from last Fiscal Period's closing balance (OB_JRN);  1 or NULL = insert OB_ERP and OB_ADJ may be calculated

	@JobID int = NULL,
	@JobLogID int = NULL,
	@Rows int = NULL,
	@ProcedureID int = 880000981,
...
EXEC [pcINTEGRATOR].[dbo].[spIU_DC_Journal_EpicorERP] @Book='MAIN',@Debug='0',@Entity_MemberKey='AD-001',@FiscalYear='2020',@FullReloadYN='1',@InstanceID='478'
,@JournalTable='[pcETL_ADP].[dbo].[Journal]',@SequenceBM='2',@SourceID='1092',@UserID='-10',@VersionID='1030',@DebugBM=3

EXEC [pcINTEGRATOR].[dbo].[spIU_DC_Journal_EpicorERP] @Book='Main',@DebugBM='3',@Entity_MemberKey='01', @FullReloadYN='1',@FiscalYear=2022,
@InstanceID='605',@JournalTable='[pcETL_SAMI].[dbo].[Journal]',@SequenceBM='2',@SourceID='1347',@UserID='-10',@VersionID='1096'
...
EXEC [spIU_DC_Journal_EpicorERP_sega] @GetVersion = 1
*/
...
DECLARE
	...
	@Account_RE nvarchar(50),
	@Account_RE_SumYN bit = 0,
	@BalTypeD_ExistsYN bit,
	@SQLSegment_GroupBy nvarchar(max) = '',
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2195' SET @Description = 'Handle Retained Earnings Summarized.'
		IF @Version = '2.1.2.2198' SET @Description = 'DB-1602: For @SequenceBM = 2 (Opening Balances), insert [JournalSequence] = ''OB_ERP'' rows into temp table #Journal for balance accounts only.'
		IF @Version = '2.1.2.2199' SET @Description = 'DB-1610: For @SequenceBM = 2 (Opening Balances), insert [JournalSequence] = ''OB_ERP'' rows for non-zero [GLPeriodBal].[OpenBalance] values only. DB-1620: Added Parameter @OB_ERP_InsertYN and use for @SequenceBM = 2. If set to 0, OB_ERP is not inserted, OB_ADJ will not be calculated; opening balances are calculated based from closing balances from previous Fiscal Period (OB_JRN). FDB-1726: For @SequenceBM = 2 (Opening Balances), revert to inserting [JournalSequence] = ''OB_ERP'' rows for [GLPeriodBal].[OpenBalance] zero values. Fix bug: @Owner instead of hardcodded "Erp"'
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		WHERE
			...
			EPV.[EntityPropertyTypeID] = -10 AND
			EPV.[SelectYN] <> 0
		...
		WHERE
			...
			EPV.[VersionID] = @VersionID AND
			EPV.[EntityID] = @EntityID AND
			EPV.[EntityPropertyTypeID] = -12 AND
			EPV.[SelectYN] <> 0
		...
		SELECT @OB_ERP_InsertYN = ISNULL(@OB_ERP_InsertYN, 1)
		...
		IF @DebugBM & 2 > 0
			SELECT
				...
				[@Account_RE_SumYN] = @Account_RE_SumYN,
				[@OB_ERP_InsertYN] = @OB_ERP_InsertYN
		...
		IF OBJECT_ID(N'TempDB.dbo.#GLJrnDtl_NotBalance', N'U') IS NULL
			BEGIN
				...
				IF @LoadGLJrnDtl_NotBalanceYN = 0
					BEGIN
						EXEC [pcINTEGRATOR].[dbo].[spGet_GLJrnDtl_NotBalance_Sega]
							@UserID = @UserID,
							@InstanceID = @InstanceID,
							@VersionID = @VersionID,
							@SourceID  = @SourceID,
...
-- select * into _sege_GLJrnDtl_NotBalance from #GLJrnDtl_NotBalance
	...
	SET @Step = 'Fill temp table #Segment'
		...
		SELECT @SQLSegment_GroupBy = @SQLSegment_GroupBy + SourceCode + ',' FROM #Segment ORDER BY SegmentNo

		IF @DebugBM & 2 > 0 PRINT '@SQLSegment >> ' + @SQLSegment
		IF @DebugBM & 2 > 0 PRINT '@SQLSegment_GroupBy >> ' + @SQLSegment_GroupBy
	...
	SET @Step = 'Insert GL transactions into temp table #GLJrnDtl based on @MaxSourceCounter'
		IF @SequenceBM & 1 > 0
			BEGIN
				SET @SQLStatement = '
					INSERT INTO [dbo].[#GLJrnDtl]  WITH (TABLOCK)
						(
						[Company],
						[BookID],
						[FiscalYear],
		...
		IF  @SequenceBM & 1 > 0 AND (@MaxSourceCounter IS NOT NULL OR (SELECT COUNT(1) FROM #GLJrnDtl_NotBalance WHERE [SourceID] = @SourceID AND [JournalAmount] = 0) > 0)
			BEGIN
				...
				IF @DebugBM & 2 > 0 AND LEN(@SQLStatement) > 4000
					BEGIN
						PRINT 'Length of @SQLStatement more than 4000, see pcINTEGRATOR_Log..wrk_Debug; Insert previously loaded GL transactions into temp table #Journal from Journal table'
						EXEC [pcINTEGRATOR].[dbo].[spSet_wrk_Debug]
							...
							@Comment = 'Insert previously loaded GL transactions into temp table #Journal from Journal table',
...
IF NOT EXISTS (
    ...
    WHERE name = 'CCSI_tempGLJrnDtl' 
    AND object_id = OBJECT_ID('tempdb..#GLJrnDtl')
)
    CREATE CLUSTERED COLUMNSTORE INDEX CCSI_tempGLJrnDtl 
    ON #GLJrnDtl
    WITH (DATA_COMPRESSION = COLUMNSTORE, ONLINE = OFF);

IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_create_index_[CCSI_tempGLJrnDtl]', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
IF NOT EXISTS (SELECT 1 FROM tempdb.sys.indexes WHERE name = 'IX_FiscalPeriod' AND object_id = OBJECT_ID('tempdb..#FiscalPeriod'))
    CREATE INDEX IX_FiscalPeriod ON #FiscalPeriod (FiscalYear, FiscalPeriod);

IF NOT EXISTS (SELECT 1 FROM tempdb.sys.indexes WHERE name = 'IX_BalanceAccount' AND object_id = OBJECT_ID('tempdb..#BalanceAccount'))
    CREATE INDEX IX_BalanceAccount ON #BalanceAccount (Company, COACode, Account);

IF NOT EXISTS (SELECT 1 FROM tempdb.sys.indexes WHERE name = 'IX_Customer' AND object_id = OBJECT_ID('tempdb..#Customer'))
    CREATE INDEX IX_Customer ON #Customer (Company, ARInvoiceNum);

IF NOT EXISTS (SELECT 1 FROM tempdb.sys.indexes WHERE name = 'IX_Journal' AND object_id = OBJECT_ID('tempdb..#Journal'))
    CREATE INDEX IX_Journal ON #Journal (SourceGUID);
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_create_small_indexes', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
...
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_01', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)

						WHILE @@FETCH_STATUS = 0
							BEGIN
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_02', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
								SELECT
									@EntityID = E.EntityID,
									@Currency = EB.Currency
								FROM
...
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_03', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
								...
								WHERE
									...
									JSN.EntityID = @EntityID AND
									JSN.Book = @Book
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_04', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
								...
								IF  @LinkedServer IS NOT NULL EXEC [pcINTEGRATOR].[dbo].[spGet_Connection] @LinkedServer = @LinkedServer
								EXEC (@SQLStatement)
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_05', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
									...
									BEGIN
										...
										IF @Customer_ExistsYN <> 0
											BEGIN
												...
												IF @DebugBM & 2 > 0 
													BEGIN
														SELECT TempTable = '#Invoice', * FROM #Invoice
														INSERT INTO pcINTEGRATOR_Log..tmp_Invoice SELECT * FROM #Invoice
													END
												...
												IF  @LinkedServer IS NOT NULL EXEC [pcINTEGRATOR].[dbo].[spGet_Connection] @LinkedServer = @LinkedServer
												EXEC (@SQLStatement)
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_06', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
												...
												IF @DebugBM & 2 > 0 PRINT @SQLStatement
												EXEC (@SQLStatement)
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_07', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
												IF @DebugBM & 2 > 0 SELECT * FROM #Customer
											END
					...
					FROM ' + @SourceDatabase + '.[' + @Owner + '].[GLJrnHed]'
											...
											IF  @LinkedServer IS NOT NULL EXEC [pcINTEGRATOR].[dbo].[spGet_Connection] @LinkedServer = @LinkedServer
											EXEC (@SQLStatement)
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_071', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
											...
											END
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_072', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)

/*  CREATE NONCLUSTERED COLUMNSTORE INDEX [NCCSI_tempGLJrnDtl_Columnstore_Wide]
ON [dbo].[#GLJrnDtl] 
([Company],[BookID],[FiscalYear],[FiscalPeriod],[JournalCode],[JournalNum],[JournalLine],[FiscalYearSuffix],[COACode],[SegValue1],[SegValue2],[SegValue3],[SegValue4],[SegValue5],[JEDate],[PostedDate],[Posted],[PostedBy],[Description],[BookDebitAmount],[BookCreditAmount],[CurrencyCode],[DebitAmount],[CreditAmount],[SourceModule],[VendorNum],[ARInvoiceNum],[APInvoiceNum],[SysRevID],[SysRowID]
)*/

										--Fill temp table #Journal
										SET @SQLStatement = '
					INSERT INTO #Journal  WITH (TABLOCK)
						(
						[JobID],
						[InstanceID],
						[Entity],
...
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_08', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
...
IF @DebugBM & 16 > 0 SELECT [Step] = @Step + '_99', [TimeConsumed] = CONVERT(Time(7), GetDate() - @StartTime)
...
---------------------------------
				IF @DebugBM & 2 > 0 SELECT [@OB_ERP_InsertYN] = @OB_ERP_InsertYN, [@Account_RE_SumYN] = @Account_RE_SumYN

				IF (@OB_ERP_InsertYN <> 0 OR @Account_RE_SumYN = 0)
					BEGIN
						SET @SQLStatement = '
							INSERT INTO #Journal
								(
							...
							FROM
								...
								-- INNER JOIN #BalanceAccount B on B.Company = GLD.Company COLLATE DATABASE_DEFAULT AND B.COACode = GLD.COACode COLLATE DATABASE_DEFAULT AND B.Account = GLD.' + @AccountSourceCode + ' COLLATE DATABASE_DEFAULT AND B.BalanceYN = 1
								INNER JOIN #BalanceAccount B on B.Company = GLD.Company COLLATE DATABASE_DEFAULT AND ' + case when @Owner = 'Erp' then 'B.COACode = GLD.COACode COLLATE DATABASE_DEFAULT AND ' else '' end +  'B.Account = GLD.' + @AccountSourceCode + ' COLLATE DATABASE_DEFAULT AND B.BalanceYN = 1
							...
							WHERE
								GLD.[Company] = ''' + @Entity_MemberKey + ''' AND
								GLD.[BookID] = ''' + @Book + ''' AND
								--GLD.[OpenBalance] <> 0 AND
								GLD.[FiscalPeriod] = 0
								' + CASE WHEN @OB_ERP_InsertYN = 0 AND @Account_RE_SumYN = 0
										THEN + ' AND GLD.SegValue' + CONVERT(nvarchar(15), @AccountSegmentNo) + ' = ''' + @Account_RE + ''''
										ELSE
											CASE WHEN @Account_RE_SumYN <> 0
												THEN + ' AND GLD.SegValue' + CONVERT(nvarchar(15), @AccountSegmentNo) + ' <> ''' + @Account_RE + ''''
												ELSE ''
												END
										END

								--' + CASE WHEN @Account_RE_SumYN <> 0 THEN + ' AND GLD.SegValue' + CONVERT(nvarchar(15), @AccountSegmentNo) + ' <> ''' + @Account_RE + ''''  ELSE '' END
...
----------------------------------------------------------------------
			...
			--Check if BalanceType D exists
			SET @SQLStatement = '
				SELECT
					@InternalVariable1 = COUNT(1)
				FROM
					' + @SourceDatabase + '.[' + @Owner + '].[GLPeriodBal] GLD
					INNER JOIN #FiscalPeriod FP ON FP.[FiscalYear] = GLD.[FiscalYear] AND FP.[FiscalPeriod] = 1
				WHERE
					GLD.[Company] = ''' + @Entity_MemberKey + ''' AND
					GLD.[BookID] = ''' + @Book + ''' AND
					GLD.[FiscalPeriod] = 0 AND
					GLD.SegValue' + CONVERT(nvarchar(15), @AccountSegmentNo) + ' = ''' + @Account_RE + ''' AND
					GLD.[BalanceType] = ''D'''

			IF @DebugBM & 2 > 0 PRINT @SQLStatement
			EXEC sp_executesql @SQLStatement, N'@InternalVariable1 int OUT', @InternalVariable1 = @BalTypeD_ExistsYN OUT

			SET @BalTypeD_ExistsYN = ISNULL(@BalTypeD_ExistsYN, 0)
			IF @DebugBM & 2 > 0 SELECT [@BalTypeD_ExistsYN] = @BalTypeD_ExistsYN

			IF @Account_RE_SumYN <> 0
				BEGIN
					SET @SQLStatement = '
						INSERT INTO #Journal
							(
						...
						SELECT
							...
							[Book] = MAX(GLD.BookID),
							[FiscalYear] = GLD.[FiscalYear],
							[FiscalPeriod] = MAX(GLD.[FiscalPeriod]),
							[JournalSequence] = ''OB_ERP'',
							[JournalNo] = ''0'',
							[JournalLine] = 0,
							...
							[TransactionTypeBM] = 4,
							[BalanceYN] = 1,' + @SQLSegment + '
							[JournalDate] = MAX(CONVERT(datetime, CONVERT(nvarchar(6), FP.[YearMonth]) + ''01'', 112)),
							[TransactionDate] = MAX(CONVERT(datetime, CONVERT(nvarchar(6), FP.[YearMonth]) + ''01'', 112)),
							[PostedDate] = MAX(CONVERT(datetime, CONVERT(nvarchar(6), FP.[YearMonth]) + ''01'', 112)),
							[PostedStatus] = 1,
							[PostedBy] = '''',
							[Source] = ''E10'',
							[Scenario] = ''ACTUAL'',
							[Description_Head] = ''Opening balance'',
							[Description_Line] = ' + CASE WHEN @BalAcctDesc_ExistsYN <> 0 THEN 'MAX(GLD.[BalAcctDesc])' ELSE '''''' END + ',
							[Currency_Book] = ''' + @Currency + ''',
							[ValueDebit_Book] = SUM(CASE WHEN GLD.OpenBalance >= 0 THEN GLD.OpenBalance ELSE 0 END),
							[ValueCredit_Book] = SUM(CASE WHEN GLD.OpenBalance < 0 THEN -1 * GLD.OpenBalance ELSE 0 END)
						FROM
							' + @SourceDatabase + '.[' + @Owner + '].[GLPeriodBal] GLD
							INNER JOIN #BalanceAccount B on B.Company = GLD.Company COLLATE DATABASE_DEFAULT AND B.COACode = GLD.COACode COLLATE DATABASE_DEFAULT AND B.Account = GLD.' + @AccountSourceCode + ' COLLATE DATABASE_DEFAULT AND B.BalanceYN = 1
							INNER JOIN #FiscalPeriod FP ON FP.[FiscalYear] = GLD.[FiscalYear] AND FP.[FiscalPeriod] = 1
						WHERE
							...
							--GLD.[OpenBalance] <> 0 AND
							GLD.[FiscalPeriod] = 0 AND
							GLD.[BalanceType] COLLATE DATABASE_DEFAULT IN (''B'', ' + CASE WHEN @BalTypeD_ExistsYN <> 0 THEN '''D''' ELSE '''S''' END + ')
							' + CASE WHEN @Account_RE_SumYN <> 0 THEN + ' AND GLD.SegValue' + CONVERT(nvarchar(15), @AccountSegmentNo) + ' = ''' + @Account_RE + ''''  ELSE '' END + '
						GROUP BY
							' + @SQLSegment_GroupBy + '
							GLD.[FiscalYear]'
...
--						GLBD.[BudgetAmt] <> 0 AND
				...
				SET @SQLStatement = '
					...
					FROM
						...
						INNER JOIN #BalanceAccount B on B.Company = GLBD.Company COLLATE DATABASE_DEFAULT AND B.COACode = GLBD.COACode COLLATE DATABASE_DEFAULT AND B.Account = GLBD.' + @AccountSourceCode + ' COLLATE DATABASE_DEFAULT
					WHERE
						GLBD.[Company] = ''' + @Entity_MemberKey + ''' AND
