...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spGet_MultiDimFilter]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
	...
	@SQL_MultiDimJoin nvarchar(4000) = NULL OUT,
...
EXEC [dbo].[spGet_MultiDimFilter]
	...
	@InstanceID = 568,
	@VersionID = 1077,
	@MultiDimensionID = 5665,
	@MultiDimensionName = 'FullAASI',
	@MultiHierarchyName = 'FullAASI',
	@MultiDimFilter = 'AirMed,AM1LAF,AM2HMA,AM1LAF',
	@LeafLevelFilter = '',
	@EqualityString = 'IN',
	@CategoryYN = 1,
	@JournalYN = 0,
	@CallistoDatabase = 'pcDATA_ACAM',
	@DebugBM = 31
...
EXEC [dbo].[spGet_MultiDimFilter]
	...
	@MultiDimensionName = 'FullAASI',
	@MultiHierarchyName = 'FullAASI',
	@MultiDimFilter = 'AirMed,AM1LAF',
	--@MultiDimFilter = 'AM1LAF',
...
DECLARE
	...
	@MultiDimFilter_csv nvarchar(max) = NULL,
	@MultiDimFilter_MemberId_csv nvarchar(2000) = NULL,
	@MultiDimFilter_Value nvarchar(max) = NULL,
	@SQL_MultiDimJoin_Local nvarchar(4000) = NULL,
	@SQL_MultiDimJoin_SubQuery nvarchar(2000) = NULL,
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Create SQL JOIN clause for MultiDim selection.',
		...
		IF @Version = '2.1.2.2198' SET @Description = 'DB-1455: Handle different Multidim filters in Tuple level.'
		IF @Version = '2.1.2.2199' SET @Description = 'FEA-8707: The SP is called twice if multidim Account and FullAccount. So @SQL_MultiDimJoin is not null for next call. Its problem on REPLACE... Fixed: using @SQL_MultiDimJoin_Local'
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		IF @DebugBM & 2 > 0
			SELECT
				...
				[@SQL_MultiDimJoin_Local] = @SQL_MultiDimJoin_Local
	...
	SET @Step = 'Add needed columns to temp table #MultiDim'
		...
		SET @SQL_MultiDimJoin_Local = 'INNER JOIN #MultiDim MD' + @LoopNo + ' ON MD' + @LoopNo + '.[DimensionName]=''' + @MultiDimensionName + ''' AND'
		SET @SQL_MultiDimJoin_SubQuery = ISNULL(@SQL_MultiDimJoin_SubQuery, '') + 'SELECT DISTINCT [DimensionName],[HierarchyName],[Category_MemberKey],[Leaf_MemberId],[Leaf_MemberKey]'
...
--Remove will possibly cause problem when using multiple MultiDims in the same dataclass
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					-- Get the OBJECT_ID of the temporary table #MultiDim
					DECLARE @TableID INT;
                    SET @TableID = OBJECT_ID('tempdb..#MultiDim');
...
--                                 INNER JOIN tempdb.sys.columns c ON c.[object_id] = t.[object_id]
--                             WHERE
--                                 t.[name] LIKE '#MultiDim%' AND c.[name] = @DimensionName + '_MemberId'
                                SELECT
                                ...
                                WHERE
                                    t.object_id = @TableID
                                and c.name = @DimensionName + '_MemberId'
                            )
					    begin
                            SET @SQLStatement = '
					    ...
					    else IF @DebugBM & 2 > 0 PRINT @DimensionName + ' is already in #MultiDim table, So skip the adding.'
					...
					SELECT
						...
						@SQL_MultiDimJoin_Local = @SQL_MultiDimJoin_Local + CASE WHEN @JournalYN = 0 THEN ' MD' + @LoopNo + '.[' + @DimensionName + '_MemberId] = DC.[' + @DimensionName + '_MemberId] AND' ELSE ' MD' + @LoopNo + '.[' + @DimensionName + '_MemberKey] = J.[' + @JournalColumn + '] AND' END,
						@SQL_MultiDimJoin_SubQuery = @SQL_MultiDimJoin_SubQuery + ',[' + @DimensionName + '_MemberId],[' + @DimensionName + '_MemberKey]'
		...
		IF LEN(@SQL_MultiDimJoin_Local) > 4 SET @SQL_MultiDimJoin_Local = LEFT(@SQL_MultiDimJoin_Local, LEN(@SQL_MultiDimJoin_Local) - 4)

		IF @DebugBM & 2 > 0 SELECT TempTable = '#MultiDim', * FROM #MultiDim ORDER BY DimensionID, Leaf_MemberId

		SET @SQL_MultiDimJoin_SubQuery = '(' + @SQL_MultiDimJoin_SubQuery + ' FROM #MultiDim)'

		IF @DebugBM & 2 > 0 SELECT [@SQL_MultiDimJoin] = @SQL_MultiDimJoin, [@SQL_MultiDimJoin_Local] = @SQL_MultiDimJoin_Local, [@SQL_MultiDimJoin_SubQuery] = @SQL_MultiDimJoin_SubQuery

		SELECT @SQL_MultiDimJoin_Local = REPLACE(@SQL_MultiDimJoin_Local, '#MultiDim', @SQL_MultiDimJoin_SubQuery )
		SELECT @SQL_MultiDimJoin = ISNULL(@SQL_MultiDimJoin, '') + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + @SQL_MultiDimJoin_Local

		IF @DebugBM & 2 > 0 SELECT [@SQL_MultiDimJoin] = @SQL_MultiDimJoin, [@SQL_MultiDimJoin_Local] = @SQL_MultiDimJoin_Local
----
	SET @Step = 'Create temp tables'
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#SourceHierarchy', * FROM #SourceHierarchy

	SET @Step = 'Loop @MultiDimFilter (csv)'
		IF CURSOR_STATUS('global','MultiDimFilter_Cursor') >= -1 DEALLOCATE MultiDimFilter_Cursor
			DECLARE MultiDimFilter_Cursor CURSOR FOR

				SELECT DISTINCT [MultiDimFilter_Value] FROM (SELECT [MultiDimFilter_Value] = value FROM STRING_SPLIT(@MultiDimFilter,',')) sub

				OPEN MultiDimFilter_Cursor
				FETCH NEXT FROM MultiDimFilter_Cursor INTO @MultiDimFilter_Value

				WHILE @@FETCH_STATUS = 0
					BEGIN
						IF @DebugBM & 2 > 0 SELECT [@MultiDimFilter_Value] = @MultiDimFilter_Value

						TRUNCATE TABLE #LeafLevelFilterTable

						SET @Step = 'Create #LeafLevelFilterTable for @MultiDimFilter = ' + @MultiDimFilter_Value
						IF @DebugBM & 2 > 0 SELECT  [@Step] = @Step
							/*
							SELECT @MultiDimFilter_csv = CASE WHEN ISNULL(@MultiDimFilter_csv,'') <> '' THEN @MultiDimFilter_csv + ',' ELSE '' END + '''' + value + '''' FROM STRING_SPLIT(@MultiDimFilter,',')
							IF @DebugBM & 2 > 0 SELECT [@MultiDimFilter] = @MultiDimFilter, [@MultiDimFilter_csv] = @MultiDimFilter_csv

							SET @SQLStatement = '
								SELECT
									@InternalVariable = COALESCE(@InternalVariable + '','' , '''') + CONVERT(NVARCHAR(15), MemberId)
								FROM
									' + @CallistoDatabase + '..S_DS_' + @MultiDimensionName + '
								WHERE
									[Label] IN (' + @MultiDimFilter_csv + ')'

							IF @DebugBM & 2 > 0 PRINT @SQLStatement
							EXEC sp_executesql @SQLStatement, N'@InternalVariable nvarchar(2000) OUT', @InternalVariable = @MultiDimFilter_MemberId_csv OUT

							IF @DebugBM & 2 > 0 SELECT [@MultiDimFilter_MemberId_csv] = @MultiDimFilter_MemberId_csv
							*/
							...
							SET @Step = 'Fill temp table #MultiDim'
								IF @JournalYN <> 0
									SET @SQLStatement = '--QUERY1: IF @JournalYN <> 0
										...
										SELECT DISTINCT
											[MultiDim_Filter] = ''' + @MultiDimFilter_Value + ''',
										...
										WHERE
											...
											NOT EXISTS (SELECT 1 FROM #MultiDim MD WHERE MD.[DimensionID] = ' + CONVERT(nvarchar(15), @MultiDimensionID) + ' AND MD.[Leaf_MemberKey] = M.[Label] AND MD.[MultiDim_Filter] = ''' + @MultiDimFilter_Value + ''')
										ORDER BY
											[Leaf_MemberKey]'

								ELSE IF @CategoryYN <> 0
									SET @SQLStatement = '--QUERY2: IF @CategoryYN <> 0
										INSERT INTO #MultiDim
											(
											[MultiDim_Filter],
											[DimensionID],
...
-- select 'SEGA_02'
			...
			CLOSE MultiDimFilter_Cursor
			DEALLOCATE MultiDimFilter_Cursor
----
	...
	SET @Step = 'Drop temp tables'
		DROP TABLE #LeafLevelFilterTable
		DROP TABLE #SourceHierarchy
