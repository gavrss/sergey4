...
CREATE PROCEDURE [dbo].[spPortalGet_Journal] 
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
	...
	@TransactionTypeBM INT = 27, --1 = Source System; 2 = Calculated; 4 = Information transactions; 8 = Advanced Consolidation; 16 = Manual Transactions/ Entries made in EFP; 32 = Temporary transactions related to TransactionTypeBM 8
...
/*
EXEC [pcINTEGRATOR].[dbo].[spPortalGet_Journal] @Filter = 'FiscalYear=2023|Account=1208|Entity=SM|Book=MainBook',
                        @InstanceID = '603',
                        @ResultTypeBM = '2',
                        @UserID = '11136',
                        @VersionID = '1095'
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2197' SET @Description = '1. Handle InterCompanyEntity 2. Filter out parents for dimensions [MappingTypeID] <> 0'
		IF @Version = '2.1.2.2198' SET @Description = 'Dupplicates MemberKey fix: For leafs and StorageTypeBM & 4 > 0: MemberKey always = D.Label, instead of "MemberKey = CASE WHEN @MappingTypeID = 0 THEN D.Label ELSE D.MemberKeyBase end"'
		IF @Version = '2.1.2.2199' SET @Description = 'Handle [Group] for ResultTypeBM IN (1, 2), Changed default value for @TransactionTypeBM from 19 to 27. Changed ValueDebit_Drill and ValueCredit_Drill for TransactionTypeBM = 8 for @ResultTypeBM & 48 > 0.'
...
BEGIN TRY
	...
	SET @Step = 'Set ValueType'
			...
			IF @ValueType <> 'Group'
				BEGIN
					SELECT @AccountFilter = [Filter] FROM #DimensionFilter WHERE DimensionName = 'Account'
					
					SET @SQLStatement = '
						SELECT @InternalVariable = TimeBalance FROM [' + @CallistoDatabase + '].[dbo].[S_DS_Account] WHERE [Label] = ''' + @AccountFilter + ''''

					IF @DebugBM & 2 > 0 PRINT @SQLStatement
					EXEC sp_executesql @SQLStatement, N'@internalVariable nvarchar(100) OUT', @InternalVariable = @BalanceYN OUT
				END

			IF @DebugBM & 2 > 0 SELECT [@FieldTypeBM] = @FieldTypeBM, [@ValueType] = @ValueType, [@AccountFilter] = @AccountFilter, [@BalanceYN] = @BalanceYN
	...
	SET @Step = 'Set Entity and Book'
		SELECT
			...
			@YtdYN = MAX(CASE WHEN @ValueType = 'Group' THEN 0 ELSE CASE WHEN ([DimensionName] IN ('TimeDataView', 'TimeView') AND [Filter] IN ('YTD', 'FYTD')) OR @BalanceYN <> 0 THEN 1 ELSE 0 END END)
		FROM
			#DimensionFilter
		WHERE
			[DimensionName] IN ('Entity', 'Book', 'FiscalYear', 'FieldTypeBM', 'TimeDataView', 'TimeView')
		...
		IF @Book IS NULL
			...
			--		SELECT @BalanceYN = TimeBalance FROM pcDATA_ADP..Account WHERE [Label] = ''' + @AccountFilter + '''
...
--		SET @DimensionFilter = REPLACE(@DimensionFilter, 'Group IN', 'ConsolidationGroup IN')
		...
		SET @DimensionFilter = REPLACE(@DimensionFilter, 'InterCompanyEntity IN (''NONE'')', 'InterCompanyEntity IS NULL')
			...
			BEGIN
				...
				ELSE
					BEGIN
						...
						FROM
							(
							SELECT ParameterType = 'SingleSelect', ParameterName = 'Group', DataColumn = 'Group',  DataType = 'string', KeyColumn = 'MemberKey', SortOrder = 15
							UNION SELECT ParameterType = 'SingleSelect', ParameterName = 'Entity', DataColumn = 'Entity',  DataType = 'string', KeyColumn = 'MemberKey', SortOrder = 20
							UNION SELECT ParameterType = 'SingleSelect', ParameterName = 'Book', DataColumn = 'Book',  DataType = 'string', KeyColumn = 'MemberKey', SortOrder = 30
							UNION SELECT ParameterType = 'MultiSelect', ParameterName = 'FiscalYear', DataColumn = 'FiscalYear',  DataType = 'int', KeyColumn = 'MemberKey', SortOrder = 70
							) sub
		...
		IF @ResultTypeBM & 2 > 0
			BEGIN
				...
				SET @SQLStatement = '
					...
					SELECT DISTINCT
						[Group] = CASE WHEN [ConsolidationGroup] IS NULL THEN ''NONE'' ELSE [ConsolidationGroup] END,
						[Entity],
						[Book],
						[FiscalYear]
					FROM
							...
							IF @StorageTypeBM & 4 > 0
								BEGIN
									SET @SQLStatement = '
										...
										FROM
											' + CASE WHEN @ParameterName IN ('Group', 'Entity') THEN '#EBFDistinct' ELSE '#JournalDistinct' END + ' JD
...
------------------------------------------------------------------
								...
								BEGIN
									SET @SQLStatement = '
										...
										SELECT DISTINCT' + CASE WHEN @Rows IS NULL THEN '' ELSE ' TOP ' + CONVERT(nvarchar(10), @Rows) END + '
											...
											SortOrder = 1
										FROM
											' + CASE WHEN @ParameterName IN ('Group', 'Entity', 'Book', 'FiscalYear') THEN '#EBFDistinct' ELSE '#JournalDistinct' END + ' JD
										ORDER BY
											[' + @DataColumn + ']'
	...
	SET @Step = '@ResultTypeBM & 16, Data, Line'
		IF @ResultTypeBM & 16 > 0
			BEGIN
				IF @DebugBM & 2 > 0 SELECT [@FieldTypeBM] = @FieldTypeBM, [@ValueType] = @ValueType, [@YtdYN] = @YtdYN
				SET @SQLStatement = '
					SELECT' + CASE WHEN @Rows IS NULL THEN '' ELSE ' TOP ' + CONVERT(nvarchar(10), @Rows) END + '
						[ResultTypeBM] = 16, 
						[InstanceID],
						...
						[Supplier],
						[Description_Line],
						[ValueDebit_Drill] = CASE WHEN J.[TransactionTypeBM] = 8 THEN [ValueDebit_Group] ELSE [ValueDebit_' + @ValueType + '] END,
						[ValueCredit_Drill] = CASE WHEN J.[TransactionTypeBM] = 8 THEN [ValueCredit_Group] ELSE [ValueCredit_' + @ValueType+ '] END,
						[ValueDebit_Book],
						[ValueCredit_Book],
						[ValueDebit_Group],
						[ValueCredit_Group],
		...
		IF @ResultTypeBM & 32 > 0
			BEGIN
				SET @SQLStatement = '
					SELECT
						[ResultTypeBM] = 32, 
						[CheckSum] = SUM(CASE WHEN J.[TransactionTypeBM] = 8 THEN [ValueDebit_Group] ELSE [ValueDebit_' + @ValueType + '] END - CASE WHEN J.[TransactionTypeBM] = 8 THEN [ValueCredit_Group] ELSE [ValueCredit_' + @ValueType+ '] END),
						[RowCount] = COUNT(1)
					FROM
						' + @JournalTable + ' J' + ISNULL(@SQL_MultiDimJoin, '') + ' 
					WHERE
