...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_sega_test_20250617]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
	...
	@RoundDecimalNo INT = 5, --decimal place for ROUND() fxn (default to 5)
...
--  										@SQL_Tuple = @SQL_Tuple + 'DC.[' + FT.[DimensionName] + '_MemberID] ' + FT.[EqualityString] + ' (' + @MultiDim_Leaf_MemberId_csv + ') AND '
							...
							SET @SQL_Tuple = LEFT(@SQL_Tuple, LEN(@SQL_Tuple) -4) + ' THEN ' + @Measure + '_Value ELSE 0 END), ' + CONVERT(NVARCHAR(10), @RoundDecimalNo) + '),'
			...
			BEGIN
				...
				SET @SQLStatement = '
					SELECT
						...
						[' + @Measure + '_Value] = ' + CASE WHEN LEN(@TimeFilterString) > 0 THEN 'ROUND(SUM(CASE WHEN [TimeView].[MemberId] = [Time].[MemberId] THEN DC.[' + @Measure + '_Value] ELSE 0 END), ' + CONVERT(NVARCHAR(10), @RoundDecimalNo) + ')' ELSE 'ROUND(SUM(DC.[' + @Measure + '_Value]), ' + CONVERT(NVARCHAR(10), @RoundDecimalNo) + ')' END + '
					FROM
					...
					' + CASE WHEN LEN(@SQL_GroupBy2) > 0 THEN 'GROUP BY' + @SQL_GroupBy2 + CASE WHEN @RowList_SupressZeroYN <> 0 THEN CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'HAVING
						' + CASE WHEN LEN(@TimeFilterString) > 0 THEN 'ROUND(SUM(CASE WHEN [TimeView].[MemberId] = [Time].[MemberId] THEN DC.[' + @Measure + '_Value] ELSE 0 END), ' + CONVERT(NVARCHAR(10), @RoundDecimalNo) + ')' ELSE 'ROUND(SUM(DC.[' + @Measure + '_Value]), ' + CONVERT(NVARCHAR(10), @RoundDecimalNo) + ')' END + ' <> 0' ELSE '' END ELSE '' END
					...
					BEGIN
						...
						SET @SQLStatement = '
							...
							GROUP BY
								' + @SQL_GroupBy1 + '
							HAVING
								ROUND(SUM(DC.' + @Measure + '_Value), ' + CONVERT(NVARCHAR(10), @RoundDecimalNo) + ') <> 0.0
							OPTION (RECOMPILE)'
...
BEGIN CATCH
	...
	IF @ErrorNumber = 308
		BEGIN
			SET @InfoMessage = @ErrorMessage; -- 'Index is now recreating. Try to run the report in couple of minutes.';
			THROW 51000, @InfoMessage, 2;
		END
