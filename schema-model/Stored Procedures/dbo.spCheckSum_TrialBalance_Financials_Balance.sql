...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spCheckSum_TrialBalance_Financials_Balance] 
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,

	--SP-specific parameters
	@SequenceBM int = 8,	--8 - Journal VS FACT_Financials (Balances);
...
AS
/*
EXEC spCheckSum_TrialBalance_Financials_Balance @UserID = -10, @InstanceID = 589, @VersionID = 1085, @ResultTypeBM = 4, @CheckSumStatusBM = 7, @DebugBM=15

DECLARE @CheckSumValue int,@CheckSumStatus10 int,@CheckSumStatus20 int,@CheckSumStatus30 int,@CheckSumStatus40 int
EXEC [spCheckSum_TrialBalance_Financials_Balance] @Debug=1, @UserID = -10, @InstanceID = 531, @VersionID = 1057, @CheckSumValue = @CheckSumValue OUT, @CheckSumStatus10 = @CheckSumStatus10 OUT, @CheckSumStatus20 = @CheckSumStatus20 OUT, @CheckSumStatus30 = @CheckSumStatus30 OUT, @CheckSumStatus40 = @CheckSumStatus40 OUT, @JobID = 34814
...
EXEC [spCheckSum_TrialBalance_Financials_Balance] @UserID = -10, @InstanceID = 531, @VersionID = 1057, 
@ResultTypeBM = 4, @CheckSumStatusBM = 7, @DebugBM=3

EXEC spRun_Procedure_KeyValuePair
	@ProcedureName = 'spCheckSum_TrialBalance_Financials_Balance',
	@JSON = '
		[
...
EXEC [spCheckSum_TrialBalance_Financials_Balance] @UserID=-10, @InstanceID=531, @VersionID=1057, @DebugBM=3,@SequenceBM=4,@ShowAllYN=0
,@ResultTypeBM=7,@Entity_MemberKey='PCXNW',@Entity_Book='MAIN',@FiscalYear=2021,@FiscalPeriod=1
,@Account='40000'

EXEC [spCheckSum_TrialBalance_Financials_Balance] @UserID=-10, @InstanceID=509, @VersionID=1045, @DebugBM=3
,@ResultTypeBM=7,@ShowAllYN=0
,@FiscalYear=2021

EXEC [spCheckSum_TrialBalance_Financials_Balance] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Compare balance accounts between EFP Journal AND FACT_Financials.',
			@MandatoryParameter = '' --Without @, separated by |

		IF @Version = '2.1.1.2179' SET @Description = 'Procedure created.'
		IF @Version = '2.1.2.2187' SET @Description = 'Modified query for @Step = ''Insert balances from source table Journal''.'
...
*/
		...
		DECLARE EB_Cursor CURSOR FOR
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					SET @Step = 'Insert balances from source table Journal'
						SET @SQLStatement = '
							...
							SELECT
								...
								[SourceTable] = ''Journal_Balances'',
								[Entity] = J.[Entity],
								[Book] = J.[Book],
								[YearMonth] = FP.[YearMonth],
								[Account] = J.[Account],
								[CheckSum] = ROUND(SUM(CASE WHEN J.YearMonth <= FP.YearMonth THEN J.[ValueDebit_Book] - J.[ValueCredit_Book] ELSE 0 END), 4)
							FROM
								' + @ETLDatabase + '.dbo.Journal J
								--INNER JOIN #FiscalPeriod FP ON FP.FiscalYear = J.FiscalYear 
								INNER JOIN (SELECT DISTINCT FiscalYear, YearMonth FROM #FiscalPeriod) FP ON FP.FiscalYear = J.FiscalYear
							WHERE
								J.Entity = ''' + @Entity + ''' AND
								J.Book = ''' + @Book + '''' 
							...
							GROUP BY
								...
								J.[Account]
							HAVING 
								ROUND(SUM(CASE WHEN J.YearMonth <= FP.YearMonth THEN J.[ValueDebit_Book] - J.[ValueCredit_Book] ELSE 0 END), 4) <> 0
							ORDER BY
								J.[Entity],
								J.[Book],
								FP.[YearMonth],
					...
					SET @Step = 'Insert balances from EFP source table FACT_Financials'
						SET @SQLStatement = '
							INSERT INTO #TrialBalance
								(
								[Path],
							...
							SELECT
								...
								[SourceTable] = ''FACT_Financials_View_Balances'',
								[Entity] = V.[Entity],
								[Book] = ''' + @Book + ''',
								[YearMonth] = V.[Time],
								[Account] = V.Account,
							...
							FROM
								...
								INNER JOIN #BalanceAccount BA ON BA.[Account_MemberKey] = V.[Account] AND BA.[BalanceYN] <> 0
							WHERE
								V.Entity = ''' + @Entity + '''
	...
	SET @Step = 'Create temp table #TrialBalance_CheckSum_ByYearMonth'
		SELECT
			...
			[YearMonth],
			[Account],
			[Journal] = ROUND(SUM(CASE WHEN [SourceTable] = 'Journal_Balances' THEN [CheckSum] ELSE 0 END),4),
			[FACT_Financials_View] = ROUND(SUM(CASE WHEN [SourceTable] = 'FACT_Financials_View_Balances' THEN [CheckSum] ELSE 0 END),4)
		INTO
			#TrialBalance_CheckSum_ByYearMonth
		FROM
			#TrialBalance
		WHERE
			[SourceTable] IN ('Journal_Balances', 'FACT_Financials_View_Balances')
		GROUP BY
			[Entity],
			[Book],
			[YearMonth],
	...
	SET @Step = 'Set CheckSumRowLogID'
		...
		SELECT
			...
			[VersionID] = @VersionID,
			[ProcedureID] = @ProcedureID,
			[CheckSumRowKey] = sub.[Entity] + '_' + sub.[Book] + '_' + CONVERT(NVARCHAR(15), sub.[YearMonth]) + '_' + sub.[Account] + '_FACT_Financials_Balances_Diff_' +  CONVERT(NVARCHAR(20), ROUND(sub.[FACT_Financials_View] - sub.[Journal], 2))
		FROM
			#TrialBalance_CheckSum_ByYearMonth sub
		WHERE
			ROUND(sub.[FACT_Financials_View] - sub.[Journal], 4) <> 0 AND
			NOT EXISTS (SELECT 1 FROM [pcINTEGRATOR_Log].[dbo].[CheckSumRowLog] CSRL WHERE CSRL.[CheckSumRowKey] = sub.[Entity] + '_' + sub.[Book] + '_' + CONVERT(NVARCHAR(15), sub.[YearMonth]) + '_' + sub.[Account] + '_FACT_Financials_Balances_Diff_' +  CONVERT(NVARCHAR(20), ROUND(sub.[FACT_Financials_View] - sub.[Journal], 4)) AND CSRL.[ProcedureID] = @ProcedureID AND CSRL.[InstanceID] = @InstanceID AND CSRL.[VersionID] = @VersionID AND CSRL.[CheckSumStatusBM] & 12 = 0)
		ORDER BY 
		...
		WHERE
			...
			CSRL.[CheckSumRowKey] LIKE '%_FACT_Financials_Balances_Diff_%' AND
			NOT EXISTS (SELECT 1 FROM #TrialBalance_CheckSum_ByYearMonth TBC WHERE TBC.InstanceID = CSRL.[InstanceID] AND TBC.VersionID = CSRL.[VersionID] AND TBC.[Entity] + '_' + TBC.[Book] + '_' + CONVERT(NVARCHAR(15), TBC.[YearMonth]) + '_' + TBC.[Account] + '_FACT_Financials_Balances_Diff_' +  CONVERT(NVARCHAR(20), ROUND(TBC.[FACT_Financials_View] - TBC.[Journal], 4)) = CSRL.[CheckSumRowKey])
		OPTION (MAXDOP 1)
		...
		IF @ResultTypeBM & 3 > 0
			BEGIN
				...
				WHERE
					...
					CSRL.[VersionID] = @VersionID AND
					CSRL.[ProcedureID] = @ProcedureID AND
                    CSRL.[CheckSumRowKey] LIKE '%_FACT_Financials_Balances_Diff_%'
				OPTION (MAXDOP 1)

				IF @Debug <> 0 
					SELECT 
						[CheckSumRowKey] = 'FACT_Financials_Balances_Diff',
	...
	SET @Step = 'Get detailed info'
		IF @ResultTypeBM & 4 > 0
			BEGIN
				...
				FROM
					[pcINTEGRATOR_Log].[dbo].[CheckSumRowLog] CSRL 
					INNER JOIN #TrialBalance_CheckSum_ByYearMonth TB ON TB.InstanceID = CSRL.[InstanceID] AND TB.VersionID = CSRL.[VersionID] AND CSRL.[CheckSumRowKey] = TB.[Entity] + '_' + TB.[Book] + '_' + CONVERT(NVARCHAR(15), TB.[YearMonth]) + '_' + TB.[Account] + '_FACT_Financials_Balances_Diff_' +  CONVERT(NVARCHAR(20), ROUND(TB.[FACT_Financials_View] - TB.[Journal], 4))
