...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_Telia]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spRun_Procedure_KeyValuePair] @ProcedureName='spPortalGet_DataClass_Data_Telia', @JSON='[{"TKey" : "DebugBM", "TValue" : "7"},
...
-- 2020
EXEC spRun_Procedure_KeyValuePair @ProcedureName='spPortalGet_DataClass_Data_Telia',@JSON='[{"TKey" : "UserID", "TValue" : "2147"},
...
-- 2021 months
EXEC spRun_Procedure_KeyValuePair @ProcedureName='spPortalGet_DataClass_Data_Telia',@JSON='[{"TKey" : "UserID", "TValue" : "2147"},{"TKey" : "InstanceID", "TValue" : "413"},
...
--CBN
EXEC [spRun_Procedure_KeyValuePair] @ProcedureName='spPortalGet_DataClass_Data_Telia', @JSON='[
...
--SSCycle
EXEC spPortalGet_DataClass_Data_Telia @AssignmentID='14516',@DataClassID='5297',@UserID='8033',@VersionID='1035',
...
--CCM
EXEC spPortalGet_DataClass_Data_Telia @AssignmentID='14542',@DataClassID='5073',@UserID='7707',@VersionID='1021',
...
EXEC spPortalGet_DataClass_Data_Telia @AssignmentID='14994',@DataClassID='5073',@InstanceID='454',@UserID='7702',@VersionID='1021',
@Filter='Scenario=F10X|Entity=All_|Currency=CAD|Account=AC_OPEX|GL_Cost_Center=All_|GL_Product_Category=All_|GL_Project=NONE|GL_Trading_Partner=All_|BusinessProcess=CONSOLIDATED',
...
--SOCAN
EXEC spPortalGet_DataClass_Data_Telia @AssignmentID='14721',@DataClassID='5307',@UserID='8060',@VersionID='1025',
...
EXEC spRun_Procedure_KeyValuePair
	@ProcedureName = 'spPortalGet_DataClass_Data_Telia',
	@JSON = '
		[
...
EXEC [dbo].[spPortalGet_DataClass_Data_Telia] @UserID = -10, @InstanceID = 114, @VersionID = 1004, @DataClassID = 1004, @ResultTypeBM = 3

--AnnJoo SalesBudget
EXEC [dbo].[spPortalGet_DataClass_Data_Telia] 
	@UserID = -10,
	@InstanceID = 114,
	@VersionID = 1004,
	@DataClassID = 1004,
...
EXEC spRun_Procedure_KeyValuePair @ProcedureName='spPortalGet_DataClass_Data_Telia',
...
EXEC spPortalGet_DataClass_Data_Telia 
	@UserID='9392',
	@InstanceID='523',
	@VersionID='1051',
	@DataClassID = 5522,
...
EXEC [spPortalGet_DataClass_Data_Telia] @GetVersion = 1
*/
...
DECLARE
	...
	@SQLMeasureList_4_NoSum nvarchar(max) = '',
	...
	@SQLWhereClause_Callisto nvarchar(max) = '',
	...
	@DimensionTypeID int,
	@TmpGlobalTable varchar(100) = '[##DC_Raw_' + convert(varchar(36),NEWID()) +']',
	@RowNo int,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.1.2169' SET @Description = 'Handle DimensionType AutoMultiDim (27). Include WorkflowState if existing in #DimensionList for ResultTypeBM=8. Enhanced debugging; used SP [spSet_wrk_Debug]. Enhanced Performance.'
		IF @Version = '2.1.1.2170' SET @Description = 'Workflow is no longer mandatory to get DataClassType=Fx editable. Brackets around @Dimension names to handle Dimension Group.'
		IF @Version = '2.1.1.2171' SET @Description = 'Handle syntax error close to ReadAccess. Set correct @DataClassName of LineItem dimension IF @Inserted_Check > 0.'
...
--Tempoarary fix for REM demo 2021-05-18
IF @ResultTypeBM = 204 SET @ResultTypeBM = 12
--Tempoarary fix
...
--Temporary hardcoded
		IF @UseCacheYN = 0 SET @UseCacheYN = 1
	...
	SET @Step = 'Create temp table #GroupBy'
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#GroupBy', * FROM #GroupBy ORDER BY DimensionName

	SET @Step = 'Create temp table #Filter'
		CREATE TABLE #Filter
			(
			[DimensionName] nvarchar(100) COLLATE DATABASE_DEFAULT,
			[EqualityString] nvarchar(10) COLLATE DATABASE_DEFAULT,
			[Filter] nvarchar(100) COLLATE DATABASE_DEFAULT
			)
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#Filter', * FROM #Filter ORDER BY DimensionName
	...
	SET @Step = 'Create temp table #DimensionList'
		...
		SELECT
			...
			[Property] = ISNULL(GB.[PropertyName], 'Label'), 
			...
			[GroupByYN] = CASE WHEN GB.DimensionName IS NULL THEN 0 ELSE 1 END,
			[Filter] = CONVERT(nvarchar(4000), ''),
			[LeafLevelFilter] = CONVERT(nvarchar(4000), ''),
		...
		FROM
			...
			LEFT JOIN #GroupBy GB ON GB.DimensionName = D.DimensionName
		...
		FROM
			...
			SELECT DISTINCT
				D.DimensionID,
				D.DimensionName,
				[Property] = GB.PropertyName
			FROM
				Dimension D
				INNER JOIN #GroupBy GB ON GB.DimensionName = D.DimensionName
			WHERE
			...
			FROM
				Dimension D
				INNER JOIN #Filter F ON F.DimensionName = D.DimensionName
			WHERE
				D.InstanceID IN (0, @InstanceID)
			) sub
		...
		WHERE
			...
			NOT EXISTS (SELECT 1 FROM #DimensionList DL WHERE DL.[DimensionID] = sub.[DimensionID] AND DL.[Property] = sub.[Property])
	...
	SET @Step = 'CREATE TABLE #ReadAccess'
		...
		EXEC [spGet_ReadAccess] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @ActingAs=@ActingAs, @StorageTypeBM_DataClass=4, @ReturnColNameYN=1

		IF @DebugBM & 2 > 0 SELECT TempTable = '#ReadAccess', * FROM #ReadAccess

		IF @DebugBM & 16 > 0 SELECT Step = 10, StartTime = @StartTime, CurrentTime = SYSUTCDATETIME(), [Time] = DATEDIFF(MILLISECOND, @StartTime, SYSUTCDATETIME())

	--SET @Step = 'Update column GroupByYN in temptable #DimensionList'
	--	IF LEN(@GroupBy) > 3
	--		BEGIN
	--			UPDATE DL
	--			SET
	...
	--			FROM
	--				#DimensionList DL
	--				INNER JOIN #GroupBy GB ON GB.[DimensionName] = DL.[DimensionName] AND GB.[PropertyName] = DL.[Property]
	--		END

	--	IF @DebugBM & 2 > 0 SELECT TempTable = '#DimensionList', * FROM #DimensionList ORDER BY SortOrder

	SET @Step = 'Update column [RowNo] in temptable #DimensionList'
		UPDATE DL
		SET
			[RowNo] = sub.[RowNo]
		FROM
			...
			INNER JOIN (
			SELECT
				[RowNo] = ROW_NUMBER() OVER(PARTITION BY DimensionID ORDER BY Property),
				[DimensionID],
				[Property]
			FROM
				#DimensionList
			) sub ON sub.[DimensionID] = DL.[DimensionID] AND sub.[Property] = DL.[Property]
	...
	SET @Step = 'Update column Filter in temptable #DimensionList'
		IF LEN(@Filter) > 3
			BEGIN
				...
				FROM
					#DimensionList DL
					INNER JOIN #Filter F ON F.[DimensionName] = DL.[DimensionName]
				WHERE
					DL.[RowNo] = 1
			END
		...
		IF @DebugBM & 16 > 0 SELECT Step = 20, StartTime = @StartTime, CurrentTime = SYSUTCDATETIME(), [Time] = DATEDIFF(MILLISECOND, @StartTime, SYSUTCDATETIME())
...
--			Parameter = @DataClassName + '_Value',
		...
		FROM
			Measure M
			INNER JOIN DataType DT ON DT.InstanceID IN (0, @InstanceID) AND DT.DataTypeID = M.DataTypeID
		WHERE
		...
		IF @DebugBM & 16 > 0 SELECT Step = 30, StartTime = @StartTime, CurrentTime = SYSUTCDATETIME(), [Time] = DATEDIFF(MILLISECOND, @StartTime, SYSUTCDATETIME())
			...
			BEGIN
				...
				IF LEN(@Hierarchy) > 3
					BEGIN
						...
						FROM
							#HierarchyList HL
							INNER JOIN Dimension D ON D.InstanceID IN (0, @InstanceID) AND D.DimensionName = HL.DimensionName AND D.SelectYN <> 0
				...
				DECLARE DimensionList_Cursor CURSOR FOR
					...
					FETCH NEXT FROM DimensionList_Cursor INTO @DimensionID, @DimensionName, @DimensionTypeID, @HierarchyNo, @HierarchyName, @StorageTypeBM_Dimension, @Filter, @FilterLevel, @LeafLevelFilter_RA, @GroupByYN, @Property, @RowNo, @SortOrder
...
--							SET @GroupByYN = 0
							IF @DebugBM & 2 > 0
								SELECT
									...
									[@StorageTypeBM_Dimension] = @StorageTypeBM_Dimension,
									[@Filter] = @Filter,
							...
							IF @GroupByYN <> 0 OR @DimensionID IN (-7, -6)
								BEGIN
									...
									--		UPDATE #DimensionList SET [GroupByYN] = 1 WHERE DimensionName = @DimensionName
											...
											IF @StorageTypeBM_DataClass & 2 > 0
												BEGIN
													...
													IF @GroupByYN <> 0 SET @SQLDimList = @SQLDimList + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + @DimensionName + '_MemberKey = DC.' + @DimensionName + @DimensionDistinguishing + ','
											...
											ELSE IF @StorageTypeBM_DataClass & 4 > 0
												BEGIN
													IF @DebugBM & 2 > 0 SELECT SQLDimList = '[' + @DimensionName + '_MemberKey] = [' + @DimensionName + '].[' + @Property + ']', SQLGroupBy = '[' + @DimensionName + '].[' + @Property + ']'
													IF @GroupByYN <> 0 
														BEGIN
															SET @SQLDimList = @SQLDimList + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + '[' + @DimensionName + '_' + REPLACE(@Property, 'Label', 'MemberKey') + '] = [' + @DimensionName + '].[' + @Property + '],'
													...
													IF @DimensionTypeID = 27 AND @RowNo = 1
														BEGIN
															SELECT 
																@SQLGroupBy = @SQLGroupBy + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + 'DC.[' + AMD.[DimensionName] + '_MemberId],'
															FROM
																#AutoMultiDim AMD
															WHERE
															...
															ORDER BY
																AMD.[DimensionID]
														END
													ELSE IF @RowNo = 1 AND @GroupByYN <> 0
														BEGIN
															IF @DebugBM & 2 > 0 SELECT [@RowNo] = @RowNo, [@GroupByYN] = @GroupByYN
															SET @SQLGroupBy = @SQLGroupBy + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + 'DC.[' + @DimensionName + '_MemberId],'
														END
												END
--										END
								END
							...
							IF LEN(@Filter) > 0 AND @Filter <> 'All_'
								BEGIN
									IF @DimensionName <> 'TimeDataView'
										BEGIN
											EXEC spGet_LeafLevelFilter @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @DatabaseName=@CallistoDatabase, @DimensionName=@DimensionName, @Filter=@Filter, @StorageTypeBM_DataClass=4, @StorageTypeBM=@StorageTypeBM_Dimension, @HierarchyName=@HierarchyName, @FilterLevel=@FilterLevel, @UseCacheYN=@UseCacheYN, @ReturnColNameYN=1, @LeafLevelFilter=@LeafLevelFilter OUT, @Debug=@DebugSub	
									...
									ELSE
										SET @LeafLevelFilter = ''
									UPDATE #DimensionList SET [LeafLevelFilter] = @LeafLevelFilter WHERE DimensionName = @DimensionName AND Property = @Property
							...
							IF @StorageTypeBM_DataClass & 4 > 0 AND @DimensionName NOT IN ('TimeDataView', 'WorkflowState') AND (LEN(@LeafLevelFilter) > 0 OR LEN(@LeafLevelFilter_RA) > 0 OR @GroupByYN <> 0)
								BEGIN
									IF @DimensionTypeID = 27 AND @GroupByYN <> 0 AND @RowNo = 1
										BEGIN
											SET @SQLJoin_Callisto = @SQLJoin_Callisto + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + 'INNER JOIN ' + @CallistoDatabase + '..S_DS_' + @DimensionName + ' [' + @DimensionName + '] ON ' 
											SELECT 
											...
											ORDER BY
												AMD.[DimensionID]
										END
									ELSE IF @GroupByYN <> 0 AND @RowNo = 1
										BEGIN
											SET @SQLJoin_Callisto = @SQLJoin_Callisto + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + 'INNER JOIN ' + @CallistoDatabase + '..S_DS_' + @DimensionName + ' [' + @DimensionName + '] ON [' + @DimensionName + '].MemberId = DC.[' + @DimensionName + '_MemberId]' 
										END
										
									SET @SQLWhereClause_Callisto = @SQLWhereClause_Callisto + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CASE WHEN LEN(@LeafLevelFilter) > 0 THEN @LeafLevelFilter + ' AND' ELSE '' END + CASE WHEN LEN(@LeafLevelFilter_RA) > 0 THEN ' ' + @LeafLevelFilter_RA + ' AND' ELSE '' END
		...
		IF @DebugBM & 16 > 0 SELECT Step = 50, StartTime = @StartTime, CurrentTime = SYSUTCDATETIME(), [Time] = DATEDIFF(MILLISECOND, @StartTime, SYSUTCDATETIME())
...
--									SET @SQLMeasureList_4 = @SQLMeasureList_4 + CASE WHEN CHARINDEX (@MeasureName, @Measure) > 0 THEN CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + @MeasureName + '_Value = SUM(DC.' + @MeasureName + '_Value),' ELSE '' END
									...
									SET @SQLMeasureList_4_NoSum = @SQLMeasureList_4_NoSum + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + '[' + REPLACE(@MeasureName, '_Detail', '') + '] = DC.[' + @MeasureName + '],'
						...
						SET @SQLSelectList_4 = @SQLDimList + @SQLMeasureList_4_NoSum
						SET @SQLSelectList_4 = SUBSTRING(@SQLSelectList_4, 1, LEN(@SQLSelectList_4) - 1)
	...
	ORDER BY' + @SQLGroupBy
						...
						ELSE
							BEGIN
								SET @SQLMeasureList_4 = LEFT(@SQLMeasureList_4, LEN(@SQLMeasureList_4) -1)
								SET @SQLStatement = '
	SELECT
		' + @SQLGroupBy + ',' + @SQLMeasureList_4 + CASE WHEN @MasterDataClassID IS NULL THEN ',
		' + CASE WHEN (SELECT COUNT(1) FROM #DimensionList WHERE DimensionID = -63) > 0 THEN '[WorkflowStateID] = MAX(CONVERT(int, DC.[WorkFlowState_MemberId])),' ELSE '' END + '
		[Workflow_UpdatedBy_UserID] = 0' ELSE '' END + '
	INTO
		' + @TmpGlobalTable + '
...
--		' + CASE WHEN @DataClassName = 'Financials' THEN 'with (INDEX([NonClusteredColumnStoreIndex-20210322-221431]))' ELSE '' END + '
	WHERE 
		' + @SQLWhereClause_Callisto + '
		' + CASE WHEN @BusinessProcessYN <> 0 THEN 'DC.[BusinessProcess_MemberId] <> 118 AND' ELSE '' END + '	
		1 = 1
	GROUP BY' + @SQLGroupBy + '
	HAVING
		SUM(DC.' + @DataClassName + '_Value) <> 0'

								IF @DebugBM & 2 > 0 AND LEN(@SQLStatement) > 4000 
									BEGIN
										PRINT 'Length of @SQLStatement more than 4000, see pcINTEGRATOR_Log..wrk_Debug; Prepare Temp table for ResultTypeBM = 4'
										EXEC [dbo].[spSet_wrk_Debug]
											...
											@Comment = 'Prepare Temp table for ResultTypeBM = 4', 
...
--					END
		...
		IF @DebugBM & 16 > 0 SELECT Step = 60, StartTime = @StartTime, CurrentTime = SYSUTCDATETIME(), [Time] = DATEDIFF(MILLISECOND, @StartTime, SYSUTCDATETIME())
...
*/
	...
	SET @Step = 'Drop temp tables'
		...
		IF (SELECT COUNT(1) FROM TempDB.sys.tables WHERE [name] = REPLACE(REPLACE(@TmpGlobalTable, '[', ''), ']', '')) > 0
