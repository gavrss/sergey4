...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_sega_20240329]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
	...
	SET @Step = 'Get ReadAccess'
		...
		IF @ResultTypeBM & 812 > 0
			BEGIN
				...
				IF @DebugBM & 2 > 0 SELECT TempTable = '#ReadAccess', [@ActingAs] = @ActingAs, * FROM #ReadAccess -- sega00
...
-- sega00
if @DebugBM & 2 > 0 select [TempTable] = '#WhereTotal', * from #WhereTotal;
...
--  										@SQL_Tuple = @SQL_Tuple + 'DC.[' + FT.[DimensionName] + '_MemberID] ' + FT.[EqualityString] + ' (' + @MultiDim_Leaf_MemberId_csv + ') AND '
										@SQL_Tuple = case when FT.[EqualityString] = 'IN' then
                                                                @SQL_Tuple + '''' + @MultiDim_Leaf_MemberId_csv + ','' LIKE CAST(DC.[' + FT.[DimensionName] + '_MemberID] as NVARCHAR(MAX)) + N'',%'' AND '
                                                            else
	...
	SET @Step = '@ResultTypeBM & 12'
		IF @ResultTypeBM & 12 > 0
			BEGIN
print 'sega009'
-- select * into _Time_sega_20240328 from #Time;
