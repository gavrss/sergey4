...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_Process_CB]
(
	@UserID int = NULL,
	@InstanceID int = NULL,
...
/*
	EXEC dbo.[spPortalGet_Process_CB] @UserID = 1005, @InstanceID = 357, @ApplicationID = 0, @Debug = 1
	EXEC dbo.[spPortalGet_Process_CB] @UserID = 1005, @Debug = 1
	EXEC dbo.[spPortalGet_Process_CB] @GetVersion = 1
*/	
...
IF @UserID IS NULL OR @InstanceID IS NULL OR @ApplicationID IS NULL
	BEGIN
		SET @Message = 'Parameter @UserID, @InstanceID and @ApplicationID must be set'
		SET @Severity = 16
