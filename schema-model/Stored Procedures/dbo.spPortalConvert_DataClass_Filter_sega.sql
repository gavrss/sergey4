...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalConvert_DataClass_Filter_sega]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
*/
	SET @Step = 'Set FiscalYear'
		...
		IF @DebugBM & 2 > 0 SELECT [@FiscalYear0] = @FiscalYear, [@FiscalPeriod] = @FiscalPeriod, [@StartMonth] = @StartMonth, [@Time_MemberId] = @MonthID
		...
		IF @DebugBM & 1 > 0
			BEGIN
			    SELECT [TempTable] = '#DimensionFilter0', * FROM #DimensionFilter
			END
		...
		IF @DebugBM & 1 > 0
			BEGIN
			    SELECT [TempTable] = '#DimensionFilter1', * FROM #DimensionFilter
			END
