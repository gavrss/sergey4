...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalSet_DataClass_Data_Table_New]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
BEGIN TRY
	...
	SET @Step = 'Create and fill #DataClass table'
		...
		DECLARE AddColumn_Cursor CURSOR FOR
			...
			FETCH NEXT FROM AddColumn_Cursor INTO @ObjectName, @ObjectType

			WHILE @@FETCH_STATUS = 0
				BEGIN
					IF @ObjectType = 'Dimension'
						SET @SQLStatement = '
							ALTER TABLE #DataClass ADD [' + @ObjectName + '_MemberKey] nvarchar(100)
							ALTER TABLE #DataClass ADD [' + @ObjectName + '_MemberId] bigint'

					ELSE IF @ObjectType = 'Measure'
						SET @SQLStatement = '
							ALTER TABLE #DataClass ADD [' + @ObjectName + '_Value] float'
		...
		IF @DebugBM & 2 > 0 SELECT TempTable='#DataClass', * FROM #DataClass
	...
	SET @Step = 'DataClass_Data_Cursor'
		DECLARE DataClass_Data_Cursor CURSOR FOR
			...
			FETCH NEXT FROM DataClass_Data_Cursor INTO @RowNo, @Filter, @MeasureValue

			WHILE @@FETCH_STATUS = 0
				BEGIN
					IF @DebugBM & 2 > 0 SELECT [@RowNo] = @RowNo, [@Filter] = @Filter, [@MeasureValue] = @MeasureValue
