...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_EnvironmentLevel_CB]
(
	@UserID int = NULL,
	@InstanceID int = NULL,
	@JobID int = 0,
...
/*
	EXEC dbo.[spPortalGet_EnvironmentLevel_CB] @UserID = 1005, @InstanceID = 357, @Debug = 1
	EXEC dbo.[spPortalGet_EnvironmentLevel_CB] @UserID = 1005, @Debug = 1
	EXEC dbo.[spPortalGet_EnvironmentLevel_CB] @GetVersion = 1
*/	
...
BEGIN TRY
	...
	SET @Step = 'Return Name and description'
		...
		ORDER BY
			CASE WHEN EnvironmentLevelID <= 0 THEN EnvironmentLevelID ELSE -1000 END DESC,
			EnvironmentLevelName
