...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_CustomerCategory_Raw]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_Dim_CustomerCategory_Raw] @UserID = -10, @InstanceID = 454, @VersionID = 1021, @SourceTypeID = 11, @DebugBM = 3, @SourceDatabase = 'DSPSOURCE01.CCM_E10'
EXEC [spIU_Dim_CustomerCategory_Raw] @UserID = -10, @InstanceID = 529, @VersionID = 1001, @SourceTypeID = 3, @DebugBM = 3

EXEC [spIU_Dim_CustomerCategory_Raw] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Get list of CustomerCategories from Source system',
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		IF OBJECT_ID(N'TempDB.dbo.#CustomerCategory_Members', N'U') IS NULL SET @CalledYN = 0
	...
	SET @Step = 'Create temp table #CustomerCategory_Members_Raw'
		CREATE TABLE #CustomerCategory_Members_Raw
			(
			[MemberId] bigint,
	...
	SET @Step = 'Fill temptable #CustomerCategory_Members_Raw, Entity level'
		IF @MappingTypeID IN (1, 2)
			BEGIN
				INSERT INTO #CustomerCategory_Members_Raw
					(
					[MemberKey],
					[Description],
					[NodeTypeBM],
	...
	SET @Step = 'Fill temptable #CustomerCategory_Members_Raw'
		IF @SourceTypeID IN (1, 11) --Epicor ERP (E9, E10)
			BEGIN
				IF @SequenceBMStep & 1 > 0 --CustomerCategory Leafs
					BEGIN
						SET @SQLStatement = '
							INSERT INTO #CustomerCategory_Members_Raw
								(
							...
							SELECT DISTINCT
								[MemberKey] = CG.[GroupCode] COLLATE DATABASE_DEFAULT,
								[Description] = CG.[GroupDesc] COLLATE DATABASE_DEFAULT,
								[NodeTypeBM] = 1,
								...
								BEGIN
									SET @SQLStatement = '
										...
										WHERE
											...
											NOT EXISTS (SELECT 1 FROM #CustomerCategory_Members_Raw CCMR WHERE CCMR.[MemberKey] = [SY24].[SY24002] COLLATE DATABASE_DEFAULT AND CCMR.[Entity] = ''' + @Entity + ''')'
	...
	SET @Step = 'Return data'
		IF @CalledYN = 0
			BEGIN
				SELECT
					TempTable = '#CustomerCategory_Members_Raw',
					*
				FROM
					#CustomerCategory_Members_Raw
				ORDER BY
				...
				DECLARE Account_Members_Cursor CURSOR FOR
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
							...
							INSERT INTO [#CustomerCategory_Members]
								(
								[MemberId],
								[MemberKey],
								[Description],
							...
							FROM
								...
								FROM
									[#CustomerCategory_Members_Raw] [Raw]
							...
							WHERE
								...
								NOT EXISTS (SELECT 1 FROM [#CustomerCategory_Members] AM WHERE AM.[MemberKey] = [MaxRaw].[MemberKey])
							ORDER BY
	...
	SET @Step = 'Drop the temp tables'
		DROP TABLE #CCTable
		DROP TABLE #CustomerCategory_Members_Raw
