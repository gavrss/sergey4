...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_WorkflowState]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_Dim_WorkflowState] @UserID = -10, @InstanceID = -1318, @VersionID = -1256, @DebugBM = 1

EXEC [spIU_Dim_WorkflowState] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Insert rows into WorkflowState dimension table.',
...
BEGIN TRY
	...
	SET @Step = 'Fetch members'
		EXEC [spIU_Dim_WorkflowState_Raw] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @Debug = @DebugSub
		...
		SET @SQLStatement = '
			...
			FROM
				...
				LEFT JOIN [' + @ETLDatabase + '].[dbo].[pcD_Scenario] [Scenario] ON [Scenario].[MemberKey] = [WFS].[Scenario]'
	...
	SET @Step = 'Update Description and dimension specific Properties from source system where Synchronized is set to true.'
		SET @SQLStatement = '
			...
			FROM
				[' + @ETLDatabase + '].[dbo].[pcD_WorkflowState] [WorkflowState] 
				INNER JOIN [#WorkflowState_Members] [Members] ON [Members].[MemberKey] = [WorkflowState].[MemberKey] 
			WHERE 
				[WorkflowState].[Synchronized] <> 0 AND
				(
				[WorkflowState].[Description] <> [Members].[Description] OR
				[WorkflowState].[HelpText] <> [Members].[HelpText] OR
				[WorkflowState].[SendTo_MemberId] <> [Members].[SendTo_MemberId] OR
				[WorkflowState].[Source] <> [Members].[Source]
				)'
	...
	SET @Step = 'Insert new members from source system'
		SET @SQLStatement = '
			SET IDENTITY_INSERT [' + @ETLDatabase + '].[dbo].[pcD_WorkflowState] ON

			INSERT INTO [' + @ETLDatabase + '].[dbo].[pcD_WorkflowState]
				(
				[MemberId],
				[MemberKey],
				[Description],
			...
			WHERE
				...
				NOT EXISTS (SELECT 1 FROM [' + @ETLDatabase + '].[dbo].[pcD_WorkflowState] [WorkflowState] WHERE Members.[MemberID] = [WorkflowState].[MemberID]) AND
				NOT EXISTS (SELECT 1 FROM [' + @ETLDatabase + '].[dbo].[pcD_WorkflowState] [WorkflowState] WHERE Members.[MemberKey] = [WorkflowState].[MemberKey])
			
			SET IDENTITY_INSERT [' + @ETLDatabase + '].[dbo].[pcD_WorkflowState] OFF'
	...
	SET @Step = 'Update default hierarchy'
		SET @SQLStatement = '		
			UPDATE T1 
			SET
				[ParentMemberID] = CASE T1.MemberKey WHEN ''All_'' THEN 0 ELSE [WorkflowState].[MemberID] END
			FROM
				#WorkflowState_Members T1
				LEFT JOIN #WorkflowState_Members T2 ON T2.[MemberKey] = T1.[Parent]
				LEFT JOIN [' + @ETLDatabase + '].[dbo].[pcD_WorkflowState] [WorkflowState] ON [WorkflowState].[MemberKey] = T2.MemberKey'
		...
		SET @SQLStatement = '
			...
			FROM
				[' + @ETLDatabase + '].[dbo].[pcD_WorkflowState] [WorkflowState] 
				INNER JOIN [#WorkflowState_Members] [Members] ON [Members].[MemberKey] = [WorkflowState].[MemberKey] AND [Members].[ParentMemberID] IS NOT NULL
			WHERE 
				[WorkflowState].[Synchronized] <> 0 AND
				[WorkflowState].[ParentMemberID] IS NULL'
	...
	SET @Step = 'Remove all parent members without leaf members as children.'
		SET @SQLStatement = '
			...
			SET
				[ParentMemberID] = NULL
			FROM
				[' + @ETLDatabase + '].[dbo].[pcD_WorkflowState] T1
			WHERE
				T1.[NodeTypeBM] & 2 > 0 AND
				NOT EXISTS (SELECT 1 FROM [' + @ETLDatabase + '].[dbo].[pcD_WorkflowState] T2 WHERE T2.[ParentMemberID] = T1.[MemberID])'
		...
		IF @DebugBM & 1 > 0 EXEC('SELECT [Table] = ''[' + @ETLDatabase + '].[dbo].[pcD_WorkflowState]'', * FROM [' + @ETLDatabase + '].[dbo].[pcD_WorkflowState] ORDER BY SortOrder')
