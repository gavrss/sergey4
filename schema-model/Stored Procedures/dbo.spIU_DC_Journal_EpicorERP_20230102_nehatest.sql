...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_Journal_EpicorERP_20230102_nehatest]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
EXEC [pcINTEGRATOR].[dbo].[spIU_DC_Journal_EpicorERP_20230102_nehatest] @DebugBM='31',@Book='Dummy',@Entity_MemberKey='Dummy',@FullReloadYN='0',@InstanceID='663',@JournalTable='[pcETL_GPTI].[dbo].[Journal]',@MaxSourceCounter='112387619',@SequenceBM='1',@SourceID='1434',@UserID='-10',@VersionID='1127'
...
BEGIN TRY
	...
	SET @Step = 'Fill table #GLJrnDtl_NotBalance'
		...
		IF @DebugBM & 2 > 0 SELECT [TempTable_#GLJrnDtl_NotBalance]='#GLJrnDtl_NotBalance', * FROM #GLJrnDtl_NotBalance ORDER BY Company,FiscalYear,FiscalPeriod,JournalCode,JournalNum
	...
	SET @Step = 'Create and fill temp table #FiscalPeriod'
		...
		IF @DebugBM & 2 > 0 SELECT [TempTable_#FiscalPeriod] = '#FiscalPeriod', * FROM #FiscalPeriod ORDER BY YearMonth, FiscalPeriod
			...
			BEGIN
				...
				IF @DebugBM & 2 > 0
					BEGIN
						...
						SELECT [TempTable_#GLJrnDtl] = '#GLJrnDtl', * FROM #GLJrnDtl ORDER BY Company,FiscalYear,FiscalPeriod,JournalCode,JournalNum
					END
			END
	...
	SET @Step = 'Insert previously loaded GL transactions into temp table #Journal from Journal table'
		IF  @SequenceBM & 1 > 0 AND (@MaxSourceCounter IS NOT NULL OR (SELECT COUNT(1) FROM #GLJrnDtl_NotBalance WHERE [SourceID] = @SourceID AND [JournalAmount] = 0) > 0)
			BEGIN
				...
				IF @DebugBM & 2 > 0 SELECT [TempTable_#DistinctPeriods] = '#DistinctPeriods', * FROM #DistinctPeriods
					...
					DECLARE Journal_Entity_Cursor CURSOR FOR
						...
						WHILE @@FETCH_STATUS = 0
							BEGIN
								...
								IF @DebugBM & 2 > 0 SELECT [TempTable_#FiscalPeriod] = '#FiscalPeriod', * FROM #FiscalPeriod ORDER BY YearMonth
