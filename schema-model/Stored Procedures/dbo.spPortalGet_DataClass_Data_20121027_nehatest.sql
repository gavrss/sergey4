...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_20121027_nehatest]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
AS
/*
EXEC [pcINTEGRATOR].[dbo].[spPortalGet_DataClass_Data_20121027_nehatest] @ActingAs='11324',@AssignmentID='22755',@DataClassID='5795'
,@Filter='Flow=NONE|Time=2024|Scenario=BUDGET|Entity=DLMAC|Currency=MXN|FullAccount=4101-01000-003|BusinessProcess=INPUT|BusinessRule=NONE|Version=NONE'
,@GroupBy='Time|FullAccount',@Hierarchy='FullAccount=3',@InstanceID='596',@Measure='Financials',@ResultTypeBM='8',@RowList='SupressZeroYN=0',@UseCacheYN='true',@UserID='17569',@VersionID='1091',@DebugBM=63
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'DB-1638: '
				...
				DECLARE MeasureList_8_Cursor CURSOR FOR
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
							...
							SET @SQLMeasureList_8 = @SQLMeasureList_8 + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9)+ CHAR(9) + @MeasureName + ' = ' + CASE WHEN @InputAllowedYN = 0 THEN '0,' ELSE + 'CASE WHEN MAX(WS.[WorkflowStateID]) IS NULL ' + CASE WHEN @WorkflowStateYN <> 0 AND LEN(@TimeFilterString) > 0 THEN 'OR MAX([Time].[MemberId]) NOT BETWEEN MAX(ISNULL(WSS.[TimeFrom],-1)) AND MAX(ISNULL(WSS.[TimeTo],-1)) ' ELSE '' END + 'AND ' + CONVERT(nvarchar(15), @DataClassTypeID) + ' NOT IN (-6) THEN 0 ELSE 1 END,' END
