...
SET ANSI_NULLS ON
GO
create PROCEDURE [dbo].[spPortalGet_DataClass_Data_20221026]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2190' SET @Description = 'EA-2983: Exclude DimensionTypeID 27 in @ResultTypeBM = 64.'
...
IF @DebugBM & 2 > 0 SELECT 'Arne5'
	...
	SET @Step = '@ResultTypeBM & 160'
		IF @ResultTypeBM & 160 > 0 --LineItem
			BEGIN
				IF @DebugBM & 2 > 0
					BEGIN
	...
	SET @Step = '@ResultTypeBM & 4'
		IF @ResultTypeBM & 4 > 0
			BEGIN
				...
				SET @SQLStatement = '			
					SELECT
						[ResultTypeBM] = 4,' + @SQL_Select2 + CASE WHEN @ResultTypeBM & 128 > 0 THEN CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '[LineItemYN] = MAX(CASE WHEN LI.[Dummy] IS NOT NULL AND [TimeView].[MemberId] = [Time].[MemberId] THEN 1 ELSE 0 END),' ELSE '' END + CASE WHEN @WorkflowStateYN <> 0 THEN '
					...
					FROM
						...
						INNER JOIN #Time [Time] ON 1 = 1' ELSE '' END + @SQL_Join2 + CASE WHEN @ResultTypeBM & 128 > 0 THEN 'LEFT JOIN ' + @TmpGlobalTable_128 + ' [LI] ON' + LEFT(@SQLJoin_128, LEN(@SQLJoin_128) - 3) ELSE '' END + '
