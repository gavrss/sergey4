...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_New4]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2196' SET @Description = 'Added option RECOMPILE for the queries reading from the DataClass'
...
IF @DebugBM & 2 > 0 SELECT 'Arne3'
							...
							EXEC [dbo].[spGet_MultiDimFilter_New4]
								@UserID = @UserID,
								@InstanceID = @InstanceID,
								@VersionID = @VersionID,
								@MultiDimensionID = @DimensionID,
	...
	SET @Step = '@ResultTypeBM & 768 INTO @TmpGlobalTable'
		IF @ResultTypeBM & 768 > 0
			BEGIN
				...
				--SET @SQLStatement = @SQLStatement + '; WITH RECOMPILE'
				EXEC (@SQLStatement)
