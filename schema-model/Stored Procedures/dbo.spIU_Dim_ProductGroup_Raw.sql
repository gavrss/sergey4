...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_ProductGroup_Raw]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_Dim_ProductGroup_Raw] @UserID = -10, @InstanceID = 531, @VersionID = 1057, @SourceTypeID = 11, @DebugBM = 3
EXEC [spIU_Dim_ProductGroup_Raw] @UserID = -10, @InstanceID = 454, @VersionID = 1021, @SourceTypeID = 11, @DebugBM = 3, @SourceDatabase = 'DSPSOURCE01.CCM_E10'

EXEC [spIU_Dim_ProductGroup_Raw] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Get ProductGroups from Source system',
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		IF OBJECT_ID(N'TempDB.dbo.#ProductGroup_Members', N'U') IS NULL SET @CalledYN = 0
	...
	SET @Step = 'Create temp table #ProductGroup_Members_Raw'
		CREATE TABLE #ProductGroup_Members_Raw
			(
			[MemberId] bigint,
	...
	SET @Step = 'Fill temptable #ProductGroup_Members_Raw, Entity level'
		IF @MappingTypeID IN (1, 2)
			BEGIN
				INSERT INTO #ProductGroup_Members_Raw
					(
					[MemberKey],
					[Description],
					[NodeTypeBM],
	...
	SET @Step = 'Fill temptable #ProductGroup_Members_Raw'
		IF @SourceTypeID IN (1, 11) --Epicor ERP (E9, E10)
			BEGIN
				IF @SequenceBMStep & 1 > 0 --ProductGroup Leafs
					BEGIN
						SET @SQLStatement = '
							INSERT INTO #ProductGroup_Members_Raw
								(
							...
							SELECT DISTINCT
								[MemberId] = NULL,
								[MemberKey] = PG.[ProdCode] COLLATE DATABASE_DEFAULT,
	...
	SET @Step = 'Return data'
		IF @CalledYN = 0
			BEGIN
				SELECT
					TempTable = '#ProductGroup_Members_Raw',
					*
				FROM
					#ProductGroup_Members_Raw
				ORDER BY
				...
				DECLARE Entity_Members_Cursor CURSOR FOR
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
							...
							INSERT INTO [#ProductGroup_Members]
								(
								[MemberId],
								[MemberKey],
								[Description],
							...
							WHERE
								...
								NOT EXISTS (SELECT 1 FROM [#ProductGroup_Members] AM WHERE AM.[MemberKey] = [MaxRaw].[MemberKey])
							ORDER BY
	...
	SET @Step = 'Drop the temp tables'
		DROP TABLE #ProductGroup_Members_Raw
