...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spSetup_Entity_EpicorERP]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spSetup_Entity_EpicorERP] @UserID=-10, @InstanceID=52, @VersionID=1033, @SourceTypeID = 11, @DebugBM=3

EXEC [spSetup_Entity_EpicorERP] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Setup Entity from Epicor ERP',
			@MandatoryParameter = '' --Without @, separated by |

		IF @Version = '2.0.3.2154' SET @Description = 'Procedure created.'
		IF @Version = '2.1.0.2157' SET @Description = 'Handle update.'
		IF @Version = '2.1.0.2161' SET @Description = 'Added filter on @SourceTypeID from [Source]. Check @LinkedServer if NOT NULL before calling [spGet_Connection]. Added GROUP BY Company when inserting into #Company.'
		IF @Version = '2.1.1.2171' SET @Description = 'Disable update of [EntityBook].[Currency].'
		IF @Version = '2.1.2.2199' SET @Description = 'Update to latest SP template. FDB-2285: Trim extra spaces on Currency value. FDB-2906: Automatically enable newly inserted books in [Entity_Book] table'
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		FROM
			...
			INNER JOIN [pcINTEGRATOR_Data].[dbo].[Model] M ON M.[InstanceID] = S.[InstanceID] AND M.[VersionID] = @VersionID AND M.[ModelID] = S.[ModelID] AND M.[BaseModelID] IN (-3, -7)
		WHERE
	...
	SET @Step = 'Create and fill temp table #Company'
		...
		EXEC (@SQLStatement)
		IF @DebugBM & 2 > 0 SELECT [#Company] = '#Company', C.* FROM #Company C
	...
	SET @Step = 'Create and fill temp table #BalanceType'
		...
		IF @DebugBM & 2 > 0 PRINT @SQLStatement
		EXEC (@SQLStatement)
		IF @DebugBM & 2 > 0 SELECT [#BalanceType]='#BalanceType', BT.* FROM #BalanceType BT
	...
	SET @Step = 'Fill Entity_Book'
		...
		SELECT
			...
			[EntityID] = E.[EntityID],
			[Book] = GLB.[BookID],
			[Currency] = LTRIM(RTRIM(UPPER(SUBSTRING(GLB.[CurrencyCode], 1, 3)))),
			[BookTypeBM] = 1 + CASE WHEN GLB.[MainBook] <> 0 THEN 2 ELSE 0 END,
			[COA] = GLB.COACode,
			[BalanceType] = ISNULL(BT.[BalanceType], 'B'),
			[SelectYN] = 1 --GLB.[MainBook]
		FROM
