...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_InvoiceNo_AR_Raw]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_Dim_InvoiceNo_AR_Raw] @UserID = -10, @InstanceID = 231, @VersionID = 1063, @SourceTypeID = 11, @DebugBM = 3, @SourceDatabase='DSPSOURCE01.pcSOURCE_APU'

EXEC [spIU_Dim_InvoiceNo_AR_Raw] @UserID = -10, @InstanceID = 454, @VersionID = 1021, @SourceTypeID = 11, @DebugBM = 3, @SourceDatabase = 'DSPSOURCE01.CCM_E10'

EXEC [spIU_Dim_InvoiceNo_AR_Raw] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Get InvoiceNo_ARs from Source system',
			@MandatoryParameter = 'SourceTypeID' --Without @, separated by |

		IF @Version = '2.1.0.2162' SET @Description = 'Procedure created.'
		IF @Version = '2.1.0.2163' SET @Description = 'Added filter on @StartYear when inserting into temp table #InvoiceNo_AR_Members_Raw.'
		IF @Version = '2.1.1.2169' SET @Description = 'Changed default Priority to 99999999.'
		IF @Version = '2.1.1.2171' SET @Description = 'Modified query in setting @StartYear.'
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		SELECT
			@MappingTypeID_Customer = MappingTypeID
		FROM
			pcINTEGRATOR_Data.dbo.[Dimension_StorageType]
		WHERE
			InstanceID = @InstanceID AND
			VersionID = @VersionID AND
			DimensionID = -9 --Customer

		SELECT DISTINCT
			@SourceID_Sales = S.SourceID,
		...
		WHERE
			...
			M.BaseModelID IN (-4, -10) AND --Sales -4, AccountReceivable -10
			M.SelectYN <> 0
		
		IF OBJECT_ID(N'TempDB.dbo.#InvoiceNo_AR_Members', N'U') IS NULL SET @CalledYN = 0

		IF @DebugBM & 2 > 0 SELECT [@SourceDatabase] = @SourceDatabase, [@DimensionName] = @DimensionName, [@SourceTypeName] = @SourceTypeName, [@SourceTypeBM] = @SourceTypeBM, [@MappingTypeID] = @MappingTypeID, [@CalledYN] = @CalledYN, [@StartYear] = @StartYear
	...
	SET @Step = 'Create temp table #InvoiceNo_AR_Members_Raw'
		CREATE TABLE #InvoiceNo_AR_Members_Raw
			(
			[MemberId] bigint,
			[MemberKey] nvarchar(100) COLLATE DATABASE_DEFAULT,
			[Description] nvarchar(512) COLLATE DATABASE_DEFAULT,
			[HelpText] nvarchar(1024) COLLATE DATABASE_DEFAULT,
			[Customer] nvarchar (255) COLLATE DATABASE_DEFAULT,
			[NodeTypeBM] int,
	...
	SET @Step = 'Fill temptable #InvoiceNo_AR_Members_Raw, Entity level'
		IF @MappingTypeID IN (0, 1, 2) -- 0 shoul be removed later !!!!!!
			BEGIN
				INSERT INTO #InvoiceNo_AR_Members_Raw
					(
					[MemberKey],
					[Description],
					[NodeTypeBM],
	...
	SET @Step = 'Fill temptable #InvoiceNo_AR_Members_Raw'
		IF @SourceTypeID IN (1, 11) --Epicor ERP (E9, E10)
			BEGIN
				...
				--				INNER JOIN [#EB] EB ON EB.Entity = IH.Company COLLATE DATABASE_DEFAULT'
				...
				IF @SequenceBMStep & 2 > 0 --Parent
					BEGIN
						SET @SQLStatement = '
							...
							SELECT DISTINCT
								[MemberId] = NULL,
								[MemberKey] = CASE LEN(IH.[InvoiceNum] / 10000) WHEN 1 THEN ''0'' ELSE '''' END + CONVERT(nvarchar(255), IH.[InvoiceNum] / 10000) COLLATE DATABASE_DEFAULT,
								--[MemberKey] = CASE WHEN ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' IN (0, 1) AND EB.Entity <> '''' THEN EB.Entity + ''_'' ELSE '''' END + CASE LEN(IH.[InvoiceNum] / 10000) WHEN 1 THEN ''0'' ELSE '''' END + CONVERT(nvarchar(255), IH.[InvoiceNum] / 10000) COLLATE DATABASE_DEFAULT + CASE WHEN (SELECT CASE WHEN ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' IN (0, 1) THEN 1 ELSE ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' END) = 2 AND EB.Entity <> '''' THEN ''_'' + EB.Entity ELSE '''' END,
								[Description] = CASE LEN(IH.[InvoiceNum] / 10000) WHEN 1 THEN ''0'' ELSE '''' END + CONVERT(nvarchar(255), IH.[InvoiceNum] / 10000) COLLATE DATABASE_DEFAULT,
								[HelpText] = '''',
								[Customer] = ''NONE'',
								[NodeTypeBM] = 2,
								[Source] = ''ETL'',								
							...
							FROM
								...
								INNER JOIN [#EB] EB ON EB.Entity = IH.Company COLLATE DATABASE_DEFAULT
							WHERE
								YEAR(IH.InvoiceDate) >= ' + CONVERT(NVARCHAR(15), @StartYear)
				...
				IF @SequenceBMStep & 4 > 0 --Parent
					BEGIN
						SET @SQLStatement = '
							...
							SELECT DISTINCT
								[MemberId] = NULL,
								[MemberKey] = CASE LEN(IH.[InvoiceNum] / 100) WHEN 1 THEN ''000'' WHEN 2 THEN ''00'' WHEN 3 THEN ''0'' ELSE '''' END + CONVERT(nvarchar(255), IH.[InvoiceNum] / 100) COLLATE DATABASE_DEFAULT,
								--[MemberKey] = CASE WHEN ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' IN (0, 1) AND EB.Entity <> '''' THEN EB.Entity + ''_'' ELSE '''' END + CASE LEN(IH.[InvoiceNum] / 100) WHEN 1 THEN ''000'' WHEN 2 THEN ''00'' WHEN 3 THEN ''0'' ELSE '''' END + CONVERT(nvarchar(255), IH.[InvoiceNum] / 100) COLLATE DATABASE_DEFAULT + CASE WHEN (SELECT CASE WHEN ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' IN (0, 1) THEN 1 ELSE ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' END) = 2 AND EB.Entity <> '''' THEN ''_'' + EB.Entity ELSE '''' END,
								[Description] = CASE LEN(IH.[InvoiceNum] / 100) WHEN 1 THEN ''000'' WHEN 2 THEN ''00'' WHEN 3 THEN ''0'' ELSE '''' END + CONVERT(nvarchar(255), IH.[InvoiceNum] / 100) COLLATE DATABASE_DEFAULT,
								[HelpText] = '''',
								[Customer] = ''NONE'',
								[NodeTypeBM] = 2,
								[Source] = ''ETL'',								
								[Parent] = CASE WHEN ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' IN (0, 1) AND EB.Entity <> '''' THEN EB.Entity + ''_'' ELSE '''' END + CASE LEN(IH.[InvoiceNum] / 10000) WHEN 1 THEN ''0'' ELSE '''' END + CONVERT(nvarchar(255), IH.[InvoiceNum] / 10000) COLLATE DATABASE_DEFAULT + CASE WHEN (SELECT CASE WHEN ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' IN (0, 1) THEN 1 ELSE ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' END) = 2 AND EB.Entity <> '''' THEN ''_'' + EB.Entity ELSE '''' END,
								[Priority] = EB.[Priority],
								[Entity] = EB.Entity
							FROM
								' + @SourceDatabase + '.[' + @Owner + '].[InvcHead] IH
				...
				IF @SequenceBMStep & 8 > 0 --Leaf
					BEGIN
						SET @SQLStatement = '
							...
							SELECT DISTINCT
								[MemberId] = NULL,
								[MemberKey] = CASE LEN(IH.[InvoiceNum]) WHEN 1 THEN ''00000'' WHEN 2 THEN ''0000'' WHEN 3 THEN ''000'' WHEN 4 THEN ''00'' WHEN 5 THEN ''0'' ELSE '''' END + CONVERT(nvarchar(255), IH.[InvoiceNum]) COLLATE DATABASE_DEFAULT,
								--[MemberKey] = CASE WHEN ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' IN (0, 1) AND EB.Entity <> '''' THEN EB.Entity + ''_'' ELSE '''' END + CASE LEN(IH.[InvoiceNum]) WHEN 1 THEN ''00000'' WHEN 2 THEN ''0000'' WHEN 3 THEN ''000'' WHEN 4 THEN ''00'' WHEN 5 THEN ''0'' ELSE '''' END + CONVERT(nvarchar(255), IH.[InvoiceNum]) COLLATE DATABASE_DEFAULT + CASE WHEN (SELECT CASE WHEN ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' IN (0, 1) THEN 1 ELSE ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' END) = 2 AND EB.Entity <> '''' THEN ''_'' + EB.Entity ELSE '''' END,
								[Description] = CASE LEN(IH.[InvoiceNum]) WHEN 1 THEN ''00000'' WHEN 2 THEN ''0000'' WHEN 3 THEN ''000'' WHEN 4 THEN ''00'' WHEN 5 THEN ''0'' ELSE '''' END + CONVERT(nvarchar(255), IH.[InvoiceNum]) COLLATE DATABASE_DEFAULT,
								[HelpText] = '''',
								[Customer] = CASE WHEN ' + CONVERT(NVARCHAR(15), @MappingTypeID_Customer) + ' = 1 AND EB.Entity <> '''' THEN EB.Entity + ''_'' ELSE '''' END + ISNULL(CONVERT(nvarchar(255), [C].[CustID]), ''NONE'') COLLATE DATABASE_DEFAULT + CASE WHEN ' + CONVERT(NVARCHAR(15), @MappingTypeID_Customer) + ' = 2 THEN ''_'' + EB.Entity COLLATE DATABASE_DEFAULT ELSE '''' END,
								[NodeTypeBM] = 1,
								[Source] = ''' + @SourceTypeName + ''',
								[Parent] = CASE WHEN ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' IN (0, 1) AND EB.Entity <> '''' THEN EB.Entity + ''_'' ELSE '''' END + CASE LEN(IH.[InvoiceNum] / 100) WHEN 1 THEN ''000'' WHEN 2 THEN ''00'' WHEN 3 THEN ''0'' ELSE '''' END + CONVERT(nvarchar(255), IH.[InvoiceNum] / 100) COLLATE DATABASE_DEFAULT + CASE WHEN (SELECT CASE WHEN ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' IN (0, 1) THEN 1 ELSE ' + CONVERT(NVARCHAR(15), @MappingTypeID) + ' END) = 2 AND EB.Entity <> '''' THEN ''_'' + EB.Entity ELSE '''' END,
								[Priority] = EB.[Priority],
								[Entity] = EB.Entity
							FROM
								' + @SourceDatabase + '.[' + @Owner + '].[InvcHead] [IH]
	...
	SET @Step = 'Return data'
		IF @CalledYN = 0
			BEGIN
				SELECT
					TempTable = '#InvoiceNo_AR_Members_Raw',
					*
				FROM
					#InvoiceNo_AR_Members_Raw
				ORDER BY
				...
				DECLARE Entity_Members_Cursor CURSOR FOR
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
							...
							INSERT INTO [#InvoiceNo_AR_Members]
								(
								[MemberId],
								[MemberKey],
								[Description],
							...
							WHERE
								...
								NOT EXISTS (SELECT 1 FROM [#InvoiceNo_AR_Members] AM WHERE AM.[MemberKey] = [MaxRaw].[MemberKey])
							ORDER BY
	...
	SET @Step = 'Drop the temp tables'
		DROP TABLE #InvoiceNo_AR_Members_Raw
