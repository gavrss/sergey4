...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_Journal_CYNI_New]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
EXEC [spIU_DC_Journal_CYNI_New] @UserID = -10, @InstanceID = 527, @VersionID = 1055, @Entity_MemberKey = 'GGI01', @Book = 'NewBook', @FiscalYear = 2021, @Debug = 1

EXEC [spIU_DC_Journal_CYNI_New] @UserID = -10, @InstanceID = 527, @VersionID = 1055, @Entity_MemberKey = 'GGI01', @Book = 'NewBook', @Debug = 1

EXEC [spIU_DC_Journal_CYNI_New] @UserID = -10, @InstanceID = 527, @VersionID = 1055, @Debug = 1
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		IF @Debug <> 0 SELECT [@FiscalYear] = @FiscalYear, [@CalledFiscalPeriod] = @CalledFiscalPeriod, [@JournalTable] = @JournalTable
	...
	SET @Step = 'Create temp #Entity_Cursor'
		...
		ELSE
			...
			IF @Debug <> 0 SELECT TempTable = '#Entity_Cursor', * FROM #Entity_Cursor ORDER BY Entity_MemberKey, Book, FiscalYear
	...
	SET @Step = 'Entity_Cursor'
		...
		DECLARE Entity_Cursor CURSOR FOR
			...
			OPEN Entity_Cursor
			FETCH NEXT FROM Entity_Cursor INTO @Entity_MemberKey, @Book, @FiscalYear

			WHILE @@FETCH_STATUS = 0
				BEGIN
					IF @Debug <> 0 SELECT [@Entity_MemberKey] = @Entity_MemberKey, [@Book] = @Book, [@FiscalYear] = @FiscalYear
						...
						WHILE @@FETCH_STATUS = 0
							BEGIN
								...
								IF ISNULL(@CYNI_Value, 0.0) <> 0.0
									BEGIN
										UPDATE #FiscalYear_CYNI
										SET
											CYNI_Value = @CYNI_Value
										WHERE
