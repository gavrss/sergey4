...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_20230523_neha]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
EXEC [pcINTEGRATOR].[dbo].[spPortalGet_DataClass_Data_20230523_neha] @DebugBM=15,
@UserID='7881',@InstanceID='596',@VersionID='1091',@DataClassID='5795',@ResultTypeBM='204',@GroupBy='Time|FullAccount',
@Measure='Financials',@AssignmentID=21155,@Filter='Flow=NONE|Time=2023|Scenario=BUDGET|Entity=MAC|Currency=MXN|FullAccount=UAI|BusinessProcess=INPUT|BusinessRule=NONE|Version=NONE',
@Hierarchy='FullAccount=1',@RowList='SupressZeroYN=0',@ActingAs='5892',@UseCacheYN=1
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2196' SET @Description = 'Added option RECOMPILE for the queries reading from the DataClass. Exclude DimensionTypeID 27 when setting @SQLSelect128 and @SQLJoin_128 queries.'
...
BEGIN TRY
	...
	SET @Step = 'Get temp table #MultiDim when needed'
		IF @ResultTypeBM & 812 > 0 AND (SELECT COUNT(1) FROM #FilterTable WHERE [MultiDimIncludedYN] <> 0 AND [Filter] <> 'All_') > 0 
			BEGIN
				SET @MultiDimYN = 1
