...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_Journal_iScala]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
EXEC [spIU_DC_Journal_iScala] @UserID = -10, @InstanceID = 621, @VersionID = 1105, @Entity_MemberKey = 'NC', @Book = 'GL', @FiscalYear = 2023, @SequenceBM = 3, @DebugBM = 3
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2197' SET @Description = 'Handle FiscalYearStartMonth <> 1.'
	...
	SET @Step = 'Insert GL06 transactions into temp table #Journal'
		...
		SET @SQLStatement = @SQLStatement + '
			SELECT
				...
				[JournalSequence] = CASE WHEN FP.[FiscalPeriod] = 0 THEN ''OB_JRN'' ELSE GL06.[GL06038] END,
...
--				INNER JOIN #FiscalPeriod FP ON FP.[FiscalYear] = ' + CONVERT(nvarchar(15), @FiscalYear) + ' AND FP.FiscalPeriod = CASE TTiS.[BusinessProcess] WHEN ''FP0'' THEN 0 WHEN ''FP13'' THEN 13 ELSE MONTH(GL06.[GL06003]) END
