...
CREATE PROCEDURE [dbo].[spBR_BR01_20220510_before_view_to_table]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
EXEC [spBR_BR01_20220510_before_view_to_table] @GetVersion = 1
*/
...
--			EXEC (@PreExec)
	...
	SET @Step = 'Create temp table #PipeStringSplit and related tables'
		...
		CREATE TABLE #FilterTableSmall
			(
			[DimensionName] nvarchar(100) COLLATE DATABASE_DEFAULT,
			[Value1] nvarchar(max) COLLATE DATABASE_DEFAULT
			)
	...
	SET @Step = 'Set @DimensionFilterLeafLevel'
		...
		EXEC pcINTEGRATOR..spGet_FilterTable
			...
			@PipeString = @DimensionFilter,
			@StorageTypeBM_DataClass = 3, --@StorageTypeBM_DataClass,
			@StorageTypeBM = 4, --@StorageTypeBM,
			@Debug = 0

		SELECT
			@DimensionFilterLeafLevel = @DimensionFilterLeafLevel + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'V.[' + DimensionName + '] IN (' + LeafLevelFilter + ') AND'
		FROM
			#FilterTable
		WHERE
			[StepReference] = 'BR01_0'
	...
	SET @Step = 'Create cursor table for creating temp tables for data'
		...
		SELECT 
			...
			[ColumnName] = D.[DimensionName] + '_MemberKey',
		...
		SET
			...
			SortOrder = -10
		WHERE
			ColumnName = @BaseObject + '_MemberKey'
...
--			NOT EXISTS (SELECT 1 FROM BR01_Step D WHERE D.BusinessRuleID = S.BusinessRuleID AND D.BR01_StepPartID = -1 AND D.MemberKey = S.MemberKey AND ISNULL(D.DataClassID, M.DataClassID) = S.DataClassID)
	...
	SET @Step = 'Run Cursor on #BR01_Initial_Cursor_Table'
		...
		DECLARE BR01_Initial_Cursor CURSOR FOR
			...
			FETCH NEXT FROM BR01_Initial_Cursor INTO @BR01_StepID, @InitialStepID, @MemberKey, @ModifierID, @MultiplyWith, @Parameter, @DataClassID, @DataClassName, @DimensionFilterStep
...
--					SET @PipeString = @BaseObject + '=' + @MemberKey + CASE WHEN @DimensionFilterStep IS NOT NULL THEN '|' + @DimensionFilterStep ELSE '' END
					...
					IF @DebugBM & 2 > 0 SELECT [@BR01_StepID] = @BR01_StepID, [@InitialStepID] = @InitialStepID, [@MemberKey] = @MemberKey, [@ModifierID] = @ModifierID, [@Parameter] = @Parameter, [@DataClassID] = @DataClassID, [@PipeString] = @PipeString, [@DimensionFilterStep] = @DimensionFilterStep
					...
					SELECT
						...
						@SQL_Select = @SQL_Select + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '[' + DCL.ColumnName + '] = ' + CASE WHEN DCL.BaseObjectYN <> 0 THEN '''' + @MemberKey + '''' ELSE CASE WHEN DCL.DataType = 'float' THEN 'SUM(V.[' + DCL.ColumnName + ']) * ' + CONVERT(nvarchar(20), @MultiplyWith) + CASE @ModifierID WHEN 10 THEN ' * ' + CONVERT(nvarchar(10), @Parameter) WHEN 20 THEN ' / ' + CONVERT(nvarchar(10), @Parameter) ELSE '' END ELSE CASE WHEN REPLACE(DCL.ColumnName, '_MemberKey', '') = 'Time' THEN 'MV.[MemberKey]' ELSE 'V.[' + REPLACE(DCL.ColumnName, '_MemberKey', '') + ']' END END END + ',',
						@SQL_Select_70 = @SQL_Select_70 + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '[' + DCL.ColumnName + '] = ' + CASE WHEN DCL.BaseObjectYN <> 0 THEN '''' + @MemberKey + '''' ELSE CASE WHEN DCL.DataType = 'float' THEN '(ISNULL(CM.[' + @MasterDataClassName + '_Value], 0.0) - ISNULL(PM.[' + @MasterDataClassName + '_Value], 0.0)) * ' + CONVERT(nvarchar(20), @MultiplyWith) ELSE 'M.[' + REPLACE(DCL.ColumnName, '_MemberKey', '') + ']' END END + ',',
						--@SQL_Select = @SQL_Select + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '[' + DCL.ColumnName + '] = ' + CASE WHEN DCL.BaseObjectYN <> 0 THEN '''' + @MemberKey + '''' ELSE CASE WHEN DCL.DataType = 'float' THEN 'SUM(V.[' + DCL.ColumnName + '])' + CASE @ModifierID WHEN 10 THEN ' * ' + CONVERT(nvarchar(10), @Parameter) WHEN 20 THEN ' / ' + CONVERT(nvarchar(10), @Parameter) ELSE '' END ELSE CASE WHEN REPLACE(DCL.ColumnName, '_MemberKey', '') = 'Time' THEN 'MV.[MemberKey]' ELSE 'V.[' + REPLACE(DCL.ColumnName, '_MemberKey', '') + ']' END END END + ',',
						--@SQL_Select_70 = @SQL_Select_70 + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '[' + DCL.ColumnName + '] = ' + CASE WHEN DCL.BaseObjectYN <> 0 THEN '''' + @MemberKey + '''' ELSE CASE WHEN DCL.DataType = 'float' THEN 'ISNULL(CM.[' + @MasterDataClassName + '_Value], 0.0) - ISNULL(PM.[' + @MasterDataClassName + '_Value], 0.0)' ELSE 'M.[' + REPLACE(DCL.ColumnName, '_MemberKey', '') + ']' END END + ',',
						@SQL_GroupBy = @SQL_GroupBy + CASE WHEN DCL.BaseObjectYN <> 0 OR DCL.DataType = 'float' THEN '' ELSE CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CASE WHEN REPLACE(DCL.ColumnName, '_MemberKey', '') = 'Time' THEN 'MV.[MemberKey],' ELSE 'V.[' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '],' END END,
						@SQL_Select_MV_70 = @SQL_Select_MV_70 + CASE WHEN DCL.BaseObjectYN <> 0 OR DCL.DataType = 'float' THEN '' ELSE CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CASE WHEN REPLACE(DCL.ColumnName, '_MemberKey', '') = 'Time' THEN '[Time] = MV.[MemberKey],' ELSE '[' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '] = V.[' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '],' END END,
						@SQL_Join_CM_70 = @SQL_Join_CM_70 + CASE WHEN DCL.BaseObjectYN <> 0 OR DCL.DataType = 'float' THEN '' ELSE CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'CM.[' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '] = [M].[' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '] AND' END,
						@SQL_Join_PM_70 = @SQL_Join_PM_70 + CASE WHEN DCL.BaseObjectYN <> 0 OR DCL.DataType = 'float' THEN '' ELSE CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'PM.[' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '] = [M].[' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '] AND' END
					FROM
						#DataClassList DCL
					...
					WHERE
						...
						NOT EXISTS (SELECT 1 FROM #DimensionSetting DS WHERE DS.DimensionName = REPLACE(DCL.ColumnName, '_MemberKey', ''))
					ORDER BY
						SortOrder

					if @InstanceID = 454
						begin
						truncate table #FilterTableSmall;
						insert into #FilterTableSmall (DimensionName, Value1)
						select DimensionName, Value
						FROM
							#FilterTable FT   
							INNER JOIN #DataClassList DCL  ON DCL.[DataClassID] = @DataClassID AND DCL.[DimensionID] = FT.[DimensionID]
							CROSS APPLY STRING_SPLIT(replace(FT.[LeafLevelFilter], '''',''), ',');  
						IF @DebugBM & 2 > 0
							SELECT TempTable = '#FilterTableSmall', * FROM #FilterTableSmall ORDER BY 1, 2 
						END

					SELECT
						@SQL_Where = @SQL_Where + 
										case when @InstanceID = 454 then
												CASE 
													WHEN CASE 
																WHEN FT.[LeafLevelFilter] = '' THEN NULL 
																ELSE FT.[LeafLevelFilter] 
																END 
															--IS NOT NULL THEN CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'V.[' + FT.[DimensionName] + '] IN (' + FT.[LeafLevelFilter] + ') AND' 
															IS NOT NULL THEN CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'EXISTS (SELECT TOP 1 1 FROM #FilterTableSmall FTS WHERE FTS.DimensionName = ''' + FT.[DimensionName] + ''' and FTS.Value1 = V.[' + FT.[DimensionName] + ']) AND' 
													ELSE ''
													END
												else
												CASE 
...
--						@SQL_Where = @SQL_Where + CASE WHEN FT.[LeafLevelFilter] IS NOT NULL THEN CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'V.[' + FT.[DimensionName] + '] IN (' + FT.[LeafLevelFilter] + ') AND' ELSE '' END,
						@SQL_Join = @SQL_Join + CASE WHEN FT.[PropertyFilter] IS NOT NULL THEN CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + FT.[PropertyFilter] ELSE '' END
					FROM
					...
					--Deleted  2.1.0.2162 Jawo
					--SELECT
					--	@SQL_Where = @SQL_Where + CASE WHEN LEN(@DimensionFilterLeafLevel) > 0 THEN @DimensionFilterLeafLevel ELSE '' END
						...
						BEGIN
							SET @SQLStatement = '
								...
								FROM
									...
									FROM
										[' + @DataClassDatabase + '].[dbo].[FACT_' + @MasterDataClassName + '_View] V
										INNER JOIN [#Modifier_Value] MV ON MV.ModifierID = 0 AND MV.Parameter = 0 AND MV.DimensionName = ''Time'' AND MV.[MemberKey_Modified] = V.Time
									WHERE' + @SQL_Where + '

									UNION SELECT DISTINCT' + @SQL_Select_MV_70 + '
									FROM
										[' + @DataClassDatabase + '].[dbo].[FACT_' + @MasterDataClassName + '_View] V
										INNER JOIN [#Modifier_Value] MV ON MV.ModifierID = ' + CONVERT(nvarchar(10), @ModifierID) + ' AND MV.Parameter = ' + CONVERT(nvarchar(50), @Parameter) + ' AND MV.DimensionName = ''Time'' AND MV.[MemberKey_Modified] = V.Time
									WHERE' + @SQL_Where + '
									) [M]
									LEFT JOIN (
										SELECT' + @SQL_Select_MV_70 + ',
											[' + @MasterDataClassName + '_Value] = SUM(V.[' + @MasterDataClassName + '_Value])
										FROM
											[' + @DataClassDatabase + '].[dbo].[FACT_' + @MasterDataClassName + '_View] V
											INNER JOIN [#Modifier_Value] MV ON MV.ModifierID = 0 AND MV.Parameter = 0 AND MV.DimensionName = ''Time'' AND MV.[MemberKey_Modified] = V.Time
										WHERE' + @SQL_Where + '
						...
						BEGIN
							...
							SET @SQLStatement = '
								...
								SELECT 
									' + @SQL_Select + '
								FROM
									[' + @DataClassDatabase + '].[dbo].[FACT_' + @DataClassName + '_View] V' + @SQL_Join + '
									INNER JOIN [#Modifier_Value] MV ON MV.ModifierID = ' + CONVERT(NVARCHAR(10), @ModifierID) + ' AND MV.Parameter = ' + CONVERT(NVARCHAR(50), @Parameter) + ' AND MV.DimensionName = ''Time'' AND MV.[MemberKey_Modified] = V.Time
								WHERE' + @SQL_Where + '
								GROUP BY' + @SQL_GroupBy
						END
		...
		IF @DebugBM & 1 > 0 SELECT TempTable = '#BR01_FACT', InitialStepID, Account_MemberKey, COUNT(1) FROM #BR01_FACT GROUP BY InitialStepID, Account_MemberKey ORDER BY InitialStepID, Account_MemberKey
		IF @DebugBM & 2 > 0
			BEGIN
	...
	SET @Step = 'Create temp table #BR01_Destination_Cursor_Table'
		...
		FROM 
			BR01_Step S
			INNER JOIN BR01_Step_SortOrder SSO ON SSO.[InstanceID] = S.InstanceID AND SSO.[VersionID] = S.VersionID AND  SSO.[BusinessRuleID] = S.BusinessRuleID AND SSO.[BR01_StepID] = S.BR01_StepID
			LEFT JOIN #BR01_Initial_Cursor_Table [IS] ON [IS].[MemberKey] = S.[MemberKey] AND ISNULL([IS].[ModifierID], 0) = ISNULL(S.[ModifierID], 0) AND ISNULL([IS].[Parameter], 0) = ISNULL(S.[Parameter], 0) AND ISNULL([IS].[DimensionFilter], '') = ISNULL(S.[DimensionFilter],'')
		WHERE
		...
		IF @DebugBM & 1 > 0 SELECT TempTable = '#BR01_Destination_Cursor_Table', * FROM #BR01_Destination_Cursor_Table ORDER BY [SortOrder]
	...
	SET @Step = 'Run BR01_Destination_Cursor'
		...
		DECLARE BR01_Destination_Cursor CURSOR FOR
			...
			FETCH NEXT FROM BR01_Destination_Cursor INTO @BR01_StepID, @DestinationMemberKey, @ModifierID, @MultiplyWith, @Parameter, @Decimal, @MasterMemberKey, @InitialStepID
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					SELECT
						...
						@SQL_Select = @SQL_Select + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '[' + ColumnName + ']' + CASE WHEN DCL.BaseObjectYN <> 0 THEN ' = ''' +  @DestinationMemberKey + '''' ELSE '' END + ',',
						@SQL_Select_Multiply_Master = @SQL_Select_Multiply_Master + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '[' + ColumnName + '] = ' + CASE WHEN DCL.BaseObjectYN <> 0 THEN '''' +  @DestinationMemberKey + '''' ELSE CASE WHEN DCL.DataType = 'float' THEN 'F.[' + ColumnName + '] @Operator OP.[' + ColumnName + ']' ELSE 'F.[' + ColumnName + ']' END END + ',',
						@SQL_Select_Addition_Master = @SQL_Select_Addition_Master + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '[' + ColumnName + '] = ' + CASE WHEN DCL.BaseObjectYN <> 0 THEN '''' +  @DestinationMemberKey + '''' ELSE CASE WHEN DCL.DataType = 'float' THEN 'ISNULL(F.[' + ColumnName + '], 0) @Operator ISNULL(OP.[' + ColumnName + '], 0)' ELSE 'M.[' + ColumnName + ']' END END + ',',
...
--						@SQL_Join_Multiply = LEFT(@SQL_Join_Multiply, LEN(@SQL_Join_Multiply) - 4),
					...
					SET @SQLStatement = '
						...
						FROM
							#BR01_FACT F
						WHERE
							F.[' + @BaseObject + '_MemberKey] = ''' + @MasterMemberKey + ''' AND
					...
					IF @DebugBM & 2 > 0 
						BEGIN
							...
							SELECT [Cursor] = 'BR01_Destination_Cursor', [@BR01_StepID] = @BR01_StepID, [@SQL_Insert] = @SQL_Insert, [@SQL_Select] = @SQL_Select, [@BaseObject] = @BaseObject, [@DestinationMemberKey] = @DestinationMemberKey, [@Decimal] = @Decimal, [@MasterMemberKey] = @MasterMemberKey, [@InitialStepID] = @InitialStepID
							PRINT '[Cursor] = BR01_Destination_Cursor, [BR01_StepID] = ' + CONVERT(nvarchar(10), @BR01_StepID) + ', [DestinationMemberKey] = ' + @DestinationMemberKey+ ', [Decimal] = ' + CONVERT(nvarchar(10), ISNULL(@Decimal, 0)) + ', [MasterMemberKey] = ' + @MasterMemberKey + ', [InitialStepID] = ' + CONVERT(nvarchar(10), @InitialStepID)
							PRINT @SQLStatement
						END
					...
					WHILE @BR01_StepPartID <= (SELECT MAX(BR01_StepPartID) FROM BR01_Step S WHERE S.BusinessRuleID = @BusinessRuleID AND S.[BR01_StepID] = @BR01_StepID)
						BEGIN
							...
							IF @DebugBM & 2 > 0 PRINT '@BR01_StepPartID = ' + CONVERT(nvarchar(10), @BR01_StepPartID) + ', @MemberKey = ' + @MemberKey + ', @InitialStepID = ' + CONVERT(nvarchar(10), @InitialStepID) + ', @Operator = ' + @Operator
							...
							WHERE
								...
								NOT EXISTS (SELECT 1 FROM #JoinExclusion JE WHERE JE.InitialStepID = @InitialStepID AND JE.DimensionName + '_MemberKey' = DCL.ColumnName)
							ORDER BY
								SortOrder
						...
						FROM
							#BR01_FACT F
						WHERE
							F.[' + @BaseObject + '_MemberKey] = ''' + @DestinationMemberKey + ''''
						...
						DELETE #BR01_FACT
						WHERE
							[' + @BaseObject + '_MemberKey] = ''' + @DestinationMemberKey + ''''
						...
						FROM
							...
							INNER JOIN (
								...
								FROM
									#BR01_FACT
								WHERE
									[' + @BaseObject + '_MemberKey] = ''' + @MemberKey + ''' AND
						...
						FROM
							...
							LEFT JOIN #BR01_FACT OP ON
								OP.[' + @BaseObject + '_MemberKey] = ''' + @MemberKey + ''' AND' + @SQL_Join_Addition2 + ' 
		...
		IF @DebugBM & 1 > 0 EXEC('SELECT TempTable = ''#BR01_FACT'', * FROM #BR01_FACT WHERE [InitialStepID] IS NULL ORDER BY ' + @BaseObject + '_MemberKey, Time_MemberKey')
	...
	SET @Step = 'Delete existing records from FACT table'
		SET @SQLStatement = '
			...
			SELECT
				[MemberId] = D.[MemberId],
				[MemberKey] = F.[' + @BaseObject + '_MemberKey]
			FROM
				[' + @DataClassDatabase + '].[dbo].[S_DS_' + @BaseObject + '] D
				INNER JOIN (SELECT DISTINCT [' + @BaseObject + '_MemberKey] FROM #BR01_FACT WHERE [InitialStepID] IS NULL) F ON F.[' + @BaseObject + '_MemberKey] = D.[Label]'
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#BaseObject_Member', * FROM #BaseObject_Member ORDER BY [MemberKey]

		SELECT
			@SQL_Join_Delete = ''
		SELECT
			@SQL_Join_Delete = @SQL_Join_Delete + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'INNER JOIN [' + @DataClassDatabase + '].[dbo].[S_DS_' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '] [' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '] ON [' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '].[MemberId] = F.[' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '_MemberId] AND [' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '].[Label] COLLATE DATABASE_DEFAULT = ' + '''' + DCL.StaticSetting + ''''
		FROM
			#DataClassList DCL
		WHERE
			DCL.DataClassID = @MasterDataClassID AND
	...
	SET @Step = 'Write back to FACT table'
		...
		SELECT
			@SQL_Insert = @SQL_Insert + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '[' + REPLACE(DCL.ColumnName, '_MemberKey', '_MemberId') + '],',
			@SQL_Select = @SQL_Select + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '[' + REPLACE(DCL.ColumnName, '_MemberKey', '_MemberId') + '] = ISNULL([' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '].[MemberId], -1),',
			@SQL_Join = @SQL_Join + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'LEFT JOIN [' + @DataClassDatabase + '].[dbo].[S_DS_' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '] [' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '] ON [' + REPLACE(DCL.ColumnName, '_MemberKey', '') + '].Label COLLATE DATABASE_DEFAULT = ' + CASE WHEN DCL.StaticSetting IS NULL THEN '[Raw].[' + DCL.ColumnName + ']' ELSE '''' + DCL.StaticSetting + '''' END
		FROM
			#DataClassList DCL
		WHERE
			DCL.DataClassID = @MasterDataClassID AND
