...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spGet_FilterTable_20221227]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
--Previous version
	...
	SET @Step = 'Run cursor to update #FilterTable'
		...
		DECLARE LeafLevelFilter_Cursor CURSOR FOR
			...
			FETCH NEXT FROM LeafLevelFilter_Cursor INTO @TupleNo, @DimensionID, @DimensionTypeID, @StorageTypeBM, @ObjectReference, @HierarchyName, @PropertyName, @EqualityString, @Filter
...
SELECT 'Arne1'
					...
					ELSE IF @PropertyName IS NOT NULL AND CASE WHEN LEN(@Filter) = 0 THEN NULL ELSE @Filter END IS NOT NULL
						BEGIN
SELECT 'Arne2'
							EXEC spGet_PropertyFilter @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @DatabaseName=@DataClassDatabase, @DimensionID=@DimensionID, @PropertyName = @PropertyName, @Filter = @Filter, @StorageTypeBM=4, @EqualityString = @EqualityString, @PropertyFilter = @PropertyFilter OUT, @Debug = @DebugSub

							UPDATE FT
							SET
								--[PropertyFilter] = CASE WHEN @StorageTypeBM_DataClass = 1 THEN REPLACE(REPLACE(@PropertyFilter, '= V.[', '= J.['), FT.[DimensionName], FT.[JournalColumn]) ELSE @PropertyFilter END
