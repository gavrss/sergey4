...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spGet_Entity_FiscalYear]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2191' SET @Description = '1: Hot-fix for PCX2 Honematic: several records in Entity_FiscalYear with defferent count of months. 2: fix begin/end after IF ISNULL(@FiscalPeriodString, '''') <> '''' '
		IF @Version = '2.1.2.2199' SET @Description = 'Filter on @Book when reading from pcETL FiscalCalendar'
