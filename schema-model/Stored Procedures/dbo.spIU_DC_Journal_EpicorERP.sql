...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_Journal_EpicorERP]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
--	@FiscalPeriod int = NULL,
	...
	@BalanceType_B_D_SumYN bit = 0, -- 0 = default setup; 1 = compute/summarize opening balances for both BalanceType B and D for non-RetainedEarnings Accounts (RetainedEarnings Account is handled via [EntityPropertyValue] metadata)
...
AS
/*
EXEC [pcINTEGRATOR].[dbo].[spIU_DC_Journal_EpicorERP] @Book=N'MAIN',@Debug=N'0',@Entity_MemberKey=N'SCCQ',@FiscalYear=N'2025',@FullReloadYN=N'1'
,@InstanceID=N'589',@JournalTable=N'[pcETL_SCCQ].[dbo].[Journal]',@SequenceBM=N'2',@SourceID=N'1305',@UserID=N'-10',@VersionID=N'1085',@DebugBM=15,@BalanceType_B_D_SumYN=1
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'DB-1610: For @SequenceBM = 2 (Opening Balances), insert [JournalSequence] = ''OB_ERP'' rows for non-zero [GLPeriodBal].[OpenBalance] values only. DB-1620: Added Parameter @OB_ERP_InsertYN and use for @SequenceBM = 2. If set to 0, OB_ERP is not inserted, OB_ADJ will not be calculated; opening balances are calculated based from closing balances from previous Fiscal Period (OB_JRN). FDB-1726: For @SequenceBM = 2 (Opening Balances), revert to inserting [JournalSequence] = ''OB_ERP'' rows for [GLPeriodBal].[OpenBalance] zero values. Fix bug: @Owner instead of hardcodded "Erp". Calculate OpeningBalances even if #Journal does not have FP=1. Added @StartFiscalYear filter when insertig into [#GLJrnDtl] and @MaxSourceCounter IS NOT NULL. Updated to latest sp template. Added parameter @BalanceType_B_D_SumYN. FDB-3146: Removed [GLPeriodBal].[COACode] reference, join to [GLBook] and [COA] source tables instead.'
	...
	SET @Step = 'Set procedure variables'
		...
		IF @DebugBM & 2 > 0
			SELECT
				...
				[@BalanceType_B_D_SumYN] = @BalanceType_B_D_SumYN
			...
			BEGIN
				...
				IF @MaxSourceCounter IS NOT NULL
					...
					WHERE
						...
						+ CASE WHEN @StartFiscalYear IS NULL THEN '' ELSE ' AND' + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'GLD.[FiscalYear] >= ' + CONVERT(NVARCHAR(10), @StartFiscalYear) END
					END
...
--	END
	...
	SET @Step = 'Insert opening balances into temp table #Journal'
		...
		IF @SequenceBM & 2 > 0 AND ((SELECT COUNT(1) FROM #FiscalPeriod WHERE FiscalPeriod = 0) > 0 OR @FullReloadYN <> 0)
			BEGIN
					...
					BEGIN
						SET @SQLStatement = '
							...
							FROM
								...
								INNER JOIN ' + @SourceDatabase + '.[' + @Owner + '].[GLBook] GLBk ON GLBk.Company = GLD.Company AND GLBk.BookID = GLD.BookID
								INNER JOIN ' + @SourceDatabase + '.[' + @Owner + '].[COA] COA ON COA.Company = GLBk.Company AND COA.COACode = GLBk.COACode								
								...
								INNER JOIN #BalanceAccount B on B.Company = GLD.Company COLLATE DATABASE_DEFAULT AND ' + case when @Owner = 'Erp' then 'B.COACode = COA.COACode COLLATE DATABASE_DEFAULT AND ' else '' end +  'B.Account = GLD.' + @AccountSourceCode + ' COLLATE DATABASE_DEFAULT AND B.BalanceYN = 1
								INNER JOIN #FiscalPeriod FP ON FP.[FiscalYear] = GLD.[FiscalYear] AND FP.[FiscalPeriod] = 1
								INNER JOIN pcINTEGRATOR_Data..[Entity_Book] EB ON EB.[InstanceID] = ' + CONVERT(nvarchar(15), @InstanceID) + ' AND EB.[VersionID] = ' + CONVERT(nvarchar(15), @VersionID) + ' AND EB.[EntityID] = ' + CONVERT(nvarchar(15), @EntityID) + ' AND EB.[Book] = ''' + @Book + '''' + CASE WHEN @BalanceType_B_D_SumYN <> 0 THEN '' ELSE ' AND EB.[BalanceType] = GLD.[BalanceType] COLLATE DATABASE_DEFAULT' END + '
							...
							WHERE
								' + CASE WHEN @BalanceType_B_D_SumYN <> 0 THEN 'GLD.[BalanceType] IN (''B'',''D'') AND ' ELSE '' END + '
								GLD.[Company] = ''' + @Entity_MemberKey + ''' AND
								GLD.[BookID] = ''' + @Book + ''' AND
				...
				BEGIN
					SET @SQLStatement = '
						...
						FROM
							...
							INNER JOIN ' + @SourceDatabase + '.[' + @Owner + '].[GLBook] GLBk ON GLBk.Company = GLD.Company AND GLBk.BookID = GLD.BookID
							INNER JOIN ' + @SourceDatabase + '.[' + @Owner + '].[COA] COA ON COA.Company = GLBk.Company AND COA.COACode = GLBk.COACode
							INNER JOIN #BalanceAccount B on B.Company = GLD.Company COLLATE DATABASE_DEFAULT AND B.COACode = COA.COACode COLLATE DATABASE_DEFAULT AND B.Account = GLD.' + @AccountSourceCode + ' COLLATE DATABASE_DEFAULT AND B.BalanceYN = 1
							INNER JOIN #FiscalPeriod FP ON FP.[FiscalYear] = GLD.[FiscalYear] AND FP.[FiscalPeriod] = 1
						WHERE
							GLD.[Company] = ''' + @Entity_MemberKey + ''' AND
							GLD.[BookID] = ''' + @Book + ''' AND
