...
CREATE PROCEDURE [dbo].[spIU_Dim_InvoiceNo_AR_Callisto_20230118]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
BEGIN TRY
	...
	SET @Step = 'Check which parent members have leaf members as children.'
		...
		EXEC [pcINTEGRATOR].[dbo].[spSet_LeafCheck] @Database = @CallistoDatabase, @Dimension = @DimensionName, @DimensionTemptable = '#InvoiceNo_AR_Members', @JobID = @JobID, @Debug = @DebugSub
	...
	SET @Step = 'Insert new members into the default hierarchy. Not Synchronized members will not be added. To change the hierarchy, use the Modeler.'
		SET @SQLStatement = '
			...
			FROM
				...
				INNER JOIN [#InvoiceNo_AR_Members] V ON V.Label COLLATE DATABASE_DEFAULT = D1.Label
