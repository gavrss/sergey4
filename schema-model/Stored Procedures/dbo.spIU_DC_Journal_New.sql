...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_Journal_New]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
--			S.SelectYN <> 0
	...
	SET @Step = 'Fill temp table #Entity_Book'
		...
		FROM
			...
			INNER JOIN pcINTEGRATOR_Data..[Entity_Book] EB ON EB.[InstanceID] = E.[InstanceID] AND EB.[VersionID] = E.[VersionID] AND EB.[EntityID] = E.[EntityID] AND BookTypeBM & 1 > 0 AND EB.[SelectYN] <> 0
			INNER JOIN pcINTEGRATOR_Data..[Source] S ON S.[InstanceID] = E.[InstanceID] AND S.[VersionID] = E.[VersionID] AND S.[SourceID] = E.[SourceID] AND S.[SelectYN] <> 0
		WHERE
		...
		IF @FullReloadYN = 0
			BEGIN
				SET @SQLStatement = '
					...
					FROM
						#Entity_Book EB
						INNER JOIN ' + @JournalTable + ' J ON J.[InstanceID] = ' + CONVERT(nvarchar(15), @InstanceID) + ' AND J.[Entity] = EB.[Entity] AND J.[Book] = EB.[Book] AND J.[Scenario] = ''ACTUAL'' AND [TransactionTypeBM] & 1 > 0
					GROUP BY
						EB.[SourceID]'
			...
			BEGIN
				...
				SET @SQLStatement = '
					...
					WHERE
						...
						[MemberId] BETWEEN 1900 AND ' + CONVERT(nvarchar(15), @EndFiscalYear)
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
							...
							SET @JSON = '
								...
								+ CASE WHEN @SourceTypeID IN (1, 11) AND @SourceID IS NOT NULL THEN ',' + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '{"TKey" : "SourceID",  "TValue": "' + CONVERT(NVARCHAR(10), @SourceID) + '"}' ELSE '' END +
							...
							IF @SourceTypeID = 5 --P21
								EXEC [pcINTEGRATOR]..spRun_Procedure_KeyValuePair @ProcedureName = 'spIU_DC_Journal_P21_New3', @JSON = @JSON
