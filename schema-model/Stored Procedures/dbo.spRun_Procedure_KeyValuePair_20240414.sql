...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spRun_Procedure_KeyValuePair_20240414]
...
BEGIN TRY
	...
	SET @Step = 'Set @SQLStatement'
		...
		SELECT @SQLStatement = @SQLStatement + '@' + TKey + '=''' + TValue + ''','
		FROM
			#KeyValuePair
		ORDER BY
			TKey
...
--			SET @SQLStatement = @SQLStatement + @OptParam + ','
	...
	SET @Step = 'Handle logging and parameters'
		...
		ELSE IF (SELECT COUNT(1) FROM #KVP_Param WHERE Parameter = '@Parameter') > 0
			SET @SQLStatement = @SQLStatement + '@Parameter=''' + REPLACE(@Parameter, '''', '''''') + ''','
