...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_20240406_sega]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,

	--SP-specific parameters[Modified_MultiDim_Cursor]
...
--SELECT TempTable = '#GroupBy', * FROM #GroupBy
		IF @DebugBM & 2 > 0 SELECT TempTable = '#FilterTable_1.7.', * FROM #FilterTable WHERE [StepReference] IN (@StepReference, 'RowList') ORDER BY  DimensionName, [TupleNo], [SortOrder]
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#FilterTable_1.8.', * FROM #FilterTable WHERE [StepReference] IN (@StepReference, 'RowList') ORDER BY  DimensionName, [TupleNo], [SortOrder]
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#FilterTable_1.9.', * FROM #FilterTable WHERE [StepReference] IN (@StepReference, 'RowList') ORDER BY  DimensionName, [TupleNo], [SortOrder]
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#FilterTable_2', * FROM #FilterTable WHERE [StepReference] IN (@StepReference, 'RowList') ORDER BY DimensionName, [TupleNo], [SortOrder]
...
-- 				select * into _Sega_Time from #Time;
-- 				select * into _Sega_Hierarchy from #Hierarchy;
-- 				SET @SQLStatement = 'select * into _Sega_Temp1 from ' + @TmpGlobalTable;
-- 				EXEC(@SQLStatement)
