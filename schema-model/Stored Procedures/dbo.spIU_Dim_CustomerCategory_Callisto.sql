...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_CustomerCategory_Callisto]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_Dim_CustomerCategory_Callisto] @UserID = -10, @InstanceID = 454, @VersionID = 1021, @DebugBM = 1
EXEC [spIU_Dim_CustomerCategory_Callisto] @UserID = -10, @InstanceID = 529, @VersionID = 1001, @DebugBM = 1
EXEC [spIU_Dim_CustomerCategory_Callisto] @UserID = -10, @InstanceID = 574, @VersionID = 1045, @DebugBM=7 --GMN
EXEC [spIU_Dim_CustomerCategory_Callisto] @UserID = -10, @InstanceID = 531, @VersionID = 1041, @DebugBM=7 --PCX

EXEC [spIU_Dim_CustomerCategory_Callisto] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Insert rows into Callisto CustomerCategory tables from different sources.',
...
BEGIN TRY
	...
	SET @Step = 'Create temp tables'
		CREATE TABLE #CustomerCategory_Members
			(
			[MemberId] bigint,
	...
	SET @Step = 'Fetch members'
		EXEC [spIU_Dim_CustomerCategory_Raw] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @SourceTypeID=@SourceTypeID, @SourceDatabase=@SourceDatabase, @ETLDatabase=@ETLDatabase, @SequenceBMStep=@SequenceBMStep, @StaticMemberYN=@StaticMemberYN, @MappingTypeID=@MappingTypeID, @JobID=@JobID, @Debug=@DebugSub

		UPDATE #CustomerCategory_Members 
		SET
			[Label] = [MemberKey], 
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#CustomerCategory_Members', * FROM #CustomerCategory_Members
	...
	SET @Step = 'Update Description and dimension specific Properties from source system where Synchronized is set to true.'
		SET @SQLStatement = '
			...
			FROM
				[' + @CallistoDatabase + '].[dbo].[S_DS_' + @DimensionName + '] [CustomerCategory] 
				INNER JOIN [#CustomerCategory_Members] Members ON Members.Label COLLATE DATABASE_DEFAULT = [CustomerCategory].LABEL 
			WHERE 
				[CustomerCategory].[Synchronized] <> 0'
	...
	SET @Step = 'Insert new members from source system'
		SET @SQLStatement = '
			...
			SELECT
				...
				[Synchronized]
			FROM   
				[#CustomerCategory_Members] Members
			WHERE
				NOT EXISTS (SELECT 1 FROM [' + @CallistoDatabase + '].[dbo].[S_DS_' + @DimensionName + '] [CustomerCategory] WHERE Members.Label = [CustomerCategory].Label)'
	...
	SET @Step = 'Drop temp tables'
		DROP TABLE [#CustomerCategory_Members]
		DROP TABLE [#EB]
