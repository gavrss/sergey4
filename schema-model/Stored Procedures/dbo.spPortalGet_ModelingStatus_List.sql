...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_ModelingStatus_List]
...
--EXEC spPortalGet_ModelingStatus_List @UserID = 1005, @InstanceID = 357, @VersionID = 0, @Debug = 1
--EXEC spPortalGet_ModelingStatus_List @UserID = 1005, @InstanceID = 400, @VersionID = 0
--EXEC spPortalGet_ModelingStatus_List @InstanceID = 357
--EXEC spPortalGet_ModelingStatus_List @GetVersion = 1
...
BEGIN TRY
	...
	SET @Step = 'Return rows'
		SELECT DISTINCT
			...
			[ModelingStatusID] = MS.[ModelingStatusID],
			[ModelingStatusName] = MS.[ModelingStatusName],
			[ModelingStatusDescription] = MS.[ModelingStatusDescription]
		FROM
			ModelingStatus MS
		WHERE
			MS.InstanceID = @InstanceID OR MS.InstanceID = 0
		ORDER BY
			MS.[ModelingStatusName]
