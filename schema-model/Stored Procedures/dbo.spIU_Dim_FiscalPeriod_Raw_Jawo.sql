...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_FiscalPeriod_Raw_Jawo]
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		IF OBJECT_ID(@TableName, N'U') IS NULL SET @FiscalCalendarYN = 0
		...
		IF @DebugBM & 2 > 0
			SELECT
				...
				[@ETLDatabase] = @ETLDatabase,
				[@TableName] = @TableName,
				[@FiscalCalendarYN] = @FiscalCalendarYN
	...
	SET @Step = 'Insert rows from FiscalCalendar'
		IF @FiscalCalendarYN <> 0
			BEGIN
				IF @SequenceBMStep & 1 > 0 --Parent
					BEGIN
						...
						SELECT DISTINCT
							...
							[Parent] = 'All_'
						FROM
							[pcETL_EFPA2].[dbo].[FiscalCalendar]
						WHERE
							[Entity] = @Entity AND
							[Book] = @Book
						ORDER BY
	...
	SET @Step = 'Drop the temp tables'
		IF @FiscalCalendarYN = 0 DROP TABLE #FYP
