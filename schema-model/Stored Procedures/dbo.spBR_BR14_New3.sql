...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR14_New3]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
						...
						WHILE @@FETCH_STATUS = 0
							BEGIN
								...
								IF @DimMethod = 'Property' AND @PropertyLen = 0
									BEGIN
										SET @SQLJoin = @SQLJoin + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'INNER JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_' + @PropertyDimension + '] [' + @PropertyDimension + '] ON [' + @PropertyDimension + '].[' + CASE WHEN @MappingTypeID > 0 THEN 'MemberKeyBase' ELSE 'Label' END + '] = J.[' + ISNULL(@Segment, @PropertyDimension) + ']'
									END
								ELSE
									BEGIN
										SET @SQLJoin = @SQLJoin + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'INNER JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_' + @PropertyDimension + '] [' + @PropertyDimension + '] ON [' + @PropertyDimension + '].[' + CASE WHEN @MappingTypeID > 0 THEN 'MemberKeyBase' ELSE 'Label' END + '] = J.[' + @PropertyDimension + ']'
