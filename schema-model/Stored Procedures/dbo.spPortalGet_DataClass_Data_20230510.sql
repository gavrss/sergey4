...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_20230510]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spRun_Procedure_KeyValuePair] @ProcedureName='spPortalGet_DataClass_Data', @JSON='[{"TKey" : "DebugBM", "TValue" : "63"},
{"TKey" : "UserID", "TValue" : "10021"},{"TKey" : "InstanceID", "TValue" : "580"},{"TKey" : "VersionID", "TValue" : "1084"},
{"TKey" : "ActingAs", "TValue" : "5782"},{"TKey" : "DataClassID", "TValue" : "5747"},
{"TKey" : "Filter", "TValue" : "TimeView=Periodic|Group=NONE|InterCompany=All_|Flow=All_|BusinessProcess=CONSOLIDATED|BusinessRule=All_|Currency=All_|Entity=All_|Scenario=ACTUAL|Time=202303|Customer=All_|Supplier=All_|Version=NONE|LineItem=All_|GL_Department=All_|GL_Division=|GL_Posted=All_"},
{"TKey" : "Tuple", "TValue" : "#Tuple=actual-AC_ASSETS|Account=AC_ASSETS|Scenario=ACTUAL;#Tuple=actual-AC_LIAB_EQUIT|Account=AC_LIAB_EQUIT|Scenario=ACTUAL;#Tuple=actual-CONS_BS_Offset|Account=CONS_BS_Offset|Scenario=ACTUAL;#Tuple=actual-Balance_Sheet_|Account=Balance_Sheet_|Scenario=ACTUAL;#Tuple=actual-AC_NET_INCOME|Account=AC_NET_INCOME|Scenario=ACTUAL;#Tuple=actual-Income_Statement_|Account=Income_Statement_|Scenario=ACTUAL;#Tuple=actual-Financials_|Account=Financials_|Scenario=ACTUAL;#Tuple=adb47ca0-4be1-4ed1-9c6f-2d3f1de0b7d0-AC_ASSETS|Account=AC_ASSETS|Scenario=ACTUAL;#Tuple=adb47ca0-4be1-4ed1-9c6f-2d3f1de0b7d0-AC_LIAB_EQUIT|Account=AC_LIAB_EQUIT|Scenario=ACTUAL;#Tuple=adb47ca0-4be1-4ed1-9c6f-2d3f1de0b7d0-CONS_BS_Offset|Account=CONS_BS_Offset|Scenario=ACTUAL;#Tuple=adb47ca0-4be1-4ed1-9c6f-2d3f1de0b7d0-Balance_Sheet_|Account=Balance_Sheet_|Scenario=ACTUAL;#Tuple=adb47ca0-4be1-4ed1-9c6f-2d3f1de0b7d0-AC_NET_INCOME|Account=AC_NET_INCOME|Scenario=ACTUAL;#Tuple=adb47ca0-4be1-4ed1-9c6f-2d3f1de0b7d0-Income_Statement_|Account=Income_Statement_|Scenario=ACTUAL;#Tuple=adb47ca0-4be1-4ed1-9c6f-2d3f1de0b7d0-Financials_|Account=Financials_|Scenario=ACTUAL"},
{"TKey" : "RowList", "TValue" : "SupressZeroYN=1|Account=Financials_"},{"TKey" : "ResultTypeBM", "TValue" : "512"},{"TKey" : "UseCacheYN", "TValue" : "true"}]'

EXEC [spRun_Procedure_KeyValuePair] @ProcedureName='spPortalGet_DataClass_Data', @JSON='[{"TKey" : "DebugBM", "TValue" : "15"},
{"TKey" : "UserID", "TValue" : "7881"},{"TKey" : "InstanceID", "TValue" : "596"},{"TKey" : "VersionID", "TValue" : "1091"},
{"TKey" : "DataClassID", "TValue" : "5795"},{"TKey" : "ResultTypeBM", "TValue" : "132"},{"TKey" : "GroupBy", "TValue" : "Time|FullAccount"},
{"TKey" : "Measure", "TValue" : "Financials"},{"TKey" : "AssignmentID", "TValue" : "21128"},
{"TKey" : "Filter", "TValue" : "Flow=NONE|Time=2023|Scenario=BUDGET|Entity=DLMAC|Currency=MXN|FullAccount=Balance_Sheet_|BusinessProcess=INPUT|Version=NONE"},
{"TKey" : "RowList", "TValue" : "SupressZeroYN=0"},{"TKey" : "ActingAs", "TValue" : "5892"},{"TKey" : "UseCacheYN", "TValue" : "true"}]'
...
BEGIN TRY
	...
	SET @Step = 'SP-Specific check'
		IF (SELECT COUNT(1) FROM pcINTEGRATOR_Data..Measure WHERE InstanceID = @InstanceID AND VersionID = @VersionID AND MeasureName = @Measure) = 0
			BEGIN
				SET @ErrorMessage = 'Report definition is not correct. Selected measure ''' + @Measure + ''' does not exist in DataClass ''' + @DataClassName + '''';
	...
	SET @Step = 'Get temp table #MultiDim when needed'
		...
		IF @ResultTypeBM & 812 > 0 AND (SELECT COUNT(1) FROM #FilterTable WHERE [MultiDimIncludedYN] <> 0) > 0
			BEGIN
				SET @MultiDimYN = 1
		...
		IF @ResultTypeBM & 772 > 0
			BEGIN
				...
				DECLARE Tuple_Cursor CURSOR FOR
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
							IF @DebugBM & 32 > 0 SELECT [@TupleNo] = @TupleNo, [@TupleName] = @TupleName, [@SQL_Tuple] = @SQL_Tuple, [LEN_@SQL_Tuple] = LEN(@SQL_Tuple)
...
/************ ADDED: by NeHa 20230329	start:	*/

						AND FT.[DimensionTypeID] <> 27

/************ ADDED: by NeHa 20230329	end	*/
		...
		IF @ResultTypeBM & 512 > 0
			BEGIN
				...
				IF @GroupByLeafLevelYN <> 0
					...
					IF @DebugBM & 2 > 0 PRINT '>>>>>>>>>>>>>>>@SQL_Select2: ' + @SQL_Select2
					IF @DebugBM & 2 > 0 PRINT '>>>>>>>>>>>>>>>@SQL_Select2: ' + @SQL_Select2
				...
				SET @SQLStatement =	@SQLStatement + '
					...
					FROM
						#Hierarchy H 
						LEFT JOIN ' + @TmpGlobalTable + ' DC ON DC.[' + @HierarchyDimension + '_MemberId] = H.MemberID AND H.[NodeTypeBM] = 1
...
--' + @SQL_Join2 + '
				...
				IF @DebugBM & 2 > 0 AND LEN(@SQLStatement) > 4000 
					BEGIN
						PRINT 'Length of @SQLStatement more than 4000, see pcINTEGRATOR_Log..wrk_Debug; INSERT INTO @TmpGlobalTable_2'
						EXEC [dbo].[spSet_wrk_Debug]
							@UserID = @UserID,
							@InstanceID = @InstanceID,
							@VersionID = @VersionID,
							@DatabaseName = @DatabaseName,
							@CalledProcedureName = @ProcedureName,
							@Comment = ' INSERT INTO @TmpGlobalTable_2', 
							@SQLStatement = @SQLStatement
					END
				ELSE
					PRINT @SQLStatement
				...
				IF @DebugBM & 2 > 0 AND LEN(@SQLStatement) > 4000 
					BEGIN
						PRINT 'Length of @SQLStatement more than 4000, see pcINTEGRATOR_Log..wrk_Debug; Return ResultTypeBM = 512'
						EXEC [dbo].[spSet_wrk_Debug]
							@UserID = @UserID,
							@InstanceID = @InstanceID,
							@VersionID = @VersionID,
							@DatabaseName = @DatabaseName,
							@CalledProcedureName = @ProcedureName,
							@Comment = 'Return ResultTypeBM = 512', 
							@SQLStatement = @SQLStatement
					END
				ELSE
					PRINT @SQLStatement
	...
	SET @Step = 'Drop temp tables'
		...
		IF @MultiDimYN <> 0
			...
			END
		IF @ResultTypeBM & 160 > 0 AND @LineItemExistsYN <> 0
			BEGIN
...
BEGIN CATCH
	...
	IF @ErrorNumber = 308 
		begin
			SET @ErrorMessage = 'Index is now recreating. Try to run the report in couple of minutes. '  + CHAR(10) + CHAR(13) + @ErrorMessage ;
