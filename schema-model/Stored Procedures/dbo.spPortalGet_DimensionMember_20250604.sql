...
SET ANSI_NULLS ON
GO
create PROCEDURE [dbo].[spPortalGet_DimensionMember_20250604]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'FDB-1948: Handle dimension property [NodeTypeBM] = NULL values by using [RNodeType]. FPA-135: FOr @ResultTypeBM=4, include NodeTypeBM=1024 for @HierarchyView <> "Level".'
	...
	SET @Step = 'Create temp table #Dimension_Member'
		IF @ResultTypeBM & 4 > 0
			BEGIN
				...
				IF @DebugBM & 2 > 0 SELECT SQLStatement_Select = @SQLStatement_Select, SQLStatement_Set = @SQLStatement_Set
