...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_OrderLine_Raw]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_Dim_OrderLine_Raw] @UserID = -10, @InstanceID = 531, @VersionID = 1057, @SourceTypeID = 11, @DebugBM = 3
EXEC [spIU_Dim_OrderLine_Raw] @UserID = -10, @InstanceID = 454, @VersionID = 1021, @SourceTypeID = 11, @DebugBM = 3, @SourceDatabase = 'DSPSOURCE01.CCM_E10'

EXEC [spIU_Dim_OrderLine_Raw] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Get OrderLines from Source system',
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		IF OBJECT_ID(N'TempDB.dbo.#OrderLine_Members', N'U') IS NULL SET @CalledYN = 0
	...
	SET @Step = 'Create temp table #OrderLine_Members_Raw'
		CREATE TABLE #OrderLine_Members_Raw
			(
			[MemberId] bigint,
	...
	SET @Step = 'Fill temptable #OrderLine_Members_Raw, Entity level'
		IF @MappingTypeID IN (1, 2)
			BEGIN
				INSERT INTO #OrderLine_Members_Raw
					(
					[MemberKey],
					[Description],
					[NodeTypeBM],
	...
	SET @Step = 'Fill temptable #OrderLine_Members_Raw'
		IF @SourceTypeID IN (1, 11) --Epicor ERP (E9, E10)
			BEGIN
				IF @SequenceBMStep & 1 > 0 --OrderLine Leafs
					BEGIN
						SET @SQLStatement = '
							INSERT INTO #OrderLine_Members_Raw
								(
							...
							SELECT DISTINCT
								[MemberId] = NULL,
								[MemberKey] = CONVERT(nvarchar(255), OD.OrderLine) COLLATE DATABASE_DEFAULT,
								[Description] = CONVERT(nvarchar(255), OD.OrderLine) COLLATE DATABASE_DEFAULT,
								[HelpText] = '''',
								[NodeTypeBM] = 1,
								[Source] = ''' + @SourceTypeName + ''',
								--[Parent] = ' + CASE WHEN @MappingTypeID IN (1, 2) THEN '''E_'' + EB.Entity' ELSE '''All_''' END + ',
								[Parent] = ' + CASE WHEN @MappingTypeID IN (1, 2) THEN '''E_'' + EB.Entity' ELSE '''Line''' END + ',
								[Priority] = EB.[Priority],
								[Entity] = EB.Entity
							FROM
								' + @SourceDatabase + '.[' + @Owner + '].[OrderDtl] OD
								INNER JOIN [#EB] EB ON EB.Entity = OD.Company COLLATE DATABASE_DEFAULT'
	...
	SET @Step = 'Return data'
		IF @CalledYN = 0
			BEGIN
				SELECT
					TempTable = '#OrderLine_Members_Raw',
					*
				FROM
					#OrderLine_Members_Raw
				ORDER BY
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
							...
							WHERE
								...
								NOT EXISTS (SELECT 1 FROM [#OrderLine_Members] AM WHERE AM.[MemberKey] = [MaxRaw].[MemberKey])
							ORDER BY
	...
	SET @Step = 'Drop the temp tables'
		DROP TABLE #OrderLine_Members_Raw
