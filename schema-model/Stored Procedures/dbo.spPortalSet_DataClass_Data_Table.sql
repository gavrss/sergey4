...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalSet_DataClass_Data_Table]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
{"TKey" : "UserID", "TValue" : "36254"},{"TKey" : "InstanceID", "TValue" : "1023"},
{"TKey" : "VersionID", "TValue" : "1418"},{"TKey" : "DataClassID", "TValue" : "7313"},
{"TKey" : "WorkflowStateID", "TValue" : "16381"},{"TKey" : "AssignmentID", "TValue" : "31284"}]',
@JSON_table='[{"Filter" : "Flow=NONE|Time=202601|Scenario=BUDGET|Entity=01|Currency=CAD|GL_Account_Reference=NONE|GL_Department=000|GL_Division=00|Account=800200|BusinessProcess=INPUT|Version=NONE", "MeasureValue" : "Financials=111.0"}]'

EXEC [spRun_Procedure_KeyValuePair] @ProcedureName='spPortalSet_DataClass_Data_Table', @JSON='[{"TKey" : "DebugBM", "TValue" : "15"},
{"TKey" : "UserID", "TValue" : "9863"},{"TKey" : "InstanceID", "TValue" : "572"},{"TKey" : "VersionID", "TValue" : "1080"},
{"TKey" : "DataClassID", "TValue" : "5736"},{"TKey" : "WorkflowStateID", "TValue" : "9660"},{"TKey" : "AssignmentID", "TValue" : "19992"}
]', @JSON_table='[
{"Filter" : "FixedAsset=NONE|Group=NONE|Flow=NONE|Supplier=NONE|Customer=NONE|Time=202404|Scenario=BUDGET|Entity=AU03|Account=AS_8743|GL_CostCenter=05|GL_Branch=229|BusinessProcess=INPUT", "MeasureValue" : "Assumption_CC_Br=5.0E-4"}]'

EXEC [spRun_Procedure_KeyValuePair] @ProcedureName='spPortalSet_DataClass_Data_Table', @JSON='[{"TKey" : "DebugBM", "TValue" : "15"},
{"TKey" : "UserID", "TValue" : "7881"},{"TKey" : "InstanceID", "TValue" : "596"},{"TKey" : "VersionID", "TValue" : "1091"},
{"TKey" : "DataClassID", "TValue" : "5795"},{"TKey" : "WorkflowStateID", "TValue" : "9826"},{"TKey" : "AssignmentID", "TValue" : "21128"}
]', @JSON_table='[
{"Filter" : "Flow=NONE|Time=202301|Scenario=BUDGET|Entity=DLMAC|Currency=MXN|FullAccount=1101-00000-001|BusinessProcess=INPUT|BusinessRule=NONE|Version=NONE", "MeasureValue" : "Financials=100.0"},
{"Filter" : "Flow=NONE|Time=202301|Scenario=BUDGET|Entity=DLMAC|Currency=MXN|Account=1102|GL_Subcuenta=00000|GL_Afectable=002|BusinessProcess=INPUT|BusinessRule=NONE|Version=NONE", "MeasureValue" : "Financials=110.0"},
{"Filter" : "Flow=NONE|Time=202301|Scenario=BUDGET|Entity=DLMAC|Currency=MXN|FullAccount=1103-00100-001|Account=1103|GL_Subcuenta=00100|GL_Afectable=001|BusinessProcess=INPUT|BusinessRule=NONE|Version=NONE", "MeasureValue" : "Financials=120.0"}]'

EXEC [spRun_Procedure_KeyValuePair] @ProcedureName='spPortalSet_DataClass_Data_Table', @JSON='[{"TKey" : "DebugBM", "TValue" : "15"},
{"TKey" : "UserID", "TValue" : "7881"},{"TKey" : "InstanceID", "TValue" : "596"},{"TKey" : "VersionID", "TValue" : "1091"},
{"TKey" : "DataClassID", "TValue" : "5795"},{"TKey" : "WorkflowStateID", "TValue" : "9826"},{"TKey" : "AssignmentID", "TValue" : "21128"}
]', @JSON_table='[
{"Filter" : "Flow=NONE|Time=202301|Scenario=BUDGET|Entity=DLMAC|Currency=MXN|FullAccount=1103-00100-001|Account=1105|GL_Subcuenta=00100|BusinessProcess=INPUT|BusinessRule=NONE|Version=NONE", "MeasureValue" : "Financials=120.0"}]'
...
DECLARE
	...
	@MultiDim_SelectCallisto NVARCHAR(MAX) = '',
	@MultiDim_DimensionName NVARCHAR(100),
	@MultiDim_Value NVARCHAR(MAX) = '',
	@MultiDim_ComponentName NVARCHAR(100),
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2195' SET @Description = 'DB-1361: Handle FullAccount/MultiDim settings.'
		IF @Version = '2.1.2.2196' SET @Description = 'Calculate FX when appropriate.'
		IF @Version = '2.1.2.2197' SET @Description = 'Test of variable @LineItem_ExistsYN moved from an IF statement to a CASE statement when building the SQL query.'
		IF @Version = '2.1.2.2198' SET @Description = 'DB-1597: Added check - Run FxTrans if Currency_MemberId exists in #FACT_DataClass_MemberId.'
		IF @Version = '2.1.2.2199' SET @Description = 'FDB-2101: added (NOLOCK) for TempDB.sys.tables. FDB-3005: Added [DeletedID] filter on LEFT JOIN [Dimension] when INSERTing INTO #ObjectListMaster.'
...
BEGIN TRY
	...
	SET @Step = 'Create temp table #ObjectListMaster'
		...
		FROM
			...
			INNER JOIN Dimension D ON D.InstanceID IN (0, @InstanceID) AND D.DimensionID = DCD.DimensionID AND D.DeletedID IS NULL
	...
	SET @Step = 'Create temp tables #MultiDim'
		CREATE TABLE #MultiDim
			(
			[DimensionID] int,
			[DimensionName] nvarchar(100) COLLATE DATABASE_DEFAULT,
			[ComponentDimensionID] int,
			[ComponentDimensionName] nvarchar(100) COLLATE DATABASE_DEFAULT,
			[InputValue] nvarchar(255) COLLATE DATABASE_DEFAULT
			)
		...
		(
			[DimensionID],
			[DimensionName],
			[ComponentDimensionID],
			[ComponentDimensionName]
		)
		SELECT
			[DimensionID] = DP.DimensionID,
			[DimensionName] = O.ObjectName,
			[ComponentDimensionID] = P.DependentDimensionID,
			[ComponentDimensionName] = OM.ObjectName
		FROM
			[pcINTEGRATOR].[dbo].[Dimension_Property] DP
			INNER JOIN [pcINTEGRATOR].[dbo].[Property] P ON P.PropertyID = DP.PropertyID
			INNER JOIN #ObjectListMaster O ON O.ObjectType = 'Dimension' AND O.ObjectID = DP.DimensionID AND O.DimensionTypeID = 27 --MultiDim/FullAccount
			INNER JOIN #ObjectListMaster OM ON OM.ObjectType = 'Dimension' AND OM.ObjectID = P.DependentDimensionID
		WHERE
			DP.InstanceID = @InstanceID AND
			DP.VersionID = @VersionID AND
			DP.MultiDimYN <> 0
		ORDER BY
			DP.DimensionID, DP.SortOrder

		IF @DebugBM & 2 > 0 SELECT TempTable = '#MultiDim', * FROM #MultiDim

	SET @Step = 'Create temp tables #MultiDim_Callisto'
		CREATE TABLE #MultiDim_Callisto
			(
			[ComponentDimensionName] nvarchar(100) COLLATE DATABASE_DEFAULT,
			[ComponentDimensionValue] nvarchar(255) COLLATE DATABASE_DEFAULT
			)
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					SET @Step = 'Generate queries for temptables @FACT_DataClass_% (Based on @Filter)'
						IF LEN(@Filter) > 3
							BEGIN
								--Reset temptable values
								TRUNCATE TABLE #PipeStringSplit
								TRUNCATE TABLE #MultiDim_Callisto
								UPDATE M SET [InputValue] = NULL FROM #MultiDim M
								SELECT @MultiDim_DimensionName = '', @MultiDim_Value = ''

								--1. get value from @Filter
								...
								--1.5 set #MultiDim [InputValue] from FullAccount Components
								SELECT
									@MultiDim_DimensionName = M.DimensionName,
									@MultiDim_Value = P.PipeFilter
								FROM
									#MultiDim M
									INNER JOIN #PipeStringSplit P ON P.PipeObject = M.DimensionName

								IF @DebugBM & 2 > 0 SELECT [@MultiDim_DimensionName] = @MultiDim_DimensionName, [@MultiDim_Value] = @MultiDim_Value
								...
								DECLARE MultiDim_Cursor CURSOR FOR
									...
									FROM
										#MultiDim
									WHERE
										DimensionName = @MultiDim_DimensionName

									OPEN MultiDim_Cursor
									FETCH NEXT FROM MultiDim_Cursor INTO @MultiDim_ComponentName

									WHILE @@FETCH_STATUS = 0
										BEGIN
											IF @DebugBM & 2 > 0 SELECT [@MultiDim_ComponentName] = @MultiDim_ComponentName

											SELECT @SQLStatement = '
												INSERT INTO #MultiDim_Callisto
													(
													[ComponentDimensionName],
													[ComponentDimensionValue]
													)
												SELECT
													[ComponentDimensionName] = ''' + @MultiDim_ComponentName + ''',
													[ComponentDimensionValue] = MD.[' + @MultiDim_ComponentName + ']
												FROM
													' + @CallistoDatabase + '.[dbo].[S_DS_' + @MultiDim_DimensionName + '] MD
												WHERE MD.[Label] = ''' + @MultiDim_Value + ''''
								...
								IF @DebugBM & 2 > 0 SELECT [#MultiDim_Callisto] = '#MultiDim_Callisto', * FROM #MultiDim_Callisto

								UPDATE M
								SET [InputValue] = MC.ComponentDimensionValue
								FROM
									#MultiDim M
									INNER JOIN #MultiDim_Callisto MC ON MC.ComponentDimensionName = M.ComponentDimensionName
								WHERE
									M.DimensionName = @MultiDim_DimensionName

								IF @DebugBM & 2 > 0 SELECT [#MultiDim] = '#MultiDim', * FROM #MultiDim

								--2. Update/add rows to #PipeStringSplit from #MultiDim
								UPDATE P
								SET
									[PipeFilter] = M.InputValue
								FROM
									#PipeStringSplit P
									INNER JOIN #MultiDim M ON M.ComponentDimensionName = P.PipeObject AND M.DimensionName = @MultiDim_DimensionName
								...
								SELECT
									TupleNo = 0,
									PipeObject = M.ComponentDimensionName,
									EqualityString = 'IN',
									PipeFilter = M.InputValue
								FROM
									#MultiDim M
								WHERE
									M.DimensionName = @MultiDim_DimensionName AND
									NOT EXISTS (SELECT 1 FROM #PipeStringSplit P WHERE P.PipeObject = M.ComponentDimensionName)

								--3. Update/add rows from #ObjectListMaster [DefaultValue]
								--update [DefaultValue] if NULL and rows if missing
								UPDATE P
								SET
									[PipeFilter] = O.DefaultValue
								FROM
									#PipeStringSplit P
									INNER JOIN #ObjectListMaster O ON O.ObjectName = P.PipeObject AND O.DefaultValue IS NOT NULL
								WHERE
									P.[PipeFilter] IS NULL
								...
								SELECT
									TupleNo = 0,
									PipeObject = O.ObjectName,
									EqualityString = 'IN',
									PipeFilter = O.DefaultValue
								FROM
									#ObjectListMaster O
								WHERE
									O.DimensionTypeID <> 27 AND
									O.ObjectType = 'Dimension' AND
									NOT EXISTS (SELECT 1 FROM #PipeStringSplit P WHERE P.PipeObject = O.ObjectName)

								IF @DebugBM & 2 > 0 SELECT [#PipeStringSplit] = '#PipeStringSplit', * FROM #PipeStringSplit
								IF @DebugBM & 2 > 0 SELECT [#ObjectListMaster] = '#ObjectListMaster', * FROM #ObjectListMaster

								--4.Check for Info Message
								SET @Step = 'Info message - Check for NULL dimensions.'
								IF (SELECT COUNT(1) FROM #PipeStringSplit WHERE PipeFilter IS NULL) > 0
									BEGIN
										SET @ErrorMessage = 'Value for Dimension [' + (SELECT TOP(1) PipeObject from #PipeStringSplit where PipeFilter IS NULL) + '] is not set.';
								...
								IF @StorageTypeBM_DataClass & 4 > 0
									BEGIN
										...
										DECLARE AddColumnValue_Cursor CURSOR FOR
											...
											WHERE
												OLM.[ObjectType] = 'Dimension' AND
												OLM.DimensionTypeID <> 27
	...
	SET @Step = 'Set @Filter for FxTrans conversion'
		--Check Currency_MemberId
			SELECT
				@RunFxTransYN = CASE WHEN COUNT(1) = 5 THEN 1 ELSE 0 END
			FROM
				TempDB.sys.tables t (NOLOCK)
				INNER JOIN TempDB.sys.columns c (NOLOCK) ON c.object_id = t.object_id AND c.[name] IN ('BusinessProcess_Memberid', 'Entity_Memberid', 'Scenario_Memberid', 'Time_Memberid', 'Currency_MemberId')
			WHERE
				t.[name] LIKE '#FACT_DataClass_MemberId%'

		IF @DebugBM & 2 > 0 SELECT [@RunFxTransYN] = @RunFxTransYN

		--Check Currencies to convert
		SET @SQLStatement = '
			SELECT @InternalVariable = COUNT(1)
			FROM ' + @CallistoDatabase + '.[dbo].[S_DS_Currency]
			WHERE Reporting <> 0'

		EXEC sp_executesql @SQLStatement, N'@internalVariable int OUT', @InternalVariable = @ReturnVariable OUT

		SET @RunFxTransYN = CONVERT(int, @RunFxTransYN) * CASE WHEN @ReturnVariable > 0 THEN 1 ELSE 0 END

		IF @DebugBM & 2 > 0 SELECT [@RunFxTransYN] = @RunFxTransYN

		IF @DebugBM & 2 > 0 SELECT TempTable = '#FACT_DataClass_MemberId before FX', * FROM #FACT_DataClass_MemberId

		IF @RunFxTransYN = 0 GOTO NoFxTrans

		SET @Filter = 'FilterType=MemberID'
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#Parameter', * FROM #Parameter
					...
					DECLARE Parameter_Cursor CURSOR FOR
						...
						WHILE @@FETCH_STATUS = 0
							BEGIN
								...
								SET @Filter = @Filter + CONVERT(nvarchar(20), @MemberId) + ','
	...
	SET @Step = 'Run FxTrans'
		IF @DebugBM & 1 > 0 SELECT [TempTable] = '#FACT_DataClass_MemberId before fx calculation', [@Rows] = COUNT(1) FROM #FACT_DataClass_MemberId
		...
		SET @JSON = '
			...
			{"TKey" : "TempTable",  "TValue": "#FACT_DataClass_MemberId"},
		...
		IF @DebugBM & 1 > 0 SELECT [TempTable] = '#FACT_DataClass_MemberId after fx calculation', [@Rows] = COUNT(1) FROM #FACT_DataClass_MemberId

		IF @DebugBM & 2 > 0 SELECT [TempTable] = '#FACT_DataClass_MemberId after fx calculation', * FROM #FACT_DataClass_MemberId
	...
	SET @Step = 'Delete row from Callisto (Measure_Text)'
		...
		IF @FACT_DataClass_Text_Table_ExistsYN <> 0
			BEGIN
				SET @SQLStatement = '
					DELETE F
					FROM
						' + @CallistoDatabase + '.[dbo].[' + @FACT_DataClass_Text_Table + '] F
						' + CASE WHEN @LineItem_ExistsYN <> 0 THEN 'INNER JOIN #FACT_DataClass_MemberId [#FACT_DataClass_MemberId_LI] ON [#FACT_DataClass_MemberId_LI].[LineItem_MemberId] <> -1 AND [#FACT_DataClass_MemberId_LI].[' + @MeasureName + '_Text] IS NOT NULL' ELSE '' END + '
						INNER JOIN #FACT_DataClass_MemberId [#FACT_DataClass_MemberId] ON ' + @FACT_DataClass_MemberId_DeleteCallisto
			...
			BEGIN
				SET @SQLStatement = '
					...
					FROM
						#FACT_DataClass_MemberId
					WHERE
						' + CASE WHEN @LineItem_ExistsYN <> 0 THEN '[LineItem_MemberId] <> -1 AND' ELSE '' END + '
						[' + @MeasureName + '_Text] IS NOT NULL'
