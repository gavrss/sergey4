...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spGet_FilterTable]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2196' SET @Description = 'Exclude calculation of LeafLevelFilter for @DimensionTypeID = 27. Handled in spGet_MultiDimFilter.'
		IF @Version = '2.1.2.2197' SET @Description = 'Handle InterCompanyEntity'
		IF @Version = '2.1.2.2198' SET @Description = 'DB-1492: Set @FilterString with #FilterTable.[Filter] NOT NULL value.'
		IF @Version = '2.1.2.2199' SET @Description = 'Set [Method] = Property when setting [PropertyFilter]'
...
--						[DimensionID] NOT IN (-7, -49)
	...
	SET @Step = 'Update ObjectReference'
		...
		UPDATE FT
		SET
			[JournalColumn] = CASE FT.DimensionID WHEN -31 THEN 'InterCompanyEntity' ELSE FT.[JournalColumn] END
		FROM
			#FilterTable FT
		WHERE
			@StorageTypeBM_DataClass & 1 > 0
		...
		DECLARE LeafLevelFilter_Cursor CURSOR FOR
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					ELSE IF @HierarchyName IS NOT NULL AND CASE WHEN LEN(@Filter) = 0 THEN NULL ELSE @Filter END IS NOT NULL AND @DimensionTypeID <> 27
						BEGIN
