...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_FxRate]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_DC_FxRate] @UserID=-10, @InstanceID=454, @VersionID=1021, @Debug=1
EXEC [spIU_DC_FxRate] @UserID=-10, @InstanceID=454, @VersionID=1021, @InvertBaseCurrencyYN = 1, @InvertRateYN = 1, @Debug=1
EXEC [spIU_DC_FxRate] @UserID=-10, @InstanceID=454, @VersionID=1021, @Entity_MemberKey = 'R510', @InvertBaseCurrencyYN = 0, @InvertRateYN = 0, @Debug=1
EXEC [spIU_DC_FxRate] @UserID=-10, @InstanceID=485, @VersionID=1034, @InvertBaseCurrencyYN = 1, @StartYear=2018, @Debug=1

EXEC [spIU_DC_FxRate] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Load DataClass FxRate.',
...
BEGIN TRY
	...
	SET @Step = 'Create and fill temp table #FxRate_Raw'
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#FxRate_Raw', * FROM #FxRate_Raw ORDER BY [BaseCurrency], [TimeDay], [Currency], [Entity], [Scenario]
	...
	SET @Step = 'Handle MasterEntity'
		...
		WHERE
			...
			E.DeletedID IS NULL
		ORDER BY
			ISNULL(E.[Priority], 99999999),
			E.MemberKey

		IF @DebugBM & 2 > 0 SELECT [@MasterEntity] = @MasterEntity
