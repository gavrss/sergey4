...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR05_Fx_Opening]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'Added correction of Opening Retained earnings to match Opening from ERP. Include flow OP_OPENING if it is the first year when YTD is calculated, Handle when no historic rows are found'
...
BEGIN TRY
	...
	SET @Step = 'HistoricCursor'
		IF @HistoricYN <> 0
		BEGIN
			IF (Select COUNT(1) from #HistoricCursor_Table) > 0 
				BEGIN
		...
		IF @HistoricYN <> 0
			BEGIN
				SET @SQLStatement = '
					DELETE #JournalBase
					WHERE 
						SourceModule = ''Fx_Opening''' + CASE WHEN LEN(@JournalFIlter) > 0 THEN ' AND ' + @JournalFilter  ELSE '' END 
...
J.[Account] NOT IN (''PYNI_B'') AND
							...
							J.[BalanceYN] <> 0 AND
							J.[JournalSequence] NOT IN (''ELIM'', ''G_ELIMADJ'', ''DIVIDEND'')   --, ''G_HISTRATEADJ'', ''HISTRATE_ELIM''
							' + CASE WHEN LEN(@JournalFilter) > 0 THEN ' AND ' + @JournalFilter ELSE '' END
				...
				SET @SQLStatement = @SQLStatement + '
						...
						WHERE
							...
							J.Scenario = ''' + @Scenario + ''' AND
							J.[BalanceYN] <> 0 AND
							(ISNULL(J.[Flow], ''NONE'') NOT IN (''OP_Opening'') OR EB.[FirstFiscalYearYN] <> 0 OR J.Account = ''PYNI_B'') AND
							' + CASE WHEN LEN(@JournalFilter) > 0 THEN  @JournalFilter + ' AND ' ELSE '' END + ' 
...
IF @InstanceID = 603 -- Only for Entity PCXNW
-- IF 1=2
BEGIN
...
--							[FiscalPeriod] = J.[FiscalPeriod],
							...
							[Segment20] = J.[Segment20],
							[Flow] = 'OP_OpenYear',
							[InterCompanyEntity] = CASE WHEN J.[InterCompanyEntity] = '' THEN 'NONE' ELSE J.[InterCompanyEntity] END ,
							[Currency_Book] = J.[Currency_Book],
							[Value_Book] = SUM([Value_Book]),
							[Value_Group] = SUM([Value_Group]) 
				INTO #OBERPADJ 
				FROM #JournalBase J
				Inner Join #EntityBook EB ON EB.MemberKey = J.Entity AND EB.Book = J.Book AND EB.SelectYN <> 0 AND EB.Account_RE = J.Account AND EB.MemberKey IN ('PCXNW','20')
				WHERE ((SourceModule = 'Fx_Opening' AND Flow = 'OP_OpenYear') OR Flow = 'OP_AdjSrc') AND FiscalPeriod = '1' 
				GROUP BY  J.[Entity],
						  J.[Book],
						  J.[FiscalYear],
--						  J.[FiscalPeriod],
						  ...
						  J.[Segment19],
						  J.[Segment20],
						  CASE WHEN J.[InterCompanyEntity] = '' THEN 'NONE' ELSE J.[InterCompanyEntity] END,
						  J.[Currency_Book]

				Set @SQLStatement = '
				INSERT INTO #OBERPADJ
	                        (
	                        [Entity] ,
							[Book] ,
							[FiscalYear] ,
...
--							[FiscalPeriod] = J.[FiscalPeriod],
				...
				FROM ' + @JournalTable + ' J
				Inner Join #EntityBook EB ON EB.MemberKey = J.Entity AND EB.Book = J.Book AND EB.SelectYN <> 0 AND EB.Account_RE = J.Account  AND EB.MemberKey IN (''PCXNW'',''20'')
				WHERE FiscalYear = ' + Convert(nvarchar(15),@FiscalYear) + ' AND FiscalPeriod = 0 AND JournalSequence = ''OB_ERP'' AND TransactionTypeBM = 4
				GROUP BY  J.[Entity],
						  J.[Book],
						  J.[FiscalYear],
--						  J.[FiscalPeriod],
				...
				IF @DebugBM & 2 > 0 SELECT temptable = '#OBERPADJ',  * from #OBERPADJ
				IF @DebugBM & 2 > 0 SELECT temptable = '#JournalBase', * from #JournalBase WHERE Account IN ('3300') AND SourceModule = 'Fx_Opening' AND Flow = 'OP_OpenYear' AND FiscalPeriod = '1'

				IF (SELECT count(1) from #OBERPADJ WHERE JournalSequence = 'OBERP') > 0
				BEGIN
					...
					SELECT    
						  [ReferenceNo] = 13000000 + ROW_NUMBER() OVER(ORDER BY J.[Entity], J.[Book], J.[FiscalYear], J.[Account]),
						  ...
						  J.[FiscalYear],
						    ...
						    [JournalNo] = 23000000 + ROW_NUMBER() OVER(PARTITION BY J.[Entity], J.[Book], J.[FiscalYear] ORDER BY J.[Entity]),
						    [YearMonth] = MAX(FP2.YearMonth),
						    [BalanceYN] = 1,
						  J.[Account],
						  J.[Segment01],
						  J.[Segment02],
						  ...
						  J.[InterCompanyEntity],
						    ...
						    [Supplier] = '',
						    [Description_Head] = 'Fx_Opening',
						    [Description_Line] = 'Difference Between Closing Prev.Year and Opening from ERP system' ,
						  J.[Currency_Book],
						    [Value_Book] = SUM(CASE WHEN J.JournalSequence = 'OBERP' THEN J.Value_Book ELSE 0 END) - SUM(CASE WHEN J.JournalSequence = 'PREVYEAR' THEN J.Value_Book ELSE 0 END),
						    [Currency_Group] = @Currency_Group,
						    [Value_Group] = SUM(CASE WHEN J.JournalSequence = 'OBERP' THEN J.Value_Group ELSE 0 END) - SUM(CASE WHEN J.JournalSequence = 'PREVYEAR' THEN J.Value_Group ELSE 0 END),
		 			      --[Currency_Transaction],
						  --[Value_Transaction],
						    [SourceModule] = 'Fx_Opening',
						    [SourceModuleReference] = ''
					...
					from #OBERPADJ J
					INNER JOIN #EntityBook EB ON EB.[MemberKey] = J.[Entity] AND EB.[Book] = J.[Book] AND EB.[SelectYN] <> 0 AND EB.MemberKey IN ('PCXNW','20')
					INNER JOIN #CountedFiscalPeriods FP2 ON FP2.FiscalYear = J.FiscalYear AND FP2.FiscalPeriod <> 0
					GROUP BY
						  J.[Entity],
						  J.[Book],
						  J.[FiscalYear],
						  ...
						  J.[Currency_Book]
					HAVING 
					ROUND(SUM(CASE WHEN J.JournalSequence = 'OBERP' THEN J.Value_Book ELSE 0 END),1) <> Round(SUM(CASE WHEN J.JournalSequence = 'PREVYEAR' THEN J.Value_Book ELSE 0 END),1)
				 END
				 -- End of Added code for PCX
				 END
