...
CREATE PROCEDURE [dbo].[spJIRA_ErrorAttention_IU_Issues]

AS
-- exec [spJIRA_ErrorAttention_IU_Issues]
...
WHERE
	EA.JiraIssueID IS NULL AND
	JL.[ErrorNumber] NOT IN (0, -1)
...
INNER JOIN [pcINTEGRATOR_Log].[dbo].[JobLog] JL ON 1 = 1
...
ORDER BY
	...
	JL.StartTime DESC'
		...
		FROM [pcINTEGRATOR_Log].[dbo].[JobLog] JL
		LEFT JOIN [pcINTEGRATOR_Log].[dbo].[ErrorAttention] EA ON EA.[JobLogID] = JL.[JobLogID]
		LEFT JOIN (SELECT	 JL_with_Jira.InstanceID
							...
							,COUNT(1) AS EACount
					FROM [pcINTEGRATOR_Log].[dbo].[JobLog] JL_with_Jira 
...
DECLARE @BodyDataUpdate VARCHAR(8000) = '{
										  "jql": "updated >= \"' + @startupdate + '\" AND updated <= \"' + @stopupdate + '\" AND project = \"' + @JiraProject + '\" AND issuetype in (\"Bug\", \"ErrorAttention\") AND labels in (\"ErrorAttention\") order by created DESC",
										  "maxResults": 100,
										  "fields": [
											"summary",
											"status"
