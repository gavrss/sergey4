...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR05_20230113]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
EXEC [spBR_BR05] @UserID=-10, @InstanceID=576, @VersionID=1082, @BusinessRuleID = 2717, @FiscalYear = 2021, @ConsolidationGroup = 'G_AXYZ', @DebugBM=19
EXEC [spBR_BR05] @UserID=-10, @InstanceID=576, @VersionID=1082, @BusinessRuleID = 2717, @FiscalYear = 2022, @ConsolidationGroup = 'G_AXYZ', @DebugBM=19
							...
							WHILE @@FETCH_STATUS = 0
								BEGIN
									...
									SET @Step = 'Set Retained earnings from previous year'
										IF @SequenceBM & 4 > 0
											BEGIN
												EXEC [pcINTEGRATOR].[dbo].[spBR_BR05_RE] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @FiscalYear = @FiscalYear, @ConsolidationGroup = @ConsolidationGroup, @JournalTable = @JournalTable, @JobID = @JobID, @Debug = 1 --@DebugSub
											END
