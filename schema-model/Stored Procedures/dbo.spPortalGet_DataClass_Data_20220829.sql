...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_20220829]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2187' SET @Description = 'Handle @ResultTypeBM=8 and other bugfixes.'
...
BEGIN TRY
	...
	SET @Step = 'Get parts for SQL-statement'
		IF @ResultTypeBM & 812 > 0 --4, 8, 32, 256, 512
			BEGIN
				SELECT
	...
	SET @Step = '@ResultTypeBM & 12'
		IF @ResultTypeBM & 12 > 0
			BEGIN
				...
				IF @DebugBM & 2 > 0  SELECT [@WorkflowStateYN] = @WorkflowStateYN, [@AddScenarioYN] = @AddScenarioYN
