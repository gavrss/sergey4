...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spSet_f_ScanString]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
EXEC spRun_Procedure_KeyValuePair
	@ProcedureName = 'spSet_f_ScanString',
	@JSON = '
		[
...
EXEC [spSet_f_ScanString] @UserID=-10, @InstanceID=390, @VersionID=1011, @Debug=1

EXEC [spSet_f_ScanString] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Update function f_ScanString',
			@MandatoryParameter = '' --Without @, separated by |

		IF @Version = '1.4.0.2139' SET @Description = 'Procedure created.'
		IF @Version = '2.1.0.2155' SET @Description = 'Made generic.'
		IF @Version = '2.1.1.2169' SET @Description = 'Fixed bug with return statement.'
...
BEGIN TRY
	...
	SET @Step = 'Set first generic part'
		SET @SQLStatement = 'ALTER FUNCTION [dbo].[f_ScanString]
(
	@InstanceID int,
	@VersionID int,
...
BEGIN
		...
		BEGIN
			IF @StringTypeBM & 1 > 0 --Code
				BEGIN'
					...
					SELECT 
						@SQLRow = @SQLRow + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'SELECT @Position = CHARINDEX (char(' + CONVERT(nvarchar, CS.Input) + '), @InputText) IF @Position <> 0 SET @ReturnStatement = @ReturnStatement + ''InputText = '' + @InputText + '', CharacterCode = char(' + CONVERT(nvarchar, CS.Input) + '), Position = '' + CONVERT(nvarchar, @Position) + ''; '''
					FROM
	...
	IF @StringTypeBM & 1 > 0 --Code
		BEGIN'
			...
			SELECT 
				@SQLRow = @SQLRow + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + 'SELECT @Position = CHARINDEX (char(' + CONVERT(nvarchar, CS.Input) + '), @InputText) IF @Position <> 0 SET @ReturnStatement = @ReturnStatement + ''InputText = '' + @InputText + '', CharacterCode = char(' + CONVERT(nvarchar, CS.Input) + '), Position = '' + CONVERT(nvarchar, @Position) + ''; '''
			FROM
