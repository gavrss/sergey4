...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_Financials_Raw]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'FDB-2102: added (NOLOCK) when @JournalTable is used'
			...
			BEGIN
				...
				FROM
					...
					SELECT
						...
						SourceColumn = CASE #C.DimensionTypeID
--								WHEN 1 THEN CASE WHEN #C.MappingTypeID = 1 THEN 'CASE WHEN J.[JournalSequence] <> ''CYNI'' THEN ''' + @Entity_CallistoLabel + '_'' ELSE '''' END + ' ELSE '' END + 'J.[Account]' + CASE WHEN #C.MappingTypeID = 2 THEN ' + CASE WHEN J.[JournalSequence] <> ''CYNI'' THEN ''_' + @Entity_CallistoLabel + ''' ELSE '''' END' ELSE '' END
								WHEN 1 THEN CASE WHEN #C.MappingTypeID = 1 THEN 'CASE WHEN J.[JournalSequence] <> ''CYNI'' AND J.[Account] <> ''PYNI_B'' THEN ''' + @Entity_CallistoLabel + '_'' ELSE '''' END + ' ELSE '' END + 'J.[Account]' + CASE WHEN #C.MappingTypeID = 2 THEN ' + CASE WHEN J.[JournalSequence] <> ''CYNI'' AND J.[Account] <> ''PYNI_B'' THEN ''_' + @Entity_CallistoLabel + ''' ELSE '''' END' ELSE '' END
