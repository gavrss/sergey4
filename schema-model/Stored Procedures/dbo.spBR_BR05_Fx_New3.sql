...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR05_Fx_New3]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
									...
									BEGIN
										...
										IF @Modifier = 'FYTD-1'
											BEGIN
												IF @DebugBM & 2 > 0
													BEGIN
														DROP TABLE pcINTEGRATOR_Log..tmp_JournalBase
														DROP TABLE pcINTEGRATOR_Log..tmp_Fx_Rate
														DROP TABLE pcINTEGRATOR_Log..tmp_FXR
...
--														[Rule_ConsolidationID] = MAX(JB.Rule_ConsolidationID),
														[Rule_ConsolidationID] = CASE WHEN FXRow.[Account] IS NULL OR FXRow.[Account] = '''' THEN MAX(JB.Rule_ConsolidationID) ELSE NULL END,
														...
														[InterCompanyEntity] = CASE WHEN FXRow.[NaturalAccountOnlyYN] = 0 THEN MAX(JB.[InterCompanyEntity]) ELSE '''' END,
														[Scenario] = MAX(JB.[Scenario]),
														[Customer] = MAX(JB.[Customer]),
														[Supplier] = MAX(JB.[Supplier]),
														[Description_Head] = ''CFX rule: ' + @RULE_FXName + '''+ '', Account: '' + JB.[Account] + '', Value_Book: '' + CONVERT(nvarchar(15), JB.[Value_Book]),
