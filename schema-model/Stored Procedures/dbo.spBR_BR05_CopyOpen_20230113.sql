...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spBR_BR05_CopyOpen_20230113]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
BEGIN TRY
	...
	SET @Step = 'CREATE TABLE #FiscalPeriod'
		IF OBJECT_ID(N'TempDB.dbo.#FiscalPeriod', N'U') IS NULL
			BEGIN
				...
				EXEC dbo.[spGet_Entity_FiscalYear] @UserID = @UserID, @InstanceID = @InstanceID, @VersionID = @VersionID, @EntityID = @EntityGroupID, @Book = @Book, @StartFiscalYear = @FiscalYear, @EndFiscalYear = @FiscalYear, @FiscalPeriod13YN = 1, @JobID = @JobID
	...
	SET @Step = 'Insert CopyOpen rows into #JournalBase'
		...
		SET @SQLStatement = @SQLStatement + '
			SELECT
				[ReferenceNo] = 12000000 + ROW_NUMBER() OVER(ORDER BY J.[Entity], J.[Book], FP.[FiscalYear], FP.[FiscalPeriod], J.[Account]),
				[ConsolidationMethodBM] = MAX(EB.[ConsolidationMethodBM]),
				[InstanceID] = ' + CONVERT(nvarchar(15), @InstanceID) + ',
				[Entity] = J.[Entity], 
				[Book] = J.[Book],
				...
				[FiscalPeriod] = FP.[FiscalPeriod],
				[JournalSequence] = ''JRNL'',
				[JournalNo] = 22000000 + ROW_NUMBER() OVER(PARTITION BY J.[Entity], J.[Book], FP.[FiscalYear] ORDER BY FP.[FiscalPeriod]),
				[YearMonth] = FP.[YearMonth],
				[BalanceYN] = J.[BalanceYN],
				[Account] = J.[Account],
				[Segment01] = J.[Segment01],
				...
				[Flow] = 
					CASE WHEN F.[Flow] = ''OP_Opening''
						THEN
							...
							END
						ELSE 
							CASE WHEN ISNULL(HR.[VAR_HistrateYN], 0) = 0 
								THEN CASE WHEN HR.[SetHistRate] IS NULL 
									THEN CASE WHEN [ValueCredit_Book] = 0 
										THEN ''VAR_Increase'' 
										ELSE ''VAR_Decrease'' 
									END 
								ELSE
		...
		SET @SQLStatement = @SQLStatement + '
				...
				INNER JOIN #EntityBook EB ON EB.[MemberKey] = J.[Entity] AND EB.[Book] = J.[Book] AND EB.[SelectYN] <> 0
				INNER JOIN (SELECT [Flow] = ''OP_Opening'' UNION SELECT [Flow] = ''Movement'') F ON 1 = 1
				LEFT JOIN [pcINTEGRATOR_Data].[dbo].[BR05_Rule_FX_HistRate] HR ON
					HR.[InstanceID] = J.[InstanceID] AND 
					HR.VersionID = ' + CONVERT(nvarchar(15), @VersionID) + ' AND 
					HR.[Entity] = J.[Entity] AND
					HR.[Book] = J.[Book] AND
					HR.[YearMonth] = FP.[YearMonth] AND
					HR.[Account] = J.[Account]
			WHERE
				J.[InstanceID] = ' + CONVERT(nvarchar(15), @InstanceID) + ' AND
				J.[TransactionTypeBM] & 19 > 0 AND
				J.[BalanceYN] <> 0 AND
				...
				J.[ConsolidationGroup] IS NULL AND
				J.PostedStatus <> 0 AND
				J.Account NOT IN (''CYNI_B'', ''PYNI_B'')
			GROUP BY
				J.[Entity], 
				J.[Book],
				FP.[FiscalYear],
				...
				J.[InterCompanyEntity]
			HAVING
				CASE WHEN F.[Flow] = ''OP_Opening'' AND FP.[FiscalPeriod] NOT IN (13, 14, 15) THEN SUM(CASE WHEN J.[FiscalPeriod] < FP.[FiscalPeriod] OR J.[FiscalPeriod] = 0 THEN J.[ValueDebit_Book] ELSE 0 END) ELSE SUM(CASE WHEN F.[Flow] <> ''OP_Opening'' AND J.[FiscalPeriod] = FP.[FiscalPeriod] AND J.[FiscalPeriod] <> 0 THEN J.[ValueDebit_Book] ELSE 0 END) END <> 0 OR
				CASE WHEN F.[Flow] = ''OP_Opening'' AND FP.[FiscalPeriod] NOT IN (13, 14, 15) THEN SUM(CASE WHEN J.[FiscalPeriod] < FP.[FiscalPeriod] OR J.[FiscalPeriod] = 0 THEN J.[ValueCredit_Book] ELSE 0 END) ELSE SUM(CASE WHEN F.[Flow] <> ''OP_Opening'' AND J.[FiscalPeriod] = FP.[FiscalPeriod] AND J.[FiscalPeriod] <> 0 THEN J.[ValueCredit_Book] ELSE 0 END) END <> 0'
