...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spCheckSum_Member_Missing_Hierarchy]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
EXEC [spCheckSum_Member_Missing_Hierarchy] @UserID=-10, @InstanceID=454, @VersionID=1021, @debug = 1
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2190' SET @Description = 'Fixed: "portal shows only one row instead of several. @Step Set CheckSumRowLogID: use ROW_NUMBER() instead TOP 1'
		IF @Version = '2.1.2.2193' SET @Description = 'Added WITH (NOLOCK) to prevent deadlocks'
		IF @Version = '2.1.2.2199' SET @Description = 'Checking if Hierarchy_Cursor exist - then CLOSE DEALLOCATE it. Added filter by InstanceID and VersionID in update wrk_CheckSum_Member_Missing_Hierarchy'
...
BEGIN TRY
									...
									BEGIN
										...
										SET @SQLStatement = '
											...
											SELECT DISTINCT
												...
												[MemberId] = ' + @Dimension + '
											FROM
												' + @CallistoDatabase + '..' + @Table + ' WITH (NOLOCK)
											OPTION (MAXDOP 1)'
	...
	SET @Step = 'Hierarchy Cursor'
	    ...
	    IF CURSOR_STATUS('global', 'Hierarchy_Cursor') >= 0
            BEGIN
	...
	SET @Step = 'Set CheckSumRowLogID'
		...
		FROM
			...
			INNER JOIN (
				SELECT
					RN = ROW_NUMBER() OVER(PARTITION BY C.[InstanceID], C.[VersionID], C.[ProcedureID], C.[CheckSumRowKey] ORDER BY C.[CheckSumRowLogID] desc),
					C.InstanceID,
					C.VersionID,
					C.[CheckSumRowLogID],
					C.[CheckSumRowKey],
					C.[ProcedureID]
				FROM
					[pcINTEGRATOR_Log].[dbo].[CheckSumRowLog] C WITH (NOLOCK)
				where   C.InstanceID = @InstanceID
				    and C.VersionID = @VersionID
			) CSRL ON CSRL.RN = 1 AND CSRL.[CheckSumRowKey] = CMMH.[FactTable] + '_' + CMMH.[Dimension] + '_'  + CMMH.[Hierarchy] + '_' + CONVERT(nvarchar(20), CMMH.[MemberId]) AND CSRL.[ProcedureID] = @ProcedureID AND CSRL.[InstanceID] = CMMH.[InstanceID] AND CSRL.[VersionID] = CMMH.[VersionID]
			AND CMMH.CheckSumRowLogID IS NULL
         OPTION (MAXDOP 1)
