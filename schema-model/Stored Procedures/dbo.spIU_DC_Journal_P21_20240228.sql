...
SET ANSI_NULLS ON
GO
create PROCEDURE [dbo].[spIU_DC_Journal_P21_20240228]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
AS
/*
EXEC [spIU_DC_Journal_P21] @UserID = -10, @InstanceID = 598, @VersionID = 1092, 
@SequenceBM = 1, @Entity_MemberKey = 'CIOT', @Book = 'GL', @FiscalYear = 2022, @DebugBM = 15
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'Made Generic.'
...
BEGIN TRY
	...
	SET @Step = 'Fill temp table #Segment'
		...
		IF @DebugBM & 2 > 0 SELECT [@SQLSegment] = @SQLSegment
