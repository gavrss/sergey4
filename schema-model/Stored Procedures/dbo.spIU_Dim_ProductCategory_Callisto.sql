...
CREATE PROCEDURE [dbo].[spIU_Dim_ProductCategory_Callisto]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
BEGIN TRY
	...
	SET @Step = 'Update Description and dimension specific Properties from source system where Synchronized is set to true.'
		SET @SQLStatement = '
			...
			WHERE 
				[ProductCategory].[Synchronized] <> 0 AND
				(
					[ProductCategory].[Description] <> Members.[Description] OR
					[ProductCategory].[HelpText] <> Members.[HelpText] OR
					' + CASE WHEN @MappingTypeID <> 0 THEN 'ISNULL([ProductCategory].[MemberKeyBase], '''') <> [Members].[MemberKeyBase] OR' ELSE '' END + '
					' + CASE WHEN @MappingTypeID <> 0 THEN 'ISNULL([ProductCategory].[Entity], '''') <> Members.[Entity] OR' ELSE '' END + '
					' + CASE WHEN @MappingTypeID <> 0 THEN 'ISNULL([ProductCategory].[Entity_MemberID], 0) <> [Members].[Entity_MemberID] OR' ELSE '' END + '
					' + CASE WHEN @NodeTypeBMYN <> 0 THEN  'ISNULL([ProductCategory].[NodeTypeBM], 0) <> [Members].[NodeTypeBM] OR [ProductCategory].[NodeTypeBM] IS NULL OR' ELSE '' END + '
					[ProductCategory].[Source] <> Members.[Source]
				)'
