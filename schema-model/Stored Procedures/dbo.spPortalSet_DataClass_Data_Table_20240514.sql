...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalSet_DataClass_Data_Table_20240514]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
BEGIN TRY
	...
	SET @Step = 'Set @Filter for FxTrans conversion'
		--Check Currency_MemberId
			...
			FROM
				TempDB.sys.tables t
				INNER JOIN TempDB.sys.columns c ON c.object_id = t.object_id AND c.[name] IN ('BusinessProcess_Memberid', 'Entity_Memberid', 'Scenario_Memberid', 'Time_Memberid', 'Currency_MemberId')
			WHERE
