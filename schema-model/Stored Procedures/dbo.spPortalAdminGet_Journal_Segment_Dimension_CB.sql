...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalAdminGet_Journal_Segment_Dimension_CB]
(
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
EXEC spRun_Procedure_KeyValuePair
	@ProcedureName = 'spPortalAdminGet_Journal_Segment_Dimension_CB',
	@JSON = '
		[
...
EXEC dbo.[spPortalAdminGet_Journal_Segment_Dimension_CB] @UserID = -10, @InstanceID = 404, @VersionID = 1003, @Debug = 1
EXEC dbo.[spPortalAdminGet_Journal_Segment_Dimension_CB] @GetVersion = 1
*/	
...
BEGIN TRY
	...
	SET @Step = 'Return list of Segment Dimensions'
		SELECT
			D.[DimensionID],
			D.[DimensionName]
		FROM
			Dimension D
			INNER JOIN [DataClass_Dimension] DCD ON DCD.InstanceID = D.InstanceID AND DCD.VersionID = @VersionID AND DCD.DimensionID = D.DimensionID 
			INNER JOIN [DataClass] DC ON DC.InstanceID = DCD.InstanceID AND DC.VersionID = @VersionID AND DC.DataClassID = DCD.DataClassID AND DC.ModelBM & 64 > 0 AND DC.SelectYN <> 0
		WHERE
