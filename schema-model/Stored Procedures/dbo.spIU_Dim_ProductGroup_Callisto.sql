...
CREATE PROCEDURE [dbo].[spIU_Dim_ProductGroup_Callisto]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
AS
/*
EXEC [spIU_Dim_ProductGroup_Callisto] @UserID = -10, @InstanceID = 531, @VersionID = 1057, @DebugBM = 1

EXEC [spIU_Dim_ProductGroup_Callisto] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Insert rows into Callisto ProductGroup tables from different sources.',
...
BEGIN TRY
	...
	SET @Step = 'Create temp tables'
		CREATE TABLE #ProductGroup_Members
			(
			[MemberId] BIGINT,
	...
	SET @Step = 'Fetch members'
		EXEC [spIU_Dim_ProductGroup_Raw] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @SourceTypeID=@SourceTypeID, @SourceDatabase=@SourceDatabase, @SequenceBMStep=@SequenceBMStep, @StaticMemberYN=@StaticMemberYN, @MappingTypeID=@MappingTypeID, @JobID=@JobID, @Debug=@DebugSub

		UPDATE #ProductGroup_Members 
		SET
			[Label] = [MemberKey], 
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#ProductGroup_Members', * FROM #ProductGroup_Members
	...
	SET @Step = 'Update Description and dimension specific Properties from source system where Synchronized is set to true.'
		SET @SQLStatement = '
			...
			FROM
				[' + @CallistoDatabase + '].[dbo].[S_DS_' + @DimensionName + '] [ProductGroup] 
				INNER JOIN [#ProductGroup_Members] Members ON Members.Label COLLATE DATABASE_DEFAULT = [ProductGroup].LABEL 
			WHERE 
				[ProductGroup].[Synchronized] <> 0 AND
				(
					[ProductGroup].[Description] <> Members.[Description] OR
					[ProductGroup].[HelpText] <> Members.[HelpText] OR
					' + CASE WHEN @MappingTypeID <> 0 THEN 'ISNULL([ProductGroup].[MemberKeyBase], '''') <> [Members].[MemberKeyBase] OR' ELSE '' END + '
					' + CASE WHEN @MappingTypeID <> 0 THEN 'ISNULL([ProductGroup].[Entity], '''') <> Members.[Entity] OR' ELSE '' END + '
					' + CASE WHEN @MappingTypeID <> 0 THEN 'ISNULL([ProductGroup].[Entity_MemberID], 0) <> [Members].[Entity_MemberID] OR' ELSE '' END + '
					' + CASE WHEN @NodeTypeBMYN <> 0 THEN  'ISNULL([ProductGroup].[NodeTypeBM], 0) <> [Members].[NodeTypeBM] OR [ProductGroup].[NodeTypeBM] IS NULL OR' ELSE '' END + '
					[ProductGroup].[Source] <> Members.[Source]
				)'
	...
	SET @Step = 'Insert new members from source system'
		SET @SQLStatement = '
			...
			SELECT
				...
				[Members].[Source]
			FROM   
				[#ProductGroup_Members] Members
			WHERE
				NOT EXISTS (SELECT 1 FROM [' + @CallistoDatabase + '].[dbo].[S_DS_' + @DimensionName + '] [ProductGroup] WHERE Members.Label = [ProductGroup].Label)'
	...
	SET @Step = 'Drop temp tables'
		DROP TABLE [#ProductGroup_Members]
		DROP TABLE [#EB]
