...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_AccountManager_Raw]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_Dim_AccountManager_Raw] @UserID = -10, @InstanceID = -1088, @VersionID = -1085, @SourceTypeID = 11, @DebugBM = 3
EXEC [spIU_Dim_AccountManager_Raw] @UserID = -10, @InstanceID = 529, @VersionID = 1001, @SourceTypeID = 3, @DebugBM = 3

EXEC [spIU_Dim_AccountManager_Raw] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Get AccountManagers from Source system',
		...
		IF @Version = '2.1.0.2162' SET @Description = 'Use @SourceDatabase parameter in the INSERT query of #AccountManager_Members_Raw.'
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		IF OBJECT_ID(N'TempDB.dbo.#AccountManager_Members', N'U') IS NULL SET @CalledYN = 0
	...
	SET @Step = 'Create temp table #AMTable'
	...
	SET @Step = 'Create temp table #AccountManager_Members_Raw'
		CREATE TABLE #AccountManager_Members_Raw
			(
			[MemberId] bigint,
	...
	SET @Step = 'Fill temptable #AccountManager_Members_Raw, Entity level'
		IF @MappingTypeID IN (1, 2)
			BEGIN
				INSERT INTO #AccountManager_Members_Raw
					(
					[MemberKey],
					[Description],
					[NodeTypeBM],
	...
	SET @Step = 'Fill temptable #AccountManager_Members_Raw'
		IF @SourceTypeID IN (1, 11) --Epicor ERP (E9, E10)
			BEGIN
				IF @SequenceBMStep & 1 > 0 --AccountManager Leafs
					BEGIN
						SET @SQLStatement = '
							INSERT INTO #AccountManager_Members_Raw
								(
		...
		IF @SourceTypeID IN (3) --iScala
			BEGIN
				SET @SQLStatement = '
					...
					WHERE
						wST.TableCode = ''ST01'' AND
						wST.SourceID = ' + CONVERT(nvarchar(15), @SourceID)

				IF @DebugBM & 2 > 0 PRINT @SQLStatement
				EXEC (@SQLStatement)

				IF @DebugBM & 2 > 0 SELECT [TempTable] = '#AMTable', * FROM #AMTable ORDER BY [Priority], [FiscalYear] DESC, [Entity_MemberKey]
							...
							IF @SequenceBMStep & 2 > 0 --Segment Leafs
								BEGIN
									SET @SQLStatement = '
										...
										SELECT DISTINCT
											[MemberId] = NULL,
											[MemberKey] = [ST01].[ST01001],
											[Description] = [ST01].[ST01002],
											[HelpText] = '''',
											[NodeTypeBM] = 1,
											[Source] = ''' + @SourceTypeName + ''',
										...
										FROM
											' + @TableName  + ' [ST01]
										WHERE
											[ST01].[ST01001] <> '''' AND [ST01].[ST01001] IS NOT NULL AND
											NOT EXISTS (SELECT 1 FROM #AccountManager_Members_Raw AMR WHERE AMR.[MemberKey] = [ST01].[ST01001] COLLATE DATABASE_DEFAULT AND AMR.[Entity] = ''' + @Entity + ''')'
	...
	SET @Step = 'Return data'
		IF @CalledYN = 0
			BEGIN
				SELECT
					TempTable = '#AccountManager_Members_Raw',
					*
				FROM
					#AccountManager_Members_Raw
				ORDER BY
				...
				DECLARE Account_Members_Cursor CURSOR FOR
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
							...
							INSERT INTO [#AccountManager_Members]
								(
								[MemberId],
								[MemberKey],
								[Description],
							...
							FROM
								...
								FROM
									[#AccountManager_Members_Raw] [Raw]
							...
							WHERE
								...
								NOT EXISTS (SELECT 1 FROM [#AccountManager_Members] AM WHERE AM.[MemberKey] = [MaxRaw].[MemberKey])
							ORDER BY
	...
	SET @Step = 'Drop the temp tables'
		DROP TABLE #AccountManager_Members_Raw
		DROP TABLE #AMTable
