...
SET ANSI_NULLS ON
GO
create PROCEDURE [dbo].[spIU_DC_Journal_EpicorERP_20250613_beforeCOACode]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'DB-1610: For @SequenceBM = 2 (Opening Balances), insert [JournalSequence] = ''OB_ERP'' rows for non-zero [GLPeriodBal].[OpenBalance] values only. DB-1620: Added Parameter @OB_ERP_InsertYN and use for @SequenceBM = 2. If set to 0, OB_ERP is not inserted, OB_ADJ will not be calculated; opening balances are calculated based from closing balances from previous Fiscal Period (OB_JRN). FDB-1726: For @SequenceBM = 2 (Opening Balances), revert to inserting [JournalSequence] = ''OB_ERP'' rows for [GLPeriodBal].[OpenBalance] zero values. Fix bug: @Owner instead of hardcodded "Erp". Calculate OpeningBalances even if #Journal does not have FP=1. Added @StartFiscalYear filter when insertig into [#GLJrnDtl] and @MaxSourceCounter IS NOT NULL. Updated to latest sp template. Added parameter @BalanceType_B_D_SumYN.'
