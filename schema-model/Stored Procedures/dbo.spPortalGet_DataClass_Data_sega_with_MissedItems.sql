...
SET ANSI_NULLS ON
GO
create PROCEDURE [dbo].[spPortalGet_DataClass_Data_sega_with_MissedItems]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2198' SET @Description = '1. Use renamed sp: [spPortalGet_CreateTableScript] instead of [sp_Tool_Generate_CreateTable]. Handle hierarchy calculation for @ResultTypeBM = 512. Handle Scenario [InputAllowedYN] in @ResultTypeBM = 8. DB-1442: Update [MultiDimIncludedYN] = 1 for [DimensionTypeID] = 27 from #FilterTable.'
		IF @Version = '2.1.2.2199' SET @Description = 'Use SP spPortalGet_MissedItems: checking existing Dimensions in DataClass before executing dynamic query'
...
--Always set to false
	...
	SET @Step = 'Checking: Does #GroupBy have the Dimensions which don''t exist in DataClass'
     ...
     --   left join #DimensionInfo DI on DI.DimensionName = TI.[Item]
        ...
        begin
            set @InfoMessage = 'Selected dimension(s): ' + @MissingItems_OUT + ' does(do) not exist in DataClass: ''' + @DataClassName + '''';
            THROW 51000, @InfoMessage, 2;
        end
	...
	SET @Step = '@ResultTypeBM & 12'
		IF @ResultTypeBM & 12 > 0
			BEGIN
				...
				IF @DebugBM & 2 > 0 PRINT @SQLStatement
/*
				-- Checking missed Dimensions in DataClass
                EXEC [spPortalGet_MissedItems] @UserID = @UserID, @InstanceID = @InstanceID, @VersionID = @VersionID, @DataClassID = @DataClassID, @SqlQuery = @SQLStatement, @MissingItems_OUT = @MissingItems_OUT OUTPUT
                IF @DebugBM & 2 > 0
                        select [@MissingItems_OUT] = @MissingItems_OUT
			    IF @MissingItems_OUT is not null
			        begin
                        set @InfoMessage = 'Selected dimension(s): ' + @MissingItems_OUT + ' does(do) not exist in DataClass: ''' + @DataClassName + '''';                        
				        THROW 51000, @InfoMessage, 2;
                    end
