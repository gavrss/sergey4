...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_Journal_CYNI_New2]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_DC_Journal_CYNI_New2] @UserID = -10, @InstanceID = 621, @VersionID = 1105, @Entity_MemberKey = 'A8', @Book = 'GL', @FiscalYear = 2024, @Debug = 1
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2199' SET @Description = 'Only include accounts inside parent Income_Statement_.'
...
BEGIN TRY
	...
	SET @Step = 'Create temp table #IncomeStatement'
		CREATE TABLE #IncomeStatement
			(
			Account nvarchar(50) COLLATE DATABASE_DEFAULT
			)

		CREATE UNIQUE NONCLUSTERED INDEX [Account_idx] ON [#IncomeStatement]
			(
				[Account] ASC
			)

	EXEC spGet_LeafLevelFilter @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @DatabaseName=@CallistoDatabase, @DimensionName='Account', @Filter= 'Income_Statement_', @StorageTypeBM=4, @UseCacheYN=0, @StorageTypeBM_DataClass=4, @LeafLevelFilter=@LeafLevelFilter OUT
	...
	EXEC (@SQLStatement)
	IF @Debug <> 0 SELECT TempTable = '#IncomeStatement', * FROM #IncomeStatement ORDER BY [Account]
					...
					DECLARE FiscalYear_CYNI_Cursor CURSOR FOR
						...
						WHILE @@FETCH_STATUS = 0
							BEGIN
								...
								FROM
									' + CASE WHEN @SequenceBM & 2 > 0 THEN '#Journal' ELSE @JournalTable END + ' J
									INNER JOIN #IncomeStatement IncSt ON IncSt.[Account] = J.[Account]
								WHERE
