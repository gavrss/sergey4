...
SET ANSI_NULLS ON
GO
create PROCEDURE [dbo].[spBR_BR14_20220806]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spBR_BR14] @UserID=-10, @InstanceID=527, @VersionID=1055, @BusinessRuleID = 2329, @DebugBM = 3
...
BEGIN TRY
	...
	SET @Step = 'Run BR14_Cursor'
		...
		DECLARE BR14_Cursor CURSOR FOR
			...
			OPEN BR14_Cursor
			FETCH NEXT FROM BR14_Cursor INTO @ICCounterpart, @Method, @DimensionFilter

			WHILE @@FETCH_STATUS = 0
				BEGIN
					IF @DebugBM & 2 > 0 SELECT [@ICCounterpart]=@ICCounterpart, [@Method] = @Method, [@DimensionFilter]=@DimensionFilter
					...
					IF @Method = 'Property'
						BEGIN
							...
							SELECT
								...
								[Segment] = NULL,
								[DimensionName] = LEFT(@ICCounterpart, CHARINDEX('.', @ICCounterpart) -1),
								[HierarchyName] = NULL,
								[PropertyName] = NULL,
								[EqualityString] = NULL,
								[Filter] = NULL,
								...
								[Method] = 'Property'
							WHERE
								NOT EXISTS (SELECT 1 FROM #FilterTable FT WHERE FT.[StepReference] = 'BR14' AND FT.[DimensionName] = LEFT(@ICCounterpart, CHARINDEX('.', @ICCounterpart) -1))
					...
					IF @DebugBM & 2 > 0 SELECT TempTable = '#FilterTable', * FROM #FilterTable WHERE [StepReference] = 'BR14'
					...
					SET @SQLFilter = ''
					SELECT
						@SQLFilter = @SQLFilter + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'J.[' + CASE WHEN FT.DimensionTypeID = -1 THEN FT.[Segment] ELSE FT.[DimensionName] END + '] IN(' + [LeafLevelFilter] + ') AND'
					FROM
						#FilterTable FT
					WHERE
						FT.[StepReference] = 'BR14' AND
						LEN(FT.[LeafLevelFilter]) > 0 AND
						NOT (FT.[DimensionTypeID] = -1 AND FT.[Segment] IS NULL)
					ORDER BY
						DimensionID
					...
					DECLARE Property_Cursor CURSOR FOR
						...
						FROM
							#FilterTable
						WHERE
							[StepReference] = 'BR14' AND
							LEN(PropertyName) > 0 OR
							[Method] = 'Property'
						ORDER BY
							[DimensionName]

						OPEN Property_Cursor
						FETCH NEXT FROM Property_Cursor INTO @PropertyDimension, @Segment, @DimMethod, @PropertyLen

						WHILE @@FETCH_STATUS = 0
							BEGIN
								IF @DebugBM & 2 > 0 SELECT [@PropertyDimension] = @PropertyDimension, [@Segment] = @Segment, [@DimMethod] = @DimMethod, [@PropertyLen] = @PropertyLen

								IF @DimMethod = 'Property' AND @PropertyLen = 0
									BEGIN
										SET @SQLJoin = @SQLJoin + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'INNER JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_' + @PropertyDimension + '] [' + @PropertyDimension + '] ON [' + @PropertyDimension + '].[Label] = J.[' + ISNULL(@Segment, @PropertyDimension) + ']'
									END
								ELSE
									BEGIN
										SET @SQLJoin = @SQLJoin + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + 'INNER JOIN [' + @CallistoDatabase + '].[dbo].[S_DS_' + @PropertyDimension + '] [' + @PropertyDimension + '] ON [' + @PropertyDimension + '].[Label] = J.[' + @PropertyDimension + ']'
						...
						BEGIN
							...
							SET @SQLStatement = '
								...
								FROM
									' + @JournalTable + ' J
									' + @SQLJoin + '
									' + CASE WHEN @InstanceID = 572 THEN 'INNER JOIN pcDATA_BRAD1..[S_DS_GL_Branch] [GL_Branch] ON [GL_Branch].[Label] = J.[Segment02]' ELSE '' END + '
...
--					IF RIGHT(RTRIM(@SQLStatement), 4) = ' AND ' SET @SQLStatement = LEFT(RTRIM(@SQLStatement), LEN(RTRIM(@SQLStatement)) - 4)
