...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_Sales_Callisto]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
EXEC [spIU_DC_Sales_Callisto] @UserID=-10, @InstanceID=907, @VersionID=1313, @StartDay= 20250401, @DebugBM=11
...
BEGIN TRY
	...
	SET @Step = 'Set procedure variables'
		...
		SET @Step = 'Create temp table #MemberSelection'
	...
	SET @Step = 'Fill temp table #DC_Sales_Raw'
		...
		EXEC [pcINTEGRATOR].[dbo].[spRun_Procedure_KeyValuePair] @DatabaseName = @DatabaseName, @ProcedureName = 'spIU_DC_Sales_Raw', @JSON = @JSON
	...
	SET @Step = 'Insert into temp table #DC_Sales'
		...
		SELECT
			@SQLInsertInto = @SQLInsertInto + CASE WHEN c.name = 'DataClassID' THEN '' ELSE CHAR(13) + CHAR(10) + CHAR(9) + '[' + c.name + '],' END,
			@SQLSelect =  @SQLSelect + CASE WHEN c.name = 'Sales_Value' THEN '' 
											WHEN c.name = 'DataClassID'	THEN ''
											ELSE CHAR(13) + CHAR(10) + CHAR(9) + '[' + c.name + '] = ' +
										CASE WHEN c.name = 'Currency_MemberId' THEN 'CASE WHEN (MS.[Label] LIKE ''SalesC%'') THEN [Entity].[Currency_MemberId] ELSE ISNULL([Document_Currency].[MemberId], -1) END,' 
											WHEN c.name = 'AccountManager_MemberId' THEN 'ISNULL([Customer].[AccountManager_MemberId] , -1 ),'  
											WHEN c.name = 'ProductCategory_MemberId' THEN 'ISNULL([Product].[ProductCategory_MemberId] , -1 ),' 
											WHEN c.name = 'ProductGroup_MemberId' THEN 'ISNULL([Product].[ProductGroup_MemberId] , -1 ),' 
											WHEN c.name = 'CustomerCategory_MemberId' THEN 'ISNULL([Customer].[CustomerCategory_MemberId] , -1 ),'
											--WHEN c.name = 'DataClassID' THEN '1,'
											WHEN c.name LIKE '%_MemberId' THEN 'ISNULL([' + REPLACE(c.name, '_MemberId', '') + '].[MemberId], -1),' 
											ELSE '' 
										END
									END,
			@SQLDimJoin = @SQLDimJoin + CASE WHEN c.name IN ('Account_MemberId', 'BusinessProcess_MemberId', 'Currency_MemberId', 'AccountManager_MemberId', 'ProductCategory_MemberId', 'ProductGroup_MemberId', 'CustomerCategory_MemberId') THEN '' ELSE CHAR(13) + CHAR(10) + CHAR(9) + 
											CASE WHEN c.name LIKE '%_MemberId' THEN 'LEFT JOIN ' + @CallistoDatabase + '.[dbo].[S_DS_' + REPLACE(c.name, '_MemberId', '') + '] [' + REPLACE(c.name, '_MemberId', '') + '] ON [' + REPLACE(c.name, '_MemberId', '') + '].Label COLLATE DATABASE_DEFAULT = [Raw].[' + REPLACE(c.name, '_MemberId', '') + ']' ELSE '' 
											END 
										END,
			@SQLGroupBy = @SQLGroupBy + CASE WHEN c.name = 'Sales_Value' THEN '' 
											WHEN c.name = 'DataClassID'	THEN ''
											ELSE CHAR(13) + CHAR(10) + CHAR(9) + 
												CASE WHEN c.name = 'Currency_MemberId' THEN 'CASE WHEN (MS.[Label] LIKE ''SalesC%'') THEN [Entity].[Currency_MemberId] ELSE ISNULL([Document_Currency].[MemberId], -1) END,'  
													WHEN c.name = 'AccountManager_MemberId' THEN 'ISNULL([Customer].[AccountManager_MemberId] , -1 ),'  
													WHEN c.name = 'ProductCategory_MemberId' THEN 'ISNULL([Product].[ProductCategory_MemberId] , -1 ),' 
													WHEN c.name = 'ProductGroup_MemberId' THEN 'ISNULL([Product].[ProductGroup_MemberId] , -1 ),' 
		...
		SET @SQLStatement = '
			...
			FROM
				...
				LEFT JOIN ' + @CallistoDatabase + '.[dbo].[S_DS_Currency] [Document_Currency] ON [Document_Currency].[Label] COLLATE DATABASE_DEFAULT = CASE WHEN MS.Unit = ''Currency'' THEN [Raw].[Document_Currency] ELSE ''NONE'' END
		...
		--LEFT JOIN ' + @CallistoDatabase + '.[dbo].[S_DS_Currency] [Entity_Currency] ON [Entity_Currency].[Label] COLLATE DATABASE_DEFAULT = [Raw].[Entity_Currency]
		...
		IF @DebugBM & 2 > 0 AND LEN(@SQLStatement) > 4000 
				BEGIN
					PRINT 'Length of @SQLStatement more than 4000, see pcINTEGRATOR_Log..wrk_Debug; INSERT INTO #DC_Sales'
					SET @Comment = 'INSERT INTO #DC_Sales'
	...
	SET @Step = 'Calculate Fx'
		...
		IF @DebugBM & 2 > 0 PRINT @JSON
		EXEC [pcINTEGRATOR].[dbo].[spRun_Procedure_KeyValuePair] @ProcedureName = 'spBR_BR04', @JSON = @JSON
	...
	SET @Step = 'Set CurrentOrderState'
		...
		IF @DebugBM & 2 > 0 PRINT @JSON
		EXEC [pcINTEGRATOR].[dbo].[spRun_Procedure_KeyValuePair] @ProcedureName = 'spIU_DC_Sales_CurrentOrderState', @JSON = @JSON
