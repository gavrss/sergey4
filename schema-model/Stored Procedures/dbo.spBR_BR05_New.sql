...
CREATE PROCEDURE [dbo].[spBR_BR05_New]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
	...
	@SequenceBM int = 511, --1=InterCompany Counterpart, 2=HistRate, 4=Retained earnings, 8=Copy open, 16=Consolidation before FX, 32=FX, 64=Consolidation after FX, 128=Into Journal, 256=Into FACT_Financials, 512=Refresh Callisto cube
...
EXEC [spBR_BR05] @UserID=-10, @InstanceID=527, @VersionID=1055, @BusinessRuleID = 1001, @FiscalYear = 2021, @ConsolidationGroup = 'Group'--, @Entity_MemberKey = 'GGI03', @DebugBM=18
...
EXEC pcINTEGRATOR..[spBR_BR05] @UserID=-10, @InstanceID=572, @VersionID=1080, @BusinessRuleID = 2587, @ConsolidationGroup = 'G_BRADKEN', @FiscalYear = 2019

EXEC [spBR_BR05] @UserID=-10, @InstanceID=576, @VersionID=1082, @BusinessRuleID = 2717, @FiscalYear = 2019, @ConsolidationGroup = 'G_AXYZ', @DebugBM=16 --, @Entity_MemberKey = '101'
EXEC [spBR_BR05] @UserID=-10, @InstanceID=576, @VersionID=1082, @BusinessRuleID = 2717, @FiscalYear = 2020, @ConsolidationGroup = 'G_AXYZ', @DebugBM=16 --, @Entity_MemberKey = '101'
EXEC [spBR_BR05] @UserID=-10, @InstanceID=576, @VersionID=1082, @BusinessRuleID = 2717, @FiscalYear = 2021, @ConsolidationGroup = 'G_AXYZ', @DebugBM=16
EXEC [spBR_BR05] @UserID=-10, @InstanceID=576, @VersionID=1082, @BusinessRuleID = 2717, @FiscalYear = 2022, @ConsolidationGroup = 'G_AXYZ', @DebugBM=16
EXEC [spBR_BR05] @UserID=-10, @InstanceID=576, @VersionID=1082, @BusinessRuleID = 2717, @FiscalYear = 2021, @ConsolidationGroup = 'G_AXYZ', @DebugBM=16, @Entity_MemberKey = '104'

EXEC [spBR_BR05] @UserID=-10, @InstanceID=580, @VersionID=1084, @BusinessRuleID = 2740, @FiscalYear = 2022, @ConsolidationGroup = 'G_WWL', @DebugBM=16, @Entity_MemberKey = 'TnMat'

EXEC [pcINTEGRATOR].[dbo].[spBR_BR05_New] @BusinessRuleID=N'2822',@ConsolidationGroup=N'G_PCX',@Debug=N'1',@FiscalYear=N'2023',@FxRate_Scenario=N'ACTUAL',@InstanceID=N'603',@Scenario=N'ACTUAL',@UserID=N'9920',@VersionID=N'1095', @Entity_MemberKey = 'PCXHC'
...
DECLARE
	...
	@Journal_DataClassID int = NULL,
	@Financials_DataClassID int = NULL,
	@JSON nvarchar(max),
	@FromTime int,
	@ToTime int,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2187' SET @Description = 'Handle multiple scenarios and dataclass sources. Calculate Retained earnings before HistRate. Run configured BusinessRules after calculation.'
		IF @Version = '2.1.2.2190' SET @Description = 'Refined cursors.'
		IF @Version = '2.1.2.2191' SET @Description = 'New structure where HistRate is not used anymore'
		IF @Version = '2.1.2.2192' SET @Description = 'Added variables @FromTime and @ToTime'
		IF @Version = '2.1.2.2199' SET @Description = 'Added PYNI_B as column in #EntityBook, filter on [ValidFrom] and [ValidTo] in table [EntityHierarchy] when filling temp table #EntityBook.'
...
--SELECT * FROM pcINTEGRATOR_Data..DataClass WHERE InstanceID = 527
	...
	SET @Step = 'Set procedure variables 2'
		...
		--SET @StartFiscalYear = ISNULL(@StartFiscalYear @MasterClosedFiscalYear + 1
	...
	SET @Step = 'Run ConsolidationGroup Cursor'
		...
		FROM
			...
			INNER JOIN Entity_Book EB ON EB.InstanceID = E.InstanceID AND EB.VersionID = E.VersionID AND EB.EntityID = E.EntityID AND EB.BookTypeBM & 16 > 0 AND EB.Currency IS NOT NULL AND EB.SelectYN <> 0
		WHERE
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					...
					SET @Step = 'Update #FiscalPeriod'
						...
						EXEC dbo.[spGet_Entity_FiscalYear] @UserID = @UserID, @InstanceID = @InstanceID, @VersionID = @VersionID, @EntityID = @EntityGroupID, @Book = @EntityGroupBook, @StartFiscalYear = @StartFiscalYear, @EndFiscalYear = @EndFiscalYear,  @FiscalYear = @FiscalYear, @FiscalPeriod0YN = 1, @FiscalPeriod13YN = 1, @JobID = @JobID, @Debug=@DebugSub

						SELECT
							@FromTime = MIN(YearMonth),
							@ToTime = MAX(YearMonth)
						FROM
							#FiscalPeriod
						WHERE
							FiscalYear = @FiscalYear
						...
						IF @DebugBM & 2 > 0
							BEGIN
								SELECT TempTable = '#FiscalPeriod', * FROM #FiscalPeriod ORDER BY [FiscalYear], [FiscalPeriod], [YearMonth]
								SELECT [@FromTime] = @FromTime, [@ToTime] = @ToTime
							END
					...
					SELECT
						...
						[FirstFiscalYearYN] = CASE WHEN Year(EH.[ValidFrom]) * 100 + Month(EH.[ValidFrom]) BETWEEN @FromTime AND @ToTime THEN 1 ELSE 0 END
					FROM
						Entity E
						...
						INNER JOIN EntityHierarchy EH ON EH.[InstanceID] = E.[InstanceID] AND EH.[VersionID] = E.[VersionID] AND EH.[EntityGroupID] = @EntityGroupID AND EH.[EntityID] = E.[EntityID] AND Year(EH.[ValidFrom]) * 100 + Month(EH.[ValidFrom]) <= @FromTime AND ISNULL(Year(EH.[ValidTo]) * 100 + Month(EH.[ValidTo]), 209912) >= @ToTime
					WHERE
						E.[InstanceID] = @InstanceID AND
						E.[VersionID] = @VersionID AND
						E.[SelectYN] <> 0 AND
					...
					UPDATE EB
					SET
						[PYNI_B_YN] = EPV.[EntityPropertyValue]
					FROM
						#EntityBook EB
						INNER JOIN [pcINTEGRATOR_Data].[dbo].[EntityPropertyValue] EPV ON EPV.[InstanceID] = @InstanceID AND EPV.[VersionID] = @VersionID AND EPV.[EntityID] = EB.[EntityID] AND EPV.[EntityPropertyTypeID] = -13 AND EPV.[SelectYN] <> 0
									...
									SET @Step = 'Insert Copy Open'
										IF @SequenceBM & 8 > 0
											BEGIN
												...
												IF (SELECT COUNT(1) FROM #EntityBook WHERE [SourceDataClassID] = @Journal_DataClassID AND [SelectYN] <> 0) > 0
													BEGIN
														SET @JSON = '
															[
															...
															{"TKey" : "ConsolidationGroup",  "TValue": "' + @ConsolidationGroup + '"},
															{"TKey" : "Scenario",  "TValue": "' + @Scenario + '"},
															{"TKey" : "JobID",  "TValue": "' + CONVERT(nvarchar(10), @JobID) + '"},
															{"TKey" : "Debug",  "TValue": "' + CONVERT(nvarchar(10), @DebugSub) + '"}'
															+ CASE WHEN @JournalTable IS NULL THEN '' ELSE ',' + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '{"TKey" : "JournalTable",  "TValue": "' + @JournalTable + '"}' END +
															+ CASE WHEN @CallistoDatabase IS NULL THEN '' ELSE ',' + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '{"TKey" : "CallistoDatabase",  "TValue": "' + @CallistoDatabase + '"}' END +
															']'

														IF @DebugBM & 2 > 0 PRINT @JSON

														EXEC [pcINTEGRATOR]..spRun_Procedure_KeyValuePair @ProcedureName = 'spBR_BR05_CopyOpen', @JSON = @JSON

													END

												IF (SELECT COUNT(1) FROM #EntityBook WHERE [SourceDataClassID] = @Financials_DataClassID AND [SelectYN] <> 0) > 0
													BEGIN
														SET @JSON = '
															[
															{"TKey" : "UserID",  "TValue": "' + CONVERT(nvarchar(10), @UserID) + '"},
														...
														EXEC [pcINTEGRATOR]..spRun_Procedure_KeyValuePair @ProcedureName = 'spBR_BR05_CopyOpen_Financials', @JSON = @JSON
												...
												IF @DebugBM & 32 > 0
													...
													--INTO [pcINTEGRATOR_Log].[dbo].[wrk_Debug_JournalBase]
													FROM #JournalBase
											END
										...
										IF @SequenceBM & 32 > 0 AND (SELECT COUNT(1) FROM #EntityBook EB WHERE EB.[SelectYN] <> 0) > 0 --EB.[Currency] <> @GroupCurrency AND
											BEGIN
												...
												SET @JSON = '
													...
													{"TKey" : "BusinessRuleID",  "TValue": "' + CONVERT(nvarchar(15), @BusinessRuleID) + '"},
													{"TKey" : "CallistoDatabase",  "TValue": "' + @CallistoDatabase + '"},
													{"TKey" : "GroupCurrency",  "TValue": "' + @GroupCurrency + '"},
													{"TKey" : "FiscalYear",  "TValue": "' + CONVERT(nvarchar(15), @FiscalYear) + '"},
													{"TKey" : "Scenario",  "TValue": "' + @FxRate_Scenario + '"},
													{"TKey" : "ConsolidationGroup",  "TValue": "' + @ConsolidationGroup + '"},
													{"TKey" : "Level",  "TValue": "' + @Level + '"},
													{"TKey" : "JobID",  "TValue": "' + CONVERT(nvarchar(10), @JobID) + '"},
													{"TKey" : "Debug",  "TValue": "' + CONVERT(nvarchar(10), @DebugSub) + '"}
													]'
												...
												EXEC [pcINTEGRATOR]..spRun_Procedure_KeyValuePair @ProcedureName = 'spBR_BR05_Fx', @JSON = @JSON
												...
												IF @InterCompany_BusinessRuleID IS NOT NULL
													BEGIN
														SET @JSON = '
															...
															{"TKey" : "InstanceID",  "TValue": "' + CONVERT(nvarchar(15), @InstanceID) + '"},
															{"TKey" : "VersionID",  "TValue": "' + CONVERT(nvarchar(15), @VersionID) + '"},
															{"TKey" : "BusinessRuleID",  "TValue": "' + CONVERT(nvarchar(15), @InterCompany_BusinessRuleID) + '"},
															{"TKey" : "FiscalYear",  "TValue": "' + CONVERT(nvarchar(15), @FiscalYear) + '"},
															{"TKey" : "JournalTable",  "TValue": "#JournalBase"},
															{"TKey" : "JobID",  "TValue": "' + CONVERT(nvarchar(10), @JobID) + '"},
															{"TKey" : "Debug",  "TValue": "' + CONVERT(nvarchar(10), 3) + '"}
															]'

														IF @DebugBM & 2 > 0 PRINT @JSON

														EXEC [pcINTEGRATOR]..spRun_Procedure_KeyValuePair @ProcedureName = 'spBR_BR14', @JSON = @JSON

													--IF @DebugBM & 32 > 0
														SELECT 'Arne1'
														SELECT TOP (1000)
															TempTable='#JournalBase InterCompany', *
														FROM #JournalBase
														WHERE
															Entity = '103' AND
															Segment05 = '30' AND
															--TransactionTypeBM & 24 > 0 AND
															--JournalSequence = 'G_GROUPADJ' AND
															--Account IN ('1750', '4000') AND FiscalPeriod = 8
															1=1
														SELECT 'Arne2'

													END
												IF @DebugBM & 8 > 0 SELECT TempTable='#JournalBase InterCompany', * FROM #JournalBase WHERE ISNULL(InterCompanyEntity, 'NONE') <> 'NONE' ORDER BY YearMonth, Account
												...
												IF @DebugBM & 2 > 0
													BEGIN
														IF LEN(@SQLStatement) > 4000
															BEGIN
																PRINT 'Length of @SQLStatement more than 4000, see pcINTEGRATOR_Log..wrk_Debug; Add TransactionTypeBM = 16 to #JournalBase.'
																EXEC [dbo].[spSet_wrk_Debug]
																	...
																	@Comment = 'Add TransactionTypeBM = 16 to #JournalBase',
...
--Test start
	SET @Step = 'Fill temp table #CopyOpen_Rule_Cursor_Table'
		...
		IF OBJECT_ID('tempdb..#CopyOpen_Rule_Cursor_Table') IS NOT NULL DROP TABLE #CopyOpen_Rule_Cursor_Table;
...
*/
	...
	SET @Step = 'Set Rule_ConsolidationID and Rule_FXID'
		...
		SELECT TempTable = '#JournalBase', * FROM #JournalBase WHERE Account = '4001' ORDER BY InterCompanyEntity, Entity, Book, Account, Segment01, Segment02, Segment03, Segment04, Segment05, FiscalYear, FiscalPeriod, YearMonth
		...
		--SELECT TempTable = '#JournalBase', * FROM #JournalBase WHERE Account = '4001' ORDER BY InterCompanyEntity, Entity, Book, Account, Segment01, Segment02, Segment03, Segment04, Segment05, FiscalYear, FiscalPeriod, YearMonth
									...
									SET @Step = 'Convert TransactionTypeBM = 32 to 8'
												...
												SET @SQLStatement = @SQLStatement + '
													...
													WHERE
														ISNULL(sub.[Value_Book], 0) + ISNULL(sub.[Value_Group], 0) + ISNULL(sub.[Value_Transaction], 0) <> 0'
--														sub.[Value_Book] + sub.[Value_Group] + sub.[Value_Transaction] <> 0'
				...
				/*	Temporary removed 2022-10-24 by JaWo
	...
	SET @Step = 'Run configured business rules after Advanced Consolidation'
		IF @Misc_BusinessRuleYN <> 0
			EXEC [dbo].[spIU_Misc_BusinessRule]
				@UserID = @UserID,
				@InstanceID = @InstanceID,
				@VersionID = @VersionID,
				@EventTypeID = 5,
				...
				@ToTime = @ToTime,
				@CallistoRefreshYN = 0,
				@CallistoRefreshAsynchronousYN = 1,
				@JobID = @JobID,
