...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_Dimension_Generic]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_Dim_Dimension_Generic] @UserID = -10, @InstanceID = -1318, @VersionID = -1256, @DimensionID = -64, @DebugBM = 1 --GL_Posted
EXEC [spIU_Dim_Dimension_Generic] @UserID = -10, @InstanceID = -1318, @VersionID = -1256, @DimensionID = -3, @DebugBM = 1 --Currency

EXEC [spIU_Dim_Dimension_Generic] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Insert rows into selected dimension table.',
...
BEGIN TRY
	...
	SET @Step = 'Fetch members'
		EXEC [spIU_Dim_Dimension_Generic_Raw] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @DimensionID=@DimensionID, @DimensionName=@DimensionName, @Debug = @DebugSub
	...
	SET @Step = 'Update Description and dimension specific Properties from source system where Synchronized is set to true.'
		SET @SQLStatement = '
			...
			FROM
				...
				INNER JOIN [#Dimension_Generic_Members] [Members] ON [Members].[MemberKey] = [' + @DimensionName + '].[MemberKey] 
			WHERE 
	...
	SET @Step = 'Update default hierarchy'
		SET @SQLStatement = '		
			...
			FROM
				#Dimension_Generic_Members T1
				LEFT JOIN #Dimension_Generic_Members T2 ON T2.[MemberKey] = T1.[Parent]
		...
		SET @SQLStatement = '
			...
			FROM
				[' + @ETLDatabase + '].[dbo].[pcD_' + @DimensionName + '] [' + @DimensionName + '] 
				INNER JOIN [#Dimension_Generic_Members] [Members] ON [Members].[MemberKey] = [' + @DimensionName + '].[MemberKey] AND [Members].[ParentMemberID] IS NOT NULL
			WHERE 
				[' + @DimensionName + '].[Synchronized] <> 0 AND
