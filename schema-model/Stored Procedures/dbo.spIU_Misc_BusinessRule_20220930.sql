...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Misc_BusinessRule_20220930]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
BEGIN TRY
	...
	SET @Step = 'Fill table #BusinessRule_Cursor'
		...
		FROM
		 ...
		 WHERE
			...
			BRE.[EventTypeID] = @EventTypeID AND
			BRE.[SelectYN] <> 0 AND
	...
	SET @Step = 'Run Callisto Refresh on all DataClasses'
		IF @CallistoRefreshYN <> 0 AND (SELECT COUNT(1) FROM #BusinessRule_Cursor) > 0
			BEGIN
