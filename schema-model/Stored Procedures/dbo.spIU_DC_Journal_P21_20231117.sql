...
CREATE PROCEDURE [dbo].[spIU_DC_Journal_P21_20231117]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_DC_Journal_P21] @UserID = -10, @InstanceID = 515, @VersionID = 1064, @SequenceBM = 2, @Entity_MemberKey = 'REM', @Book = GL, @FiscalYear = 2022, @FullReloadYN = 1, @DebugBM = 31
...
--						[Segment02] = ISNULL(CONVERT(nvarchar(50), [IL].[product_group_id]), ''''),
						...
						[Supplier] = CONVERT(nvarchar(50), CONVERT(int, AP.[vendor_id])),
				...
				IF @DebugBM & 16 > 0 SELECT [Step] = 'After Insert @SequenceBM = 1', [TimeConsumed] = CONVERT(TIME(7), GETDATE() - @StartTime)

				EXEC [dbo].[spIU_DC_Journal_RemoveDuplicate]
					@UserID = @UserID,
					@InstanceID = @InstanceID,
					@VersionID = @VersionID,
					@JobID = @JobID,
				...
				IF @DebugBM & 16 > 0 SELECT [Step] = 'After Remove duplicates @SequenceBM = 1', [TimeConsumed] = CONVERT(TIME(7), GETDATE() - @StartTime)
			END
		...
		IF @SequenceBM & 2 > 0 AND (SELECT COUNT(1) FROM #FiscalPeriod WHERE FiscalPeriod = 0) > 0 
			...
			BEGIN
				...
				FROM
					...
					SELECT
						B.[account_no],
						YearMonth = CONVERT(int, MAX([year_for_period] * 100 + [period]))
					FROM
						[DSPSOURCE01].[pcSource_PSSTP2].[dbo].[Balances] B
						INNER JOIN #BalanceAccount BA ON BA.Account = SUBSTRING(B.[account_no], 1, 4) + '-' + SUBSTRING(B.[account_no], 5, 4) AND BA.[BalanceYN] <> 0
					WHERE
						[company_no] = @Entity_MemberKey AND
						[year_for_period] * 100 + [period] <= @FiscalYear * 100
					GROUP BY
						[account_no]
					) sub
				...
				IF @DebugBM & 2 > 0 SELECT TempTable = '#MaxPeriod', * FROM #MaxPeriod
					...
					SELECT
						...
						[JournalNo] = '0',
						[JournalLine] = 0,
						[YearMonth] = @FiscalYear * 100 + 1,
						[TransactionTypeBM] = 4,
						[BalanceYN] = 1,
						[Account] = SUBSTRING(B.[account_no], 1, 4) + '-' + SUBSTRING(B.[account_no], 5, 4),
						[Segment01] = ISNULL(CONVERT(nvarchar(50), SUBSTRING(B.[account_no], 9, 4)), ''),
						[JournalDate] = CONVERT(datetime, CONVERT(nvarchar(4), @FiscalYear) + '0101', 112),
						[TransactionDate] = CONVERT(datetime, CONVERT(nvarchar(4), @FiscalYear) + '0101', 112),
						[PostedDate] = CONVERT(datetime, CONVERT(nvarchar(4), @FiscalYear) + '0101', 112),
						[PostedStatus] = 1,
						[PostedBy] = '',
						[Source] = 'P21',
						[Scenario] = 'ACTUAL',
						...
						[Currency_Book] = 'USD',
						[ValueDebit_Book] = CASE WHEN B.[cumulative_balance] > 0 THEN B.[cumulative_balance] ELSE 0 END,
						[ValueCredit_Book] = CASE WHEN B.[cumulative_balance] < 0 THEN B.[cumulative_balance] * -1 ELSE 0 END
					FROM
						[DSPSOURCE01].[pcSource_PSSTP2].[dbo].[Balances] B
						INNER JOIN #MaxPeriod MP ON MP.[account_no] = B.[account_no] AND MP.[year_for_period] = B.[year_for_period] AND MP.[Period] = B.[period]
						--INNER JOIN #FiscalPeriod FP ON FP.[ClosingFiscalYear] = B.[year_for_period] AND FP.[ClosingFiscalPeriod] = B.[period]
						--INNER JOIN #BalanceAccount BA ON BA.Account = SUBSTRING(B.[account_no], 1, 4) + '-' + SUBSTRING(B.[account_no], 5, 4) AND BA.[BalanceYN] <> 0
					WHERE
				...
				--SET @SequenceOB = CASE WHEN ISNULL(@StartFiscalYear, @FiscalYear) < (SELECT MIN(FiscalYear) FROM #FiscalPeriod) OR @StartFiscalYear = @FiscalYear THEN 2 ELSE 0 END
