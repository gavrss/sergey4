...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_Dim_AccountManager_Callisto]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_Dim_AccountManager_Callisto] @UserID = -10, @InstanceID = 454, @VersionID = 1021, @DebugBM = 1
EXEC [spIU_Dim_AccountManager_Callisto] @UserID = -10, @InstanceID = 529, @VersionID = 1001, @DebugBM = 7
EXEC [spIU_Dim_AccountManager_Callisto] @UserID = -10, @InstanceID = 531, @VersionID = 1041, @DebugBM=7 --PCX

EXEC [spIU_Dim_AccountManager_Callisto] @GetVersion = 1
*/
...
IF @GetVersion <> 0
	BEGIN
		SELECT
			...
			@ProcedureDescription = 'Insert rows into Callisto AccountManager tables from different sources.',
...
BEGIN TRY
	...
	SET @Step = 'Create temp tables'
		CREATE TABLE #AccountManager_Members
			(
			[MemberId] bigint,
	...
	SET @Step = 'Fetch members'
		EXEC [spIU_Dim_AccountManager_Raw] @UserID=@UserID, @InstanceID=@InstanceID, @VersionID=@VersionID, @SourceTypeID=@SourceTypeID, @SourceDatabase=@SourceDatabase, @ETLDatabase=@ETLDatabase, @SequenceBMStep=@SequenceBMStep, @StaticMemberYN=@StaticMemberYN, @MappingTypeID=@MappingTypeID, @JobID=@JobID, @Debug=@DebugSub

		UPDATE #AccountManager_Members 
		SET
			[Label] = [MemberKey], 
		...
		IF @DebugBM & 2 > 0 SELECT TempTable = '#AccountManager_Members', * FROM #AccountManager_Members
	...
	SET @Step = 'Update Description and dimension specific Properties from source system where Synchronized is set to true.'
		SET @SQLStatement = '
			...
			FROM
				[' + @CallistoDatabase + '].[dbo].[S_DS_' + @DimensionName + '] [AccountManager] 
				INNER JOIN [#AccountManager_Members] Members ON Members.Label COLLATE DATABASE_DEFAULT = [AccountManager].LABEL 
			WHERE 
				[AccountManager].[Synchronized] <> 0'
	...
	SET @Step = 'Insert new members from source system'
		SET @SQLStatement = '
			...
			SELECT
				...
				[Synchronized]
			FROM   
				[#AccountManager_Members] Members
			WHERE
				NOT EXISTS (SELECT 1 FROM [' + @CallistoDatabase + '].[dbo].[S_DS_' + @DimensionName + '] [AccountManager] WHERE Members.Label = [AccountManager].Label)'
	...
	SET @Step = 'Drop temp tables'
		DROP TABLE [#AccountManager_Members]
		DROP TABLE [#EB]
