...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spIU_DC_Sales_Callisto_20250708]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
...
AS
/*
EXEC [spIU_DC_Sales_Callisto] @UserID=-10, @InstanceID=-1539, @VersionID=-1539,@DebugBM=7

EXEC [spIU_DC_Sales_Callisto] @UserID=-10, @InstanceID=-1089, @VersionID=-1086, @StartDay= 20200101, @SequenceBM=32, @DebugBM=15
EXEC [spIU_DC_Sales_Callisto] @UserID=-10, @InstanceID=454, @VersionID=1021, @StartDay= 20200901, @SequenceBM=15, @Entity = 'R510', @DebugBM=2

EXEC [spIU_DC_Sales_Callisto] @GetVersion = 1
*/
...
BEGIN TRY
	...
	SET @Step = 'Insert into temp table #DC_Sales'
		...
		FROM
			tempDB.sys.tables t
			INNER JOIN tempdb.sys.columns c ON c.object_id = t.object_id AND c.name NOT IN ('BusinessRule_MemberId', 'CurrentOrderState_MemberId', 'Scenario_MemberId', 'TimeDataView_MemberId')
		WHERE
			t.[object_id] = @TempTable_ObjectID
		ORDER BY
			c.column_id
...
/*
			...
			SELECT
				...
				[InvoiceNo_AR_MemberId] = ISNULL([InvoiceNo_AR].[MemberId], -1),
				[OrderLine_MemberId] = ISNULL([OrderLine].[MemberId], -1),
				[OrderNo_MemberId] = ISNULL([OrderNo].[MemberId], -1),
				[OrderState_MemberId] = ISNULL([OrderState].[MemberId], -1),
				--[OrderType_MemberId] = -1, --ISNULL([OrderType].[MemberId], -1),
				--[Plant_MemberId] = -1, --ISNULL([Plant].[MemberId], -1),
				[Product_MemberId] = ISNULL([Product].[MemberId], -1),
			...
			FROM
				[#DC_Sales_Raw] [Raw]
				INNER JOIN #MemberSelection MS ON MS.RNodeType = 'L'
				LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_Account] [Account] ON [Account].[Label] COLLATE DATABASE_DEFAULT = MS.[Label]
				LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_BusinessProcess] [BusinessProcess] ON [BusinessProcess].[Label] COLLATE DATABASE_DEFAULT =  @BusinessProcess
				LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_Currency] [Currency] ON [Currency].Label COLLATE DATABASE_DEFAULT = CASE WHEN MS.Unit = 'Currency' THEN [Raw].[Currency] ELSE 'NONE' END
				LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_Customer] [Customer] ON [Customer].Label COLLATE DATABASE_DEFAULT = [Raw].[Customer]
				LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_Entity] [Entity] ON [Entity].Label COLLATE DATABASE_DEFAULT = [Raw].[Entity]
				LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_InvoiceNo_AR] [InvoiceNo_AR] ON [InvoiceNo_AR].Label COLLATE DATABASE_DEFAULT = [Raw].[InvoiceNo_AR]
				LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_OrderLine] [OrderLine] ON [OrderLine].Label COLLATE DATABASE_DEFAULT = [Raw].[OrderLine]
				LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_OrderNo] [OrderNo] ON [OrderNo].Label COLLATE DATABASE_DEFAULT = [Raw].[OrderNo]
				LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_OrderState] [OrderState] ON [OrderState].Label COLLATE DATABASE_DEFAULT = [Raw].[OrderState]
	--					LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_OrderType] [OrderType] ON [OrderType].Label COLLATE DATABASE_DEFAULT = [Raw].[OrderType]
	--					LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_Plant] [Plant] ON [Plant].Label COLLATE DATABASE_DEFAULT = [Raw].[Plant]
				LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_Product] [Product] ON [Product].Label COLLATE DATABASE_DEFAULT = [Raw].[Product]
				LEFT JOIN [pcDATA_SHI].[dbo].[S_DS_TimeDay] [TimeDay] ON [TimeDay].Label COLLATE DATABASE_DEFAULT = [Raw].[TimeDay]
			WHERE
				[Raw].TimeDay >= @StartDay 
			GROUP BY
				ISNULL([Account].[MemberId], -1), --[Account_MemberId] 
				ISNULL([BusinessProcess].[MemberId], -1), --[BusinessProcess_MemberId]
				ISNULL([Currency].[MemberId], -1), --[Currency_MemberId]
				ISNULL([Customer].[MemberId], -1), --[Customer_MemberId]
				ISNULL([Entity].[MemberId], -1), --[Entity_MemberId]
				ISNULL([InvoiceNo_AR].[MemberId], -1), --[InvoiceNo_AR_MemberId]
				ISNULL([OrderLine].[MemberId], -1), --[OrderLine_MemberId]
				ISNULL([OrderNo].[MemberId], -1), --[OrderNo_MemberId]
				ISNULL([OrderState].[MemberId], -1), --[OrderState_MemberId]
	--					ISNULL([OrderType].[MemberId], -1), --[OrderType_MemberId]
	--					ISNULL([Plant].[MemberId], -1), --[Plant_MemberId]
				ISNULL([Product].[MemberId], -1), --[Product_MemberId]
				ISNULL([TimeDay].[MemberId], -1) --[TimeDay_MemberId]
			HAVING
				SUM(CASE MS.[Label]
					WHEN 'SalesPrice_' THEN [Raw].[SalesPrice_] 
					WHEN 'SalesMiscCharge_' THEN [Raw].[SalesMiscCharge_] 
