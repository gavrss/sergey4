...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_New3_2]
	@UserID int = NULL,
	@InstanceID int = NULL,
	@VersionID int = NULL,
	...
	SET @Step = 'Get parts for SQL-statement'
		IF @ResultTypeBM & 876 > 0 --4, 8, 32, 64, 256, 512
			BEGIN
				...
				SELECT
					@SQL_Select1 = @SQL_Select1 + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CASE WHEN FT.[DimensionTypeID] = 27 THEN '[' + GB.[DimensionName] + '_MemberKey] = MAX(MD' + CONVERT(nvarchar(15), GB.[LoopNo]) + '.[Leaf_MemberKey]),' + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + '[' + GB.[DimensionName] + '_MemberId] = MD' + CONVERT(nvarchar(15), GB.[LoopNo]) + '.[Leaf_MemberId],' ELSE '[' + GB.[DimensionName] + '_MemberId] = DC.[' + CASE WHEN GB.[DimensionName] = 'Time' AND @TimeType = 'TimeDay' THEN @TimeType ELSE GB.[DimensionName] END + '_MemberId],' END,
					@SQL_GroupBy1 = @SQL_GroupBy1 + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CASE WHEN FT.[DimensionTypeID] = 27 THEN 'MD' + CONVERT(nvarchar(15), GB.[LoopNo]) + '.[Leaf_MemberID],' ELSE 'DC.[' + CASE WHEN GB.[DimensionName] = 'Time' AND @TimeType = 'TimeDay' THEN @TimeType ELSE GB.[DimensionName] END + '_MemberId],' END
				FROM
					(
					SELECT
						[SortOrder] = MIN ([SortOrder]),
