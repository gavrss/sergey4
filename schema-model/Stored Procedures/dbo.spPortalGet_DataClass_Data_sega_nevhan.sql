...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[spPortalGet_DataClass_Data_sega_nevhan]
	@UserID INT = NULL,
	@InstanceID INT = NULL,
	@VersionID INT = NULL,
...
IF @GetVersion <> 0
	BEGIN
		...
		IF @Version = '2.1.2.2198' SET @Description = 'Hanlde hierarchy calculation for @ResultTypeBM = 512. Handle Scenario [InputAllowedYN] in @ResultTypeBM = 8.'
	...
	SET @Step = '@ResultTypeBM & 8'
		IF @ResultTypeBM & 8 > 0 --Changeable
			BEGIN
				...
				IF CURSOR_STATUS('global','MeasureList_8_Cursor') >= -1 DEALLOCATE MeasureList_8_Cursor
				DECLARE MeasureList_8_Cursor CURSOR FOR
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
							...
							--SET @SQLMeasureList_8 = @SQLMeasureList_8 + CHAR(13) + CHAR(10) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9) + CHAR(9)+ CHAR(9) + @MeasureName + ' = ' + CONVERT(NVARCHAR(10), @InputAllowedYN)
	...
	SET @Step = '@ResultTypeBM & 512'
		IF @ResultTypeBM & 512 > 0
			BEGIN
				...
				EXEC [sp_Tool_Generate_CreateTable] @UserID = @UserID, @InstanceID = @InstanceID, @VersionID = @VersionID, @DBName = 'tempdb', @SchemaName = 'dbo', @TableName = @TmpGlobalTable, @SQLScript_OUT = @SQLScript_Result OUTPUT
